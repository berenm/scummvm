...
SpriteSet::SpriteSet(AGSEngine *vm, Common::SeekableReadStream *stream) :
	...
	case 5: _spritesAreCompressed = true; break;
	case 6:
	...
	debug(2, "sprite set has %d sprites (version %d, %s)", spriteCount, version,
	      _spritesAreCompressed ? "compressed" : "uncompressed");
...
SpriteSet::~SpriteSet() {
	...
	for (Common::HashMap<uint, Sprite *>::iterator i = _sprites.begin();
	     i != _sprites.end(); ++i)
		delete i->_value;
}
...
bool SpriteSet::loadSpriteIndexFile(uint32 spriteFileID) {
	...
	// sprite count should match, and last index should be one beyond number of
	// sprites
	if ((spriteCount != _spriteInfo.size()) || (lastIndex != spriteCount)) {
		warning("inconsistent sprite index file (%d sprites, should be %d, "
		        "last index %d)",
	...
	for (uint i = 0; i < spriteCount; ++i) {
		...
		if (_spriteInfo[i]._offset >= (uint32) _stream->size())
			error("corrupt sprite index file (item %d has offset %x of %x", i,
			      _spriteInfo[i]._offset, _stream->size());
	}
...
Sprite *SpriteSet::getSprite(uint32 spriteId) {
	...
	switch (colorDepth) {
	case 0: return NULL; // FIXME
	case 1: format = Graphics::PixelFormat::createFormatCLUT8(); break;
	case 2: format = Graphics::PixelFormat(2, 5, 6, 5, 0, 11, 5, 0, 0); break;
	case 4:
		...
		break;
	default: error("SpriteSet::getSprite: invalid sprite depth %d", colorDepth);
	}
	...
	surface->create(_spriteInfo[spriteId]._width, _spriteInfo[spriteId]._height,
	                format);
	...
	if (_spritesAreCompressed) {
		...
		case 1:
			...
			unpackSpriteBits(_stream, (byte *) surface->getPixels(),
			                 surface->w * surface->h);
			break;
		case 2:
			unpackSpriteBits16(_stream, (uint16 *) surface->getPixels(),
			                   surface->w * surface->h);
			break;
		case 4:
			unpackSpriteBits32(_stream, (uint32 *) surface->getPixels(),
			                   surface->w * surface->h);
			break;
		}
	} else {
		_stream->read((byte *) surface->getPixels(),
		              surface->w * surface->h * colorDepth);
	}

	Graphics::PixelFormat nativeFormat = _vm->_graphics->getPixelFormat();
	if (format != nativeFormat &&
	    !(format.bytesPerPixel == 4 && nativeFormat.bytesPerPixel == 4)) {
		// FIXME: converting downward?

		debug(3, "converting sprite from %dBpp to %dBpp", format.bytesPerPixel,
		      nativeFormat.bytesPerPixel);
		...
		if (format.bytesPerPixel == 1) {
			...
			nativeFormat.colorToRGB(transColor, myPalette[0], myPalette[1],
			                        myPalette[2]);
		}

		Graphics::Surface *convertedSurf =
		    surface->convertTo(nativeFormat, myPalette);

		if (format.bytesPerPixel == 2 && nativeFormat.bytesPerPixel == 4) {
			// Converted 16bpp->32bpp, fix the transparency.

			uint16 transColor16 =
			    (uint16) _vm->_graphics->getTransparentColor(2);
...
void unpackSpriteBits16(Common::SeekableReadStream *stream, uint16 *dest,
                        uint32 size) {
	uint32 offset = 0;
...
void unpackSpriteBits32(Common::SeekableReadStream *stream, uint32 *dest,
                        uint32 size) {
	uint32 offset = 0;
