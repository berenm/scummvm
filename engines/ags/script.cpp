	...
	for (uint i = 0; i < fixupsCount; ++i) {
		...
		if (fixupTypes[i] == FIXUP_DATADATA) {
			...
			if (fixupIndex >= _code.size())
				error("fixup for %d is beyond code size %d", fixupIndex,
				      _code.size());
			if (_code[fixupIndex]._fixupType)
				error("duplicate fixup (index %d, old type %d, new type %d)",
				      fixupIndex, _code[fixupIndex]._fixupType, fixupTypes[i]);
			_code[fixupIndex]._fixupType = fixupTypes[i];

			// do some in-range sanity checks (note that imports are handled
			// below)
			uint32 data = _code[fixupIndex]._data;
			if ((fixupTypes[i] == FIXUP_GLOBALDATA) &&
			    (data >= _globalData.size()))
				error("out-of-range global data fixup (at %d, for data at %d)",
				      fixupIndex, data);
			else if ((fixupTypes[i] == FIXUP_STRING) &&
			         (data >= _strings.size()))
				error("out-of-range string fixup (at %d, for data at %d)",
				      fixupIndex, data);
		} else {
	...
	for (uint i = 0; i < _code.size(); ++i) {
		if (_code[i]._fixupType == FIXUP_IMPORT) {
			...
			if (importId >= _imports.size())
				error("invalid fixup import (at %d, for import %d)", i,
				      importId);
		}
	}
	...
	for (uint i = 0; i < exportsCount; ++i) {
		...
		case EXPORT_FUNCTION:
			...
			if (_exports[i]._address >= _code.size())
				error("out-of-range script function export '%s' (%d)",
				      _exports[i]._name.c_str(), _exports[i]._address);
		...
		case EXPORT_DATA:
			...
			if (_exports[i]._address >= _globalData.size())
				error("out-of-range script data export '%s' (%d)",
				      _exports[i]._name.c_str(), _exports[i]._address);
			_exports[i]._type = sitScriptData;
			break;
		default:
			error("script export '%s' had unknown type %d",
			      _exports[i]._name.c_str(), exportType);
		}
	}
...
ccInstance::ccInstance(AGSEngine *vm, ccScript *script, bool autoImport,
	...
	if ((_script->_instances == 1) && autoImport) {
		...
		for (uint i = 0; i < _script->_exports.size(); ++i) {
			...
			// if the name is mangled, we also want to export the non-mangled
			// version
...
ccInstance::~ccInstance() {
	...
	if (_script->_instances == 0) {
		...
		for (uint i = 0; i < _script->_exports.size(); ++i) {
			...
			ScriptImport im =
			    _vm->resolveImport(_script->_exports[i]._name, false);
			if (im._owner == this)
...
void ccInstance::call(const Common::String &name,
	...
	if (_pc != 0)
		error("attempt to call() on a running instance, when calling function "
		      "'%s'",
		      name.c_str());
	...
	for (uint i = 0; i < _script->_exports.size(); ++i) {
		...
		if (symbol._name.hasPrefix(mangledName)) {
			...
			if (paramCount != params.size())
				error("tried calling function '%s' with %d parameters, but it "
				      "takes %d",
		...
		if (symbol._type != sitScriptFunction)
			error("attempt to call() '%s' which isn't a function",
			      symbol._name.c_str());
	...
	if (codeLoc == 0xffffffff) {
		error("attempt to call() function '%s' which doesn't exist",
		      name.c_str());
	}
	...
	if (_registers[SREG_SP]._type != rvtStackPointer)
		error("runCodeFrom(): SP got clobbered (now type %d, value %d)",
		      _registers[SREG_SP]._type, _registers[SREG_SP]._value);
	if (_registers[SREG_SP]._value != params.size() * 4)
		error("call(): SP invalid at end of call (%d, for %d params)",
		      _registers[SREG_SP]._value, params.size() * 4);
...
static const char *regnames[] = {"null", "sp", "mar", "ax",
                                 "bx",   "cx", "op",  "dx"};
...
void ccInstance::runCodeFrom(uint32 start) {
	...
	while (true) {
		...
		for (uint v = 0; v < neededArgs && v < 2; ++v) {
			...
			case FIXUP_IMPORT:
				...
				case sitSystemFunction:
					...
					argVal[v]._function =
					    inst->_resolvedImports[argValue]._function;
				...
				case sitScriptFunction:
					...
					argVal[v]._instance =
					    inst->_resolvedImports[argValue]._owner;

					// if the next instruction is a system call instruction
					// (CALLEXT), change it to a script one instead
					// TODO: this is what the original engine does, but can't we
					// fix it up in CALLEXT instead?
				...
				case sitScriptData:
					...
					break;
				default:
					error("internal inconsistency (got import fixup with "
					      "import type %d)",
					      inst->_resolvedImports[argValue]._type);
				}
				debugN(4, " import@%d:%s", argValue,
				       script->_imports[argValue].c_str());
				break;
			case FIXUP_STACK:
			...
			case FIXUP_DATADATA:
			default:
				error("internal inconsistency (got fixup type %d)",
				      arg[v]._fixupType);
			}
			...
			if (arg[v]._fixupType)
				error("expected integer for param %d of %s on line %d, got "
				      "fixup (type %d)",
				      v + 1, info.name, _lineNumber, arg[v]._fixupType);
			if (argType == iatRegister && arg[v]._data >= _registers.size())
				error("expected valid register for param %d of %s on line %d, "
				      "got %d",
		...
		case SCMD_WRITELIT:
			...
			// But, conveniently, this should only ever be used to write 4-byte
			// null pointers. (Or apparently, in older versions, to invalidate
			// stack variables?)
			if (_registers[SREG_MAR]._type == rvtStackPointer) {
				if (int1 > 4 || int2 != 0)
					error("script tried using WRITELIT unsafely on stack (%d "
			...
			if (int1 != 4 || int2 != 0)
				error("script tried using WRITELIT unsafely (%d bytes of %d) "
				      "on line %d",
				      int1, int2, _lineNumber);
		...
		case SCMD_RET:
			...
			// only sabotage the sanity check until returning from the function
			// which disabled it
		...
		case SCMD_MEMREAD:
			...
			case rvtScriptData:
				...
				if (tempVal._instance->_globalObjects->contains(
				        tempVal._value)) {
					// resolves to an object
					_registers[int1] =
					    (*tempVal._instance->_globalObjects)[tempVal._value];
					break;
				}
				fixup = Common::find(instScript->_globalFixups.begin(),
				                     instScript->_globalFixups.end(),
				                     tempVal._value);
				_registers[int1] = READ_LE_UINT32(
			...
			case rvtStackPointer:
				if (tempVal._value + 4 >= _stack.size())
					error("script tried to MEMREAD from out-of-bounds stack@%d "
					      "on line %d",
				...
				break;
			default:
				error("script tried to MEMREAD from runtime value of type %d "
				      "(value %d) on line %d",
		...
		case SCMD_MEMWRITE:
			...
			case rvtScriptData:
				...
				if (_registers[int1]._type == rvtSystemObject) {
					...
					WRITE_LE_UINT32(
					    &(*tempVal._instance->_globalData)[tempVal._value], 0);
					(*tempVal._instance->_globalObjects)[tempVal._value] =
					    _registers[int1];
					break;
				}
			...
			case rvtSystemObject:
				if (_registers[int1]._type != rvtInteger)
					error("script tried to MEMWRITE runtime value of type %d "
					      ...
					      _registers[int1]._type, _registers[int1]._value,
					      _lineNumber);
				if (!tempVal._object->writeUint32(tempVal._value,
				                                  _registers[int1]._value))
					error("script failed to write uint32 to offset %d of a %s "
					      "on line %d",
					      tempVal._value, tempVal._object->getObjectTypeName(),
					      _lineNumber);
				break;
			case rvtStackPointer:
				if (tempVal._value + 4 >= _stack.size())
					error("script tried to MEMWRITE to out-of-bounds stack@%d "
					      "on line %d",
					      tempVal._value, _lineNumber);
				...
				break;
			default:
				error("script tried to MEMWRITE to runtime value of type %d "
				      "(value %d) on line %d",
				      tempVal._type, tempVal._value, _lineNumber);
			}
			break;
		case SCMD_LOADSPOFFS:
			...
			if ((uint32) int1 > _registers[SREG_SP]._value)
				error("load.sp.offs tried going %d back in a stack of size %d "
				      "on line %d",
		...
		case SCMD_ISEQUAL:
			...
			_registers[int1] =
			    _registers[int1].equalTo(_registers[int2]) ? 1 : 0;
			break;
		case SCMD_NOTEQUAL:
			// reg1 != reg2
			_registers[int1] =
			    _registers[int1].equalTo(_registers[int2]) ? 0 : 1;
			break;
		case SCMD_GREATER:
			// reg1 > reg2
			_registers[int1] =
		...
		case SCMD_AND:
			// (reg1!=0) && (reg2!=0) -> reg1
			_registers[int1] =
			    (_registers[int1]._value && _registers[int2]._value) ? 1 : 0;
			break;
		case SCMD_OR:
			// (reg1!=0) || (reg2!=0) -> reg1
			_registers[int1] =
		...
		case SCMD_MODREG:
			...
			if (_registers[int2]._value == 0)
				error("script tried to divide (modulo) by zero on line %d",
				      _lineNumber);
			_registers[int1]._value %= _registers[int2]._value;
			break;
		case SCMD_NOTREG:
		...
		case SCMD_CALL:
			...
			if (_registers[int1]._type != rvtFunction)
				error("script tried to CALL non-function runtime value of type "
				      "%d (value %d) on line %d",
				      tempVal._type, tempVal._value, _lineNumber);

			if (currentStart.size() > MAXNEST - 1)
				error("too many nested script calls on line %d, infinite "
				      "recursion?",
				      _lineNumber);
		...
		case SCMD_MEMREADB:
			...
			case rvtScriptData:
				...
				instScript = tempVal._instance->_script;
				if (tempVal._instance->_globalObjects->contains(tempVal._value))
					error("script tried MEMREADB on object on line %d",
					      _lineNumber);
				fixup = Common::find(instScript->_globalFixups.begin(),
				                     instScript->_globalFixups.end(),
				                     tempVal._value);
			...
			case rvtStackPointer:
				if (tempVal._value + 2 >= _stack.size())
					error("script tried to MEMREADB from out-of-bounds "
					      "stack@%d on line %d",
					      tempVal._value, _lineNumber);
		...
		case SCMD_MEMREADW:
			...
			case rvtScriptData:
				...
				instScript = tempVal._instance->_script;
				if (tempVal._instance->_globalObjects->contains(tempVal._value))
					error("script tried MEMREADW on object on line %d",
					      _lineNumber);
				fixup = Common::find(instScript->_globalFixups.begin(),
				                     instScript->_globalFixups.end(),
				                     tempVal._value);
			...
			case rvtStackPointer:
				if (tempVal._value + 2 >= _stack.size())
					error("script tried to MEMREADW from out-of-bounds "
					      "stack@%d on line %d",
					      tempVal._value, _lineNumber);
				if (_stack[tempVal._value]._type != rvtInteger)
		...
		case SCMD_MEMWRITEB:
			...
			tempVal = _registers[SREG_MAR];
			if (_registers[int1]._type != rvtInteger)
				error("script tried to MEMWRITEB runtime value of type %d "
				      "(value %d) on line %d",
				      _registers[int1]._type, _registers[int1]._value,
				      _lineNumber);
			// FIXME: check range?
			...
			case rvtScriptData:
				...
				break;
			case rvtSystemObject:
				if (!tempVal._object->writeByte(tempVal._value,
				                                (byte) _registers[int1]._value))
					error("script failed to write byte to offset %d of a %s on "
					      "line %d",
					      tempVal._value, tempVal._object->getObjectTypeName(),
					      _lineNumber);
				break;
		...
		case SCMD_MEMWRITEW:
			...
			tempVal = _registers[SREG_MAR];
			if (_registers[int1]._type != rvtInteger)
				error("script tried to MEMWRITEW runtime value of type %d "
				      "(value %d) on line %d",
				      _registers[int1]._type, _registers[int1]._value,
				      _lineNumber);
			// FIXME: check range?
			...
			case rvtScriptData:
				...
				break;
			case rvtSystemObject:
				if (!tempVal._object->writeUint16(
				        tempVal._value, (int16) _registers[int1]._value))
					error("script failed to write uint16 to offset %d of a %s "
					      "on line %d",
					      tempVal._value, tempVal._object->getObjectTypeName(),
					      _lineNumber);
				break;
			case rvtStackPointer:
				...
				break;
			default:
				error("script tried to MEMWRITEW to runtime value of type %d "
				      "(value %d) on line %d",
				      tempVal._type, tempVal._value, _lineNumber);
			}
			break;
		...
		case SCMD_CHECKBOUNDS:
			...
			if (_registers[int1]._type != rvtInteger)
				error("script error: checkbounds got value of type %d (not "
				      "integer)",
				      _registers[int1]._type);
			if (_registers[int1]._value >= (uint32) int2)
				error("script error: checkbounds value %d was not in range 0 "
				      "to %d",
		...
		case SCMD_DYNAMICBOUNDS:
			...
			assert(tempObj->isOfType(sotDynamicArray));
			{
				...
				if (offset < 0 || (uint) offset >= da->getMaxOffset())
					error("script error: dynamic bounds value %d was not in "
					      "range 0 to %d",
					      offset, da->getMaxOffset());
			}
			break;
		...
		case SCMD_MEMWRITEPTR:
			...
			if (_registers[int1]._type != rvtInteger ||
				...
				if (!tempObj)
					error("script tried to MEMWRITEPTR using runtime value of "
		...
		case SCMD_CHECKNULL:
			// error if MAR==0
			if (_registers[SREG_MAR]._type == rvtInteger &&
			    _registers[SREG_MAR]._value == 0)
				error("script tried to dereference null pointer on line %d",
				      _lineNumber);
			break;
		...
		case SCMD_CALLAS:
			// $callscr: call external script function
			{
				if (_registers[int1]._type != rvtScriptFunction)
					error("script tried to CALLAS non-script-function runtime "
				...
				if (_registers[SREG_SP]._value != oldsp)
					error("stack corrupt after CALLAS on line %d (was %d, now "
					      "%d)",
					      _lineNumber, oldsp, _registers[SREG_SP]._value);
		...
		case SCMD_CALLEXT:
			...
			if (_registers[int1]._type != rvtSystemFunction)
				error("script tried to CALLEXT non-system-function runtime "
				      "value of type %d (value %d) on line %d",
				      _registers[int1]._type, _registers[int1]._value,
				      _lineNumber);
			...
			if (nextCallNeedsObject) {
				...
				ScriptObject *object =
				    _registers[SREG_OP]._object->getObjectAt(offset);
				if (offset != 0)
					error("script tried to CALLEXT on system-object with "
					      "offset %d (resolved to %d) on line %d",
					      _registers[SREG_OP]._value, offset, _lineNumber);

				_registers[SREG_AX] = callImportedFunction(
				    _registers[int1]._function, object, params);
			} else {
				_registers[SREG_AX] = callImportedFunction(
		...
		case SCMD_CALLOBJ:
			...
			// set the OP register
			// (we don't check for validity here because the next call might not
			// be CALLEXT)
		...
		case SCMD_THISBASE:
			...
			break;
		case SCMD_NEWARRAY:
			// reg1 = new array of reg1 elements, each of size arg2
			// (arg3=managed type?)
			{
				...
				if (elemCount < 1 || elemCount > 1000000)
					error("invalid size (%d) requested for dynamic array",
					      elemCount);
		...
		case SCMD_FDIVREG:
			...
			if (_registers[int2]._floatValue == 0.0)
				error("script tried to divide by fp zero on line %d",
				      _lineNumber);
			_registers[int1]._floatValue /= _registers[int2]._floatValue;
			break;
		case SCMD_FADDREG:
		...
		case SCMD_ZEROMEMORY:
			...
			case rvtStackPointer:
				if (tempVal._value + (uint32) int1 >= _stack.size())
					error("script tried to ZEROMEMORY out-of-bounds "
					      "stack@%d-%d on line %d",
				...
				break;
			default:
				error("script tried to ZEROMEMORY using runtime value of type "
				      "%d (value %d) on line %d",
				      tempVal._type, tempVal._value, _lineNumber);
			}
			break;
		...
		case SCMD_STRINGSEQUAL:
			...
			_registers[int1] =
			    tempStr1->getString().equals(tempStr2->getString()) ? 1 : 0;
			tempStr1->DecRef();
			tempStr2->DecRef();
			break;
		case SCMD_STRINGSNOTEQ:
			...
			tempStr2 = createStringFrom(_registers[int2]);
			_registers[int1] =
			    tempStr1->getString().equals(tempStr2->getString()) ? 0 : 1;
			tempStr1->DecRef();
			tempStr2->DecRef();
			break;
		case SCMD_LOOPCHECKOFF:
			...
			if (loopIterationCheckDisabledCount == 0)
				loopIterationCheckDisabledCount++;
			break;
		default: error("runCodeFrom(): invalid instruction %d", instruction);
		}

		if (_registers[SREG_SP]._type != rvtStackPointer ||
		    _registers[SREG_SP]._value < 4 ||
		    _registers[SREG_SP]._value >= _stack.size())
			error("runCodeFrom(): SP got clobbered (now type %d, value %d) on "
			      "line %d",
			      _registers[SREG_SP]._type, _registers[SREG_SP]._value,
			      _lineNumber);
...
public:
	...
	void setString(const Common::String &text) {
		...
		if (text.size() >= kOldScriptStringLength)
			error("ScriptStackString: new string is too large (%d)",
			      text.size());
...
public:
	...
	void setString(const Common::String &text) {
		if (text.size() >= kOldScriptStringLength)
			error("ScriptDataString: new string is too large (%d)",
			      text.size());

		// FIXME: kill any objects?
		memcpy(&(*_instance->_globalData)[_offset], text.c_str(),
		       text.size() + 1);
	}
...
ScriptString *ccInstance::createStringFrom(RuntimeValue &value,
	...
	if (value._type == rvtStackPointer)
		...
		if (value._value != 0)
			error("createStringFrom failed to create a string from string "
			      "object with offset %d",
			      value._value);
...
ccInstance::callImportedFunction(const ScriptSystemFunctionInfo *function,
	...
	while (function->signature[pos] != '\0' &&
		...
		case 's':
			...
			break;
		default:
			error("unknown entry in signature '%s' for '%s'",
	...
	// if this is a member function, make sure it's being called on an object of
	// the right type
	if (object) {
...
RuntimeValue ccInstance::popValue() {
	...
	if (_stack[stackValue]._type == rvtInvalid)
		error("script tried to pop invalid value from stack on line %d",
		      _lineNumber);
...
uint32 ccInstance::popIntValue() {
	...
	if (val._type != rvtInteger)
		error("expected to pop an integer value off the stack (got type %d) on "
		      "line %d",
		      val._type, _lineNumber);
	return val._value;
}
...
ScriptObject *ccInstance::getObjectFrom(const RuntimeValue &value) {
	...
	case rvtScriptData:
		...
		if (value._instance->_globalObjects->contains(value._value))
			return getObjectFrom(
			    (*value._instance->_globalObjects)[value._value]);

		// no object, might still have a null pointer?
		fixup = Common::find(instScript->_globalFixups.begin(),
		                     instScript->_globalFixups.end(), value._value);
		if (fixup != instScript->_globalFixups.end())
			error("getObjectFrom got fixup for data@%d on line %d",
		...
		if (val == 0)
			...
			error("script tried to get object using global data (offset %d) on "
			      "line %d",
			      value._value, _lineNumber);
	case rvtSystemObject: result = value; break;
	case rvtStackPointer:
		if (value._value + 4 > _stack.size())
			error("script tried to get object from beyond the end of the stack "
			      "(value %d) on line %d",
			      value._value, _lineNumber);
		if (_stack[value._value]._type == rvtSystemObject)
			result = _stack[value._value];
		else if (_stack[value._value]._type == rvtInteger &&
		         _stack[value._value]._value == 0)
			return NULL;
		else
			error(
			    "script tried to get object using stack (offset %d) on line %d",
			    value._value, _lineNumber);
		break;
	default:
		error("script tried to get object using runtime value of type %d "
		      "(value %d) on line %d",
	...
	if (offset != 0)
		error("script got object using runtime value of type %d (value %d), "
		      "but offset %d was left over on a '%s' on line %d",
		      value._type, value._value, offset, obj->getObjectTypeName(),
		      _lineNumber);
	return obj;
}
...
void ccInstance::writePointer(const RuntimeValue &value, ScriptObject *object) {
	...
	case rvtSystemObject:
		// FIXME: !!!
		error("script tried to writePointer to system object (value %d) on "
		      "line %d",
		      value._value, _lineNumber);
		break;
	case rvtStackPointer:
		if (value._value + 4 >= _stack.size())
			error("script tried to writePointer to out-of-bounds stack@%d on "
			      "line %d",
			      value._value, _lineNumber);
		if (object)
		...
		break;
	default:
		error("script tried to writePointer to runtime value of type %d (value "
		      "%d) on line %d",
		      value._type, value._value, _lineNumber);
	}
}
