...
void ccInstance::runCodeFrom(uint32 start) {
	...
	while (true) {
		...
		case SCMD_MEMREAD:
			...
			case rvtScriptData:
				...
				if (instScript->_globalObjects.contains(tempVal._value)) {
					...
					_registers[int1] =
					    instScript->_globalObjects[tempVal._value];
					break;
				}
		...
		case SCMD_ISEQUAL:
			...
			if (_registers[int1]._type == rvtSystemObject &&
			    _registers[int2]._type == rvtSystemObject) {
				uint32 offset1 = _registers[int1]._value;
				ScriptObject *object1 =
				    _registers[int1]._object->getObjectAt(offset1);
				uint32 offset2 = _registers[int2]._value;
				ScriptObject *object2 =
				    _registers[int2]._object->getObjectAt(offset2);
				_registers[int1]._value =
				    (offset1 == offset2) && (object1 == object2);
			} else
				_registers[int1]._value =
				    (_registers[int1]._value == _registers[int2]._value);
			break;
