...
void ccInstance::runCodeFrom(uint32 start) {
	...
	while (true) {
		...
		case SCMD_MEMWRITE:
			...
			case rvtScriptData:
				// FIXME: bounds checks
				WRITE_LE_UINT32(&tempVal._instance->_globalData[tempVal._value],
		...
		case SCMD_CHECKBOUNDS:
			...
			if (_registers[int1]._value >= (uint32) int2)
				error(
				    "script error: checkbounds value %d was not in range to %d",
		...
		case SCMD_MEMREADPTR:
			...
			if (!tempObj)
				error("script tried to MEMREADPTR using runtime value of type "
				      "%d (value %d) on line %d",
				      _registers[SREG_MAR]._type, _registers[SREG_MAR]._value,
				      _lineNumber);
		...
		case SCMD_MEMWRITEPTR:
			...
			if (!tempObj)
				error("script tried to MEMWRITEPTR using runtime value of type "
				      "%d (value %d) on line %d",
		...
		case SCMD_CALLEXT:
			...
			if (nextCallNeedsObject) {
				...
				if (_registers[SREG_OP]._type != rvtSystemObject)
					error("script tried to CALLOBJ non-system-object runtime "
					      "value of type %d (value %d) on line %d",
					      tempVal._type, tempVal._value, _lineNumber);
				...
				if (offset != 0)
					error("script tried to CALLOBJ system-object with offset "
					      "%d (resolved to %d) on line %d",
