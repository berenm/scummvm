...
void ccInstance::runCodeFrom(uint32 start) {
	...
	while (true) {
		...
		for (uint v = 0; v < neededArgs && v < 2; ++v) {
			...
			case FIXUP_NONE: debugN(2, " %d", argVal[v]._value); break;
			case FIXUP_GLOBALDATA: argVal[v]._type = rvtGlobalData; break;
			case FIXUP_FUNCTION: argVal[v]._type = rvtFunction; break;
			case FIXUP_STRING: argVal[v]._type = rvtString; break;
			case FIXUP_IMPORT: argVal[v]._type = rvtImport; break;
			case FIXUP_STACK: argVal[v]._type = rvtStackPointer; break;
			case FIXUP_DATADATA:
			default:
		...
		case SCMD_MEMWRITEW:
		case SCMD_JZ:
		case SCMD_JNZ: error("unimplemented %s", info.name); break;
		case SCMD_PUSHREG:
			// m[sp]=reg1; sp++
		...
		case SCMD_CALLOBJ:
		case SCMD_SHIFTLEFT:
		case SCMD_SHIFTRIGHT: error("unimplemented %s", info.name); break;
		case SCMD_THISBASE:
		...
		case SCMD_STRINGSNOTEQ: error("unimplemented %s", info.name); break;
...
uint32 ccInstance::popValue() {
	...
	if (_stack[stackValue]._type != rvtInteger)
		error("expected to pop an integer value off the stack (got type %d) on "
		      "line %d",
		      _stack[stackValue]._type, _lineNumber);
