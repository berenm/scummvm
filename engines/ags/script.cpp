	...
	while (true) {
		...
		for (uint v = 0; v < neededArgs && v < 2; ++v) {
			...
			case FIXUP_STRING:
				...
				debugN(2, " string@%d\"%s\"", argValue,
				       &_script->_strings[argValue]);
				break;
			case FIXUP_IMPORT:
				...
				case sitScriptData:
					...
					break;
				default:
					error("internal inconsistency (got import fixup type %d)",
					      _resolvedImports[argValue]._type);
				}
				debugN(2, " import@%d:%s", argValue,
				       _script->_imports[argValue].c_str());
				break;
			case FIXUP_STACK:
		...
		case SCMD_CALLEXT:
			...
			debug(2, "calling external function '%s'",
			      _script->_imports[_registers[int1]._value].c_str());
			...
			if (nextCallNeedsObject) {
				...
				_registers[SREG_AX] =
				    callImportedFunction(_registers[int1]._function, params);
			} else {
				_registers[SREG_AX] =
				    callImportedFunction(_registers[int1]._function, params);
			}
...
RuntimeValue
ccInstance::callImportedFunction(ScriptAPIFunction *function,
