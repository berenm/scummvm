...
bool MoveList::doStep(Common::Point &pos) {
	...
	if (_doneX) {
		...
		if ((stage.xPerMove & 0xffff0000) == 0xffff0000 ||
		    (stage.xPerMove & 0xffff0000) == 0)
			adjAmnt = 2;
		...
		if (stage.yPerMove != 0) {
			if ((stage.yPerMove & 0xffff0000) == 0) {
				...
				targetY -= adjAmnt;
			} else if ((uint) stage.yPerMove != 0xffff0000 &&
			           (stage.yPerMove & 0xffff0000) == 0xffff0000) {
				// Y per move is > -1, so finish the move
				targetY += adjAmnt;
			}
		}
		...
		pos.x =
		    _from.x + (int) (fracToDouble(stage.xPerMove) * (double) _curPart);

	// same as above, for Y
	if (_doneY) {
		int adjAmnt = 3;

		if ((stage.yPerMove & 0xffff0000) == 0xffff0000 ||
		    (stage.yPerMove & 0xffff0000) == 0)
			adjAmnt = 2;

		if (stage.xPerMove != 0) {
			if ((stage.xPerMove & 0xffff0000) == 0) {
				targetX -= adjAmnt;
			} else if ((uint) stage.xPerMove != 0xffff0000 &&
			           (stage.xPerMove & 0xffff0000) == 0xffff0000) {
				targetX += adjAmnt;
			}
		}
	} else
		pos.y =
		    _from.y + (int) (fracToDouble(stage.yPerMove) * (double) _curPart);

	// did we finish horizontal movement?
	if ((stage.xPerMove > 0 && pos.x >= targetX) ||
	    (stage.xPerMove < 0 && pos.x <= targetX)) {
		_doneX = true;
		pos.x = targetX;
	...
	if ((stage.yPerMove > 0 && pos.y >= targetY) ||
	    (stage.yPerMove < 0 && pos.y <= targetY)) {
		_doneY = true;
		pos.y = targetY;
...
protected:
	...
	bool tryThisSquare(const Common::Point &from, const Common::Point &to,
	                   bool goingRight);

	bool dijkstraCheckNeighbor(uint x, uint y, uint nX, uint nY, int modifier,
	                           int &min, Common::Array<uint> &found,
	                           Common::Array<uint> &cheapest);
	bool findNearestWalkableArea(const Graphics::Surface &tempMask, int startX,
	                             int startY, int endX, int endY, uint step);
};
...
bool PathFinder::findPath(bool onlyIfDestAllowed) {
	...
	bool isStraight = ((from.x == _dest.x) || (from.y == _dest.y) ||
	                   (abs(from.x - _dest.x) == abs(from.y - _dest.y)));
...
bool PathFinder::findNearestWalkableArea(const Graphics::Surface &tempMask,
	...
	for (int x = startX; x < endX; x += step) {
		for (int y = startY; y < endY; y += step) {
			...
			uint distance = (uint) sqrt((x - _dest.x) * (x - _dest.x) +
			                            (y - _dest.y) * (y - _dest.y));
			if (distance < nearest) {
				nearest = distance;
...
bool PathFinder::isRoutePossible(bool &foundNewCandidate) {
	...
	if (*(const byte *) _mask->getBasePtr(_moveList->_from.x,
	                                      _moveList->_from.y) == 0) {
		warning("refusing to route from unwalkable point %d,%d",
		        _moveList->_from.x, _moveList->_from.y);
		return false;
	}
	...
	// TODO: shouldn't this really stop at the end of each row/column? and reset
	...
	for (uint y = 0; y < tempMask.h; ++y) {
		for (uint x = 0; x < tempMask.w; ++x) {
			...
			// TODO: verify the walkable mask before we get here, error check
			// here would be silly
	...
	if (*(byte *) tempMask.getBasePtr(_dest.x, _dest.y) != 232) {
		...
		if (!findNearestWalkableArea(tempMask, _dest.x - 50, _dest.y - 50,
		                             _dest.x + 50, _dest.y + 50, 3)) {
			// then sweep the whole room at 5-pixel granularity
			if (!findNearestWalkableArea(tempMask, 0, 0, tempMask.w, tempMask.h,
			                             5))
				foundNewCandidate = false;
		}
...
Common::Point PathFinder::roundDownCoordinates(Common::Point pos) {
	int startGranularity =
	    _walkAreaGranularity[*(const byte *) _mask->getBasePtr(pos.x, pos.y)];
	...
	if (*(const byte *) _mask->getBasePtr(pos.x, pos.y) == 0) {
		...
		if ((pos.y < _mask->h - startGranularity) &&
		    *(const byte *) _mask->getBasePtr(pos.x, pos.y) == 0) {
...
bool PathFinder::dijkstraCheckNeighbor(uint x, uint y, uint nX, uint nY,
                                       int modifier, int &min,
	...
	// TODO: Agh, this is actually necessary, otherwise it spends forever
	// checking the same cells.
	if (found.size() >= 40)
		return true;

	// Add the neighbor and cell to the lists, if we're the cheapest path to it
	// so far.
...
bool PathFinder::findRouteDijkstra() {
	...
	*(int16 *) _beenHere.getBasePtr(from.x, from.y) = 0;
	...
	// We store the previously-visited 'parent' cells here, for later tracing
	// back.
	Graphics::Surface parent;
	parent.create(_beenHere.w, _beenHere.h,
	              Graphics::PixelFormat(4, 0, 0, 0, 0, 0, 0, 0, 0));
	...
	while (iteration < 5000 && foundAnswer == (uint) -1) {
		...
		// Update each visited cell, to take into account newly-visited
		// neighbours.
		for (uint i = 0; i < iteration; ++i) {
			...
			int granularity =
			    _walkAreaGranularity[*(const byte *) _mask->getBasePtr(x, y)];

			bool updated = false;

			if (x >= granularity)
				updated |= dijkstraCheckNeighbor(
				    x, y, x - granularity, y,
				    (_dest.x < x) ? directionBonus : 0, min, found, cheapest);

			if (y >= granularity)
				updated |= dijkstraCheckNeighbor(
				    x, y, x, y - granularity,
				    (_dest.y < y) ? directionBonus : 0, min, found, cheapest);
			...
			if (!updated) {
				// If all the adjacent cells have been done, stop checking this
				// one
		...
		for (uint i = 0; i < found.size(); ++i) {
			...
			*(int16 *) _beenHere.getBasePtr(newX, newY) =
			    *(int16 *) _beenHere.getBasePtr(cheapestX, cheapestY) + 1;

			// And that neighbor is our 'parent'.
			parentVals[found[i]] = cheapest[i];

			// The edges of the screen pose a problem, so if the current
			// position and the destination are both within a certain distance
			// of the edge, just snap to the destination.
			if ((newX >= _mask->w - MAX_GRANULARITY) &&
			    (_dest.x >= _mask->w - MAX_GRANULARITY))
				newX = _dest.x;
			if ((newY >= _mask->h - MAX_GRANULARITY) &&
			    (_dest.y >= _mask->h - MAX_GRANULARITY))
				newY = _dest.y;

			if ((newX >= _dest.x - MAX_GRANULARITY) &&
			    (newX <= _dest.x + MAX_GRANULARITY) &&
			    (newY >= _dest.y - MAX_GRANULARITY) &&
			    (newY <= _dest.y + MAX_GRANULARITY)) {
			...
			if (totalFound >= 1000) {
				...
				// TODO: This seems silly, and original says "Doesn't work cos
				// it can see the destination from the point that's not
				// nearest".
	...
	for (uint on = parentVals[foundAnswer]; on != (uint) -1;
	     on = parentVals[on]) {
...
bool PathFinder::tryThisSquare(const Common::Point &from,
                               const Common::Point &to, bool goingRight) {
	return false; // FIXME
}
...
bool PathFinder::constructPath() {
	...
	while (lastRelevantPosIndex != 0) {
		...
		if (!foundPos && lastRelevantPosIndex != 0) {
			// If we have a path but we didn't find any next stage we could
			// see..
			if (!canSee(pos, _dest, _mask)) {
				// We're stuck in a corner so advance to the next square anyway.
				if (pos.x >= 0 && pos.y >= 0 && pos.x < _mask->w &&
				    pos.y < _mask->h) {
...
// Calculate the x/y movement per frame, for this stage (moving to the one after
// it) of the movelist.
void MoveList::calculateMoveStage(uint stageId) {
	...
	if (_moveSpeedX != _moveSpeedY) {
		...
		if (_moveSpeedX > _moveSpeedY) {
			...
			useMoveSpeed =
			    _moveSpeedY + fracMul(xProportion, _moveSpeedX - _moveSpeedY);
		} else {
			// speed = x + (xproportion * (y - x))
			useMoveSpeed = _moveSpeedX + fracMul(intToFrac(1) - xProportion,
			                                     _moveSpeedY - _moveSpeedX);
		}
	}
 ...
 */
void do_line(BITMAP *bmp, int x1, int y1, int x2, int y2, int d,
...
bool canSee(const Common::Point &from, const Common::Point &to,
            const Graphics::Surface *mask, Common::Point *lastGoodPos) {
	if (from == to)
		return true;
...
bool findPath(AGSEngine *vm, const Common::Point &from, const Common::Point &to,
              const Graphics::Surface *mask, MoveList *moveList, int speedX,
              int speedY, bool onlyIfDestAllowed, bool ignoreWalls) {
	...
	pathfinder._dest = to;
	pathfinder._beenHere.create(
	    mask->w, mask->h, Graphics::PixelFormat(2, 0, 0, 0, 0, 0, 0, 0, 0));
	...
	if (vm->getGameOption(OPT_NATIVECOORDINATES) &&
	    vm->_gameFile->_defaultResolution > 2)
