...
public:
	...
	uint createTextOverlayCore(int x, int y, uint width, uint fontId, int color,
	                           const Common::String &text, int allowShrink);
	uint createTextOverlay(int x, int y, uint width, uint fontId, int color,
	                       const Common::String &text);
	uint addScreenOverlay(int x, int y, uint type,
	                      const Graphics::Surface &surface, bool alphaChannel);
	...
	void displayAt(int x, int y, int width, Common::String text, int blocking,
	               int asSpeech = 0, bool isThought = false,
	               int allowShrink = 0, bool overlayPositionFixed = false);
	uint displayMain(int x, int y, int width, const Common::String &text,
	                 int blocking, int usingFont, int asSpeech, bool isThought,
	                 int allowShrink, bool overlayPositionFixed);
	void displaySpeech(Common::String text, uint charId, int x = -1, int y = -1,
	                   int width = -1, bool isThought = false);
	...
	bool runCharacterInteraction(uint charId, uint mode,
	                             bool checkOnly = false);
	bool runInventoryInteraction(uint itemId, uint mode,
	                             bool checkOnly = false);
	bool runHotspotInteraction(uint hotspotId, uint mode,
	                           bool checkOnly = false);
