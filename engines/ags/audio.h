...
public:
	...
	uint findFreeAudioChannel(AudioClip &clip, int priority,
	...
	uint playAudioClip(AudioClip &clip, int priority, uint repeat,
	                   uint fromOffset = 0, bool queueIfNoChannel = false);
	uint playAudioClipOnChannel(uint channelId, AudioClip &clip, int priority,
	                            bool repeat, uint fromOffset = 0);
	...
	getAudioResource(const Common::String &filename);

	void stopOrFadeOutChannel(uint channelId, uint newChannelId,
	                          AudioClip *clip);
...
protected:
	...
	void moveTrackToCrossfadeChannel(uint channelId, uint speed,
	                                 uint fadeInChannel, AudioClip *clip);
};
