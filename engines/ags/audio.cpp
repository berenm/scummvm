...
bool AudioChannel::playSound(Common::SeekableReadStream *stream,
	...
	if (repeat) {
		// FIXME: horrible
		Audio::AudioStream *streamToPlay =
		    new Audio::LoopingAudioStream(_stream, 0, DisposeAfterUse::NO);
		_vm->_mixer->playStream(
		    Audio::Mixer::kSFXSoundType, &_handle, streamToPlay, -1,
		    Audio::Mixer::kMaxChannelVolume, 0, DisposeAfterUse::YES);
	} else
		_vm->_mixer->playStream(Audio::Mixer::kSFXSoundType, &_handle, _stream,
...
void AudioChannel::setVolume(uint volume) {
	...
	// TODO: The original engine seems rather inconsistent about setting this,
	// take a look at it.
	_volume = volume;
}

void AudioChannel::setPanning(int panning) {
	// FIXME: this will need a reset when the current AudioClip has finished
	// playing
