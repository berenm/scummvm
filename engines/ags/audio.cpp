...
AGSAudio::AGSAudio(AGSEngine *vm) :
    _vm(vm), _musicResources(NULL), _audioResources(NULL),
...
// old-style init: older versions don't store any audio information, so this is
// used instead of initFrom
void AGSAudio::init() {
...
void AGSAudio::addAudioResourcesFrom(ResourceManager *manager,
                                     bool isExecutable) {
	...
	for (uint i = 0; i < filenames.size(); ++i) {
		...
		// WORKAROUND: Old-style games can have clips with identical IDs. See
		// Apprentice 2 for example.
		// FIXME: Does this break things?
		bool resolved = false;
		while (!resolved) {
			resolved = true;
			for (uint n = 0; n < _audioClips.size(); ++n)
				if (_audioClips[n]._scriptName.equalsIgnoreCase(
				        newScriptName)) {
		...
		} else if (extension == "mod" || extension == "xm" ||
		           extension == "s3m" || extension == "it") {
			clip._fileType = kAudioFileMOD;
		} else
			error("AudioClip file '%s' had unknown extension '%s'",
...
AGSAudio::getAudioResource(const Common::String &filename) {
...
void AGSAudio::initFrom(Common::SeekableReadStream *stream) {
	...
	for (uint i = 0; i < audioClipTypeCount; ++i) {
		...
		_audioClipTypes[i]._volumeReductionWhileSpeechPlaying =
		    stream->readUint32LE();
...
uint AGSAudio::getLastChannelToUseFor(uint clipType) {
	if (clipType == (uint) -1 ||
	    _audioClipTypes[clipType]._reservedChannels == 0)
...
uint AGSAudio::findFreeAudioChannel(AudioClip &clip, int priority,
                                    bool interruptEqualPriority) {
	...
	for (uint i = startAtChannel; i < endAtChannel; ++i) {
		AudioClip *otherClip =
		    _channels[i]->getClip(); // avoid race: getClip() can return NULL if
		                             // !isPlaying()
...
void AGSAudio::queueAudioClipToPlay(AudioClip &clip, int priority,
                                    bool repeat) {
...
uint AGSAudio::playAudioClip(AudioClip &clip, int priority, uint repeat,
                             uint fromOffset, bool queueIfNoChannel) {
	if (!queueIfNoChannel)
	...
	if (channel != (uint) -1)
		return playAudioClipOnChannel(channel, clip, priority, doRepeat,
		                              fromOffset);

	if (queueIfNoChannel)
		queueAudioClipToPlay(clip, priority, repeat);
	else
		debug(2, "playAudioClip: no channels available for clip of priority %d",
		      priority);

	return channel;
}

uint AGSAudio::playAudioClipOnChannel(uint channelId, AudioClip &clip,
                                      int priority, bool repeat,
                                      uint fromOffset) {
...
bool AGSAudio::playSoundOnChannel(uint soundId, uint channelId) {
	...
	if (channelId >= _channels.size() - 1)
		error("playSoundOnChannel: channel %d is too high (only %d channels)",
		      channelId, _channels.size());
	if (channelId < SCHAN_NORMAL)
		error("playSoundOnChannel: channel %d is a reserved channel",
		      channelId);
...
bool AGSAudio::isMusicPlaying() {
	if (_vm->_state->_fastForward &&
	    _vm->_state->_skipUntilCharStops == (uint) -1)
		return false;
...
void AGSAudio::playAmbientSound(uint channelId, uint soundId, uint volume,
	...
	if (channelId >= _channels.size() - 1)
		error("playAmbientSound: channel %d is too high (only %d channels)",
		      channelId, _channels.size());
	if (channelId == SCHAN_SPEECH)
		error("playAmbientSound: attempt to play ambient sound on speech "
		      "channel");
	if (volume < 1 || volume > 255)
		error("playAmbientSound: volume %d is invalid (must be 1-255)", volume);

	if (_ambients[channelId]._channel == 0 ||
	    !_channels[_ambients[channelId]._channel]->isPlaying() ||
		...
		channel->setPriority(
		    15); // ambient sound higher priority than normal sfx
	}
...
void AGSAudio::stopAmbientSound(uint channelId) {
	if (channelId >= _channels.size() - 1)
		error("stopAmbientSound: channel %d is too high (only %d channels)",
		      channelId, _channels.size());
...
void AGSAudio::setSoundVolume(uint volume) {
	...
	setAudioTypeVolume(AUDIOTYPE_LEGACY_AMBIENT_SOUND, (volume * 100) / 255,
	                   VOL_BOTH);
...
void AGSAudio::startFadingInNewTrackIfApplicable(uint channelId,
                                                 AudioClip &clip) {
...
void AGSAudio::moveTrackToCrossfadeChannel(uint channelId, uint speed,
                                           uint fadeInChannel,
                                           AudioClip *clip) {
}

void AGSAudio::stopOrFadeOutChannel(uint channelId, uint newChannelId,
                                    AudioClip *clip) {
	...
	if (clipType._crossfadeSpeed != (uint) -1 &&
	    clipType._crossfadeSpeed != 0) {
		moveTrackToCrossfadeChannel(channelId, clipType._crossfadeSpeed,
		                            newChannelId, clip);
	}
}
...
void AGSAudio::registerScriptObjects() {
	for (uint i = 0; i < _audioClips.size(); ++i)
		_vm->getScriptState()->addSystemObjectImport(_audioClips[i]._scriptName,
		                                             &_audioClips[i]);
}
...
AudioChannel::AudioChannel(AGSEngine *vm, uint id) :
    _vm(vm), _id(id), _valid(false), _rewind(0), _repeat(false),
    _panning(PAN_CENTER) {
}
...
bool AudioChannel::playSound(AudioClip *clip, bool repeat) {
	...
	if (!stream) {
		warning("AudioChannel::playSound: failed to open file '%s'",
		        clip->_filename.c_str());
		return false;
	}
...
bool AudioChannel::playSound(Common::SeekableReadStream *stream,
                             AudioFileType fileType, bool repeat) {
	_clip = NULL;
	_stream = NULL;
	...
	case kAudioFileWAV: {
		...
		if (Audio::loadWAVFromStream(*stream, size, rate, rawFlags))
			_stream =
			    Audio::makeRawStream(stream->readStream(size), rate, rawFlags);
		else
	...
	DisposeAfterUse::Flag dispose =
	    repeat ? DisposeAfterUse::NO : DisposeAfterUse::YES;
	_vm->_mixer->playStream(Audio::Mixer::kSFXSoundType, &_handle, _stream, -1,
	                        Audio::Mixer::kMaxChannelVolume, false, dispose);
...
// FIXME
// Looping audiostreams aren't seekable, and the sound handle for a seekable
// audiostream is reset when the stream has ended. So to fake a repeat: rewind
// the existing stream, play it again, and apply the existing volume and
// balance.
...
void AudioChannel::setPanning(int panning) {
	// CHECKME: compare floor with original Allegro conversion to
	// IDirectSoundBuffer::SetPan

	// alleg_to_dsound_pan[0] = DSBPAN_LEFT;
	// for (v = 1; v < 128; v++)
	//	alleg_to_dsound_pan[v] = MAX(DSBPAN_LEFT, DSBPAN_CENTER + 2000.0*log10(v
	/// 127.0));
	//
	// alleg_to_dsound_pan[255] = DSBPAN_RIGHT;
	// for (v = 128; v < 255; v++)
	//	alleg_to_dsound_pan[v] = MIN(DSBPAN_RIGHT, DSBPAN_CENTER -
	// 2000.0*log10((255.0 - v) / 127.0));

	if (panning < -100 || panning > 100)
		warning("AudioChannel.Panning: panning value must be between -100 and "
		        "100 (passed=%d)",
		        panning);
...
uint32 AudioChannel::getPosition() {
	...
	case kAudioFileWAV:
		...
		return (_stream->getRate() / 1000) *
		       (_vm->_mixer->getSoundElapsedTime(_handle) - _rewind);
		break;
	case kAudioFileOGG:
	case kAudioFileMP3:
		return _vm->_mixer->getSoundElapsedTime(_handle) - _rewind;
