...
RuntimeValue
Script_ViewFrame_get_Flipped(AGSEngine *vm, ViewFrame *self,
...
RuntimeValue
Script_ViewFrame_get_Frame(AGSEngine *vm, ViewFrame *self,
                           const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_ViewFrame_get_Graphic(AGSEngine *vm, ViewFrame *self,
                             const Common::Array<RuntimeValue> &params) {
	return self->_pic;
}
...
// Gets/sets the sprite that is displayed by this frame.
RuntimeValue
Script_ViewFrame_set_Graphic(AGSEngine *vm, ViewFrame *self,
                             const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_ViewFrame_get_LinkedAudio(AGSEngine *vm, ViewFrame *self,
                                 const Common::Array<RuntimeValue> &params) {
	// FIXME
...
// Gets/sets the audio that is played when this frame comes around.
RuntimeValue
Script_ViewFrame_set_LinkedAudio(AGSEngine *vm, ViewFrame *self,
                                 const Common::Array<RuntimeValue> &params) {
	if (!params[0]._object->isOfType(sotAudioClip))
		error("ViewFrame::set_LinkedAudio got incorrect object type (expected "
		      "a AudioClip) for parameter 1");
...
RuntimeValue
Script_ViewFrame_get_Loop(AGSEngine *vm, ViewFrame *self,
                          const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_ViewFrame_get_Sound(AGSEngine *vm, ViewFrame *self,
                           const Common::Array<RuntimeValue> &params) {
	// FIXME
...
// Gets/sets the sound that is played when this frame comes around.
RuntimeValue
Script_ViewFrame_set_Sound(AGSEngine *vm, ViewFrame *self,
                           const Common::Array<RuntimeValue> &params) {
	int value = params[0]._signedValue;
...
RuntimeValue
Script_ViewFrame_get_View(AGSEngine *vm, ViewFrame *self,
                          const Common::Array<RuntimeValue> &params) {
	// FIXME
...
// Clears the surface to the specified colour, or transparent if you do not
// specify a colour.
RuntimeValue
Script_DrawingSurface_Clear(AGSEngine *vm, DrawingSurface *self,
                            const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_DrawingSurface_CreateCopy(AGSEngine *vm, DrawingSurface *self,
                                 const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_DrawingSurface_DrawCircle(AGSEngine *vm, DrawingSurface *self,
                                 const Common::Array<RuntimeValue> &params) {
...
// DrawingSurface: import void DrawImage(int x, int y, int spriteSlot, int
// transparency=0, int width=SCR_NO_VALUE, int height=SCR_NO_VALUE) Draws a
// sprite onto the surface with its top-left corner at (x,y).
RuntimeValue
Script_DrawingSurface_DrawImage(AGSEngine *vm, DrawingSurface *self,
                                const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;
	...
	if (spriteSlot >= vm->getSprites()->getSpriteCount())
		error("DrawingSurface::DrawImage: sprite %d is too high (only have %d "
		      "sprites)",
		      spriteSlot, vm->getSprites()->getSpriteCount());
	if (transparency > 100)
		error("DrawingSurface::DrawImage: transparency value must be between 0 "
		      "and 100, but got %d",
		      transparency);
	...
	if (dest->format != source->_surface->format)
		error("DrawingSurface::DrawImage: Incompatible formats (source is "
		      "%dBpp, dest is %dBpp)",
		      source->_surface->format.bytesPerPixel,
		      dest->format.bytesPerPixel);

	vm->_graphics->blit(source->_surface, dest, Common::Point(x, y),
	                    transparency);
	// FIXME: free sprite
...
// DrawingSurface: import void DrawLine(int x1, int y1, int x2, int y2, int
// thickness=1) Draws a straight line between the two points on the surface.
RuntimeValue
Script_DrawingSurface_DrawLine(AGSEngine *vm, DrawingSurface *self,
                               const Common::Array<RuntimeValue> &params) {
...
// DrawingSurface: import void DrawMessageWrapped(int x, int y, int width,
// FontType, int messageNumber) Draws a message from the Room Message Editor,
// wrapping at the specified width.
RuntimeValue Script_DrawingSurface_DrawMessageWrapped(
    AGSEngine *vm, DrawingSurface *self,
    const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
RuntimeValue
Script_DrawingSurface_DrawPixel(AGSEngine *vm, DrawingSurface *self,
                                const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
RuntimeValue
Script_DrawingSurface_DrawRectangle(AGSEngine *vm, DrawingSurface *self,
                                    const Common::Array<RuntimeValue> &params) {
	int x1 = params[0]._signedValue;
	UNUSED(x1);
	int y1 = params[1]._signedValue;
...
// DrawingSurface: import void DrawString(int x, int y, FontType, const string
// text, ...) Draws the specified text to the surface.
RuntimeValue
Script_DrawingSurface_DrawString(AGSEngine *vm, DrawingSurface *self,
                                 const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
// DrawingSurface: import void DrawStringWrapped(int x, int y, int width,
// FontType, Alignment, const string text) Draws the text to the surface,
// wrapping it at the specified width.
RuntimeValue Script_DrawingSurface_DrawStringWrapped(
    AGSEngine *vm, DrawingSurface *self,
    const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
...
// DrawingSurface: import void DrawSurface(DrawingSurface *surfaceToDraw, int
// transparency=0) Draws the specified surface onto this surface.
RuntimeValue
Script_DrawingSurface_DrawSurface(AGSEngine *vm, DrawingSurface *self,
                                  const Common::Array<RuntimeValue> &params) {
	if (!params[0]._object->isOfType(sotDrawingSurface))
		error("DrawingSurface::DrawSurface got incorrect object type (expected "
		      "a DrawingSurface) for parameter 1");
	DrawingSurface *surfaceToDraw = (DrawingSurface *) params[0]._object;
	uint transparency = params[1]._value;

	if (transparency > 100)
		error("DrawingSurface::DrawSurface: transparency value must be between "
		      "0 and 100, but got %d",
		      transparency);
	...
	if (dest == src)
		error("DrawingSurface::DrawSurface: attempted to draw a surface onto "
		      "itself");
	...
	if (src->format.bytesPerPixel == 1)
		error("DrawingSurface::DrawSurface: 8bpp surfaces cannot be drawn "
		      "transparently");
...
// DrawingSurface: import void DrawTriangle(int x1, int y1, int x2, int y2, int
// x3, int y3) Draws a filled triangle onto the surface.
RuntimeValue
Script_DrawingSurface_DrawTriangle(AGSEngine *vm, DrawingSurface *self,
                                   const Common::Array<RuntimeValue> &params) {
	int x1 = params[0]._signedValue;
	UNUSED(x1);
	int y1 = params[1]._signedValue;
...
RuntimeValue
Script_DrawingSurface_GetPixel(AGSEngine *vm, DrawingSurface *self,
                               const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
RuntimeValue
Script_DrawingSurface_Release(AGSEngine *vm, DrawingSurface *self,
                              const Common::Array<RuntimeValue> &params) {
	self->release();
...
// Gets/sets the current AGS Colour Number that will be used for drawing onto
// this surface.
RuntimeValue Script_DrawingSurface_get_DrawingColor(
    AGSEngine *vm, DrawingSurface *self,
    const Common::Array<RuntimeValue> &params) {
	// FIXME
...
// Gets/sets the current AGS Colour Number that will be used for drawing onto
// this surface.
RuntimeValue Script_DrawingSurface_set_DrawingColor(
    AGSEngine *vm, DrawingSurface *self,
    const Common::Array<RuntimeValue> &params) {
	int value = params[0]._signedValue;
	UNUSED(value);
...
RuntimeValue
Script_DrawingSurface_get_Height(AGSEngine *vm, DrawingSurface *self,
                                 const Common::Array<RuntimeValue> &params) {
	// FIXME
...
// Determines whether you use high-res or low-res co-ordinates for drawing onto
// this surface.
RuntimeValue Script_DrawingSurface_get_UseHighResCoordinates(
    AGSEngine *vm, DrawingSurface *self,
    const Common::Array<RuntimeValue> &params) {
	// FIXME
...
// Determines whether you use high-res or low-res co-ordinates for drawing onto
// this surface.
RuntimeValue Script_DrawingSurface_set_UseHighResCoordinates(
    AGSEngine *vm, DrawingSurface *self,
    const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_DrawingSurface_get_Width(AGSEngine *vm, DrawingSurface *self,
                                const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue Script_Display(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_DisplayAt(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
RuntimeValue Script_DisplayAtY(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_DisplayMessage(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
...
// Displays a message from the Room Message Editor at the specified
// y-coordinate.
RuntimeValue
Script_DisplayMessageAtY(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	uint messageNumber = params[0]._value;
	int y = params[1]._signedValue;
...
// import void DisplayTopBar(int y, int textColor, int backColor, const string
// title, const string text, ...) Displays a message in a text window with a
// title, used for speech in SCI0 games.
RuntimeValue Script_DisplayTopBar(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	int y = params[0]._signedValue;
	UNUSED(y);
...
// import void DisplayMessageBar(int y, int textColor, int backColor, const
// string title, int message) Displays a Room Message Editor message in a text
// window with a title, used for speech in SCI0 games.
RuntimeValue
Script_DisplayMessageBar(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	int y = params[0]._signedValue;
	UNUSED(y);
	int textColor = params[1]._signedValue;
...
RuntimeValue Script_SetViewport(AGSEngine *vm, ScriptObject *,
                                const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;
...
RuntimeValue Script_ReleaseViewport(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_GetViewportX(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_GetViewportY(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue
Script_CreateGraphicOverlay(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
// import int CreateTextOverlay(int x, int y, int width, FontType, int colour,
// const string text, ...) Undocumented.
RuntimeValue
Script_CreateTextOverlay(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;
	int width = params[2]._signedValue;
	...
	return vm->createTextOverlay(x, y, width, fontId, colour,
	                             text->getString());
}

// import void SetTextOverlay(int overlayID, int x, int y, int width, FontType,
// int colour, const string text, ...) Undocumented.
RuntimeValue Script_SetTextOverlay(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue Script_RemoveOverlay(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	int overlayID = params[0]._signedValue;
...
// Undocumented.
RuntimeValue Script_MoveOverlay(AGSEngine *vm, ScriptObject *,
                                const Common::Array<RuntimeValue> &params) {
	int overlayID = params[0]._signedValue;
	UNUSED(overlayID);
	int x = params[1]._signedValue;
...
// Undocumented.
RuntimeValue Script_IsOverlayValid(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue Script_LoadImageFile(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue Script_DeleteSprite(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue Script_SetSpeechFont(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue Script_SetNormalFont(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	uint32 fonttype = params[0]._value;
...
RuntimeValue Script_RawClearScreen(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	int colour = params[0]._signedValue;
	UNUSED(colour);
...
// Obsolete raw draw stuff.
RuntimeValue Script_RawDrawCircle(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
// Obsolete raw draw stuff.
RuntimeValue Script_RawDrawImage(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
// Obsolete raw draw stuff.
RuntimeValue
Script_RawDrawImageOffset(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
// import void RawDrawImageResized(int x, int y, int spriteSlot, int width, int
// height) Obsolete raw draw stuff.
RuntimeValue
Script_RawDrawImageResized(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
// import void RawDrawImageTransparent(int x, int y, int spriteSlot, int
// transparency) Obsolete raw draw stuff.
RuntimeValue
Script_RawDrawImageTransparent(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
// Obsolete raw draw stuff.
RuntimeValue Script_RawDrawLine(AGSEngine *vm, ScriptObject *,
                                const Common::Array<RuntimeValue> &params) {
	int x1 = params[0]._signedValue;
	UNUSED(x1);
	int y1 = params[1]._signedValue;
...
// Obsolete raw draw stuff.
RuntimeValue
Script_RawDrawRectangle(AGSEngine *vm, ScriptObject *,
                        const Common::Array<RuntimeValue> &params) {
	int x1 = params[0]._signedValue;
	UNUSED(x1);
	int y1 = params[1]._signedValue;
...
// Obsolete raw draw stuff.
RuntimeValue Script_RawDrawTriangle(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	int x1 = params[0]._signedValue;
	UNUSED(x1);
	int y1 = params[1]._signedValue;
...
// Obsolete raw draw stuff.
RuntimeValue Script_RawPrint(AGSEngine *vm, ScriptObject *,
                             const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
// import void RawPrintMessageWrapped (int x, int y, int width, FontType, int
// messageNumber) Obsolete raw draw stuff.
RuntimeValue
Script_RawPrintMessageWrapped(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
// Obsolete raw draw stuff.
RuntimeValue Script_RawSetColor(AGSEngine *vm, ScriptObject *,
                                const Common::Array<RuntimeValue> &params) {
	int colour = params[0]._signedValue;
	UNUSED(colour);
...
// Obsolete raw draw stuff.
RuntimeValue Script_RawSetColorRGB(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
...
// Obsolete raw draw stuff.
RuntimeValue
Script_RawDrawFrameTransparent(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
...
// Obsolete raw draw stuff.
RuntimeValue Script_RawSaveScreen(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	// FIXME
...
// Obsolete raw draw stuff.
RuntimeValue
Script_RawRestoreScreen(AGSEngine *vm, ScriptObject *,
                        const Common::Array<RuntimeValue> &params) {
	// FIXME
...
// Obsolete raw draw stuff.
RuntimeValue
Script_RawRestoreScreenTinted(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
	int red = params[0]._signedValue;
	UNUSED(red);
	int green = params[1]._signedValue;
...
// Gets the height of the specified text in the specified font when wrapped at
// the specified width
...
// transparent) Creates an overlay that displays a sprite.
RuntimeValue
Script_Overlay_CreateGraphical(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
// Overlay: import static Overlay* CreateTextual(int x, int y, int width,
// FontType, int colour, const string text, ...) Creates an overlay that
// displays some text.
RuntimeValue
Script_Overlay_CreateTextual(AGSEngine *vm, ScriptObject *,
                             const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;
	int width = params[2]._signedValue;
	...
	uint overlayType =
	    vm->createTextOverlayCore(x, y, width, fontId, colour,
	                              vm->getTranslation(text->getString()), false);
	uint overlayId = vm->findOverlayOfType(overlayType);

	return vm->_overlays[overlayId];
}

// Overlay: import void SetText(int width, FontType, int colour, const string
// text, ...) Changes the text on the overlay.
RuntimeValue Script_Overlay_SetText(AGSEngine *vm, Overlay *self,
                                    const Common::Array<RuntimeValue> &params) {
...
// DynamicSprite: import static DynamicSprite* Create(int width, int height,
// bool hasAlphaChannel=false) Creates a blank dynamic sprite of the specified
// size.
RuntimeValue
...
// frame=SCR_NO_VALUE, int x=SCR_NO_VALUE, int y=SCR_NO_VALUE, int
// width=SCR_NO_VALUE, int height=SCR_NO_VALUE) Creates a dynamic sprite as a
// copy of a room background.
RuntimeValue Script_DynamicSprite_CreateFromBackground(
...
// DynamicSprite: import static DynamicSprite*
// CreateFromDrawingSurface(DrawingSurface* surface, int x, int y, int width,
// int height) Creates a dynamic sprite as a copy of a drawing surface.
RuntimeValue Script_DynamicSprite_CreateFromDrawingSurface(
    AGSEngine *vm, ScriptObject *, const Common::Array<RuntimeValue> &params) {
	if (!params[0]._object->isOfType(sotDrawingSurface))
		error("DynamicSprite::CreateFromDrawingSurface got incorrect object "
		      "type (expected a DrawingSurface) for parameter 1");
...
// DynamicSprite: import static DynamicSprite* CreateFromFile(const string
// filename) Creates a dynamic sprite from a BMP or PCX file.
RuntimeValue
Script_DynamicSprite_CreateFromFile(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	ScriptString *filename = (ScriptString *) params[0]._object;
	UNUSED(filename);
...
// DynamicSprite: import static DynamicSprite* CreateFromSaveGame(int slot, int
// width, int height) Creates a dynamic sprite from a save game screenshot.
RuntimeValue Script_DynamicSprite_CreateFromSaveGame(
    AGSEngine *vm, ScriptObject *, const Common::Array<RuntimeValue> &params) {
	int slot = params[0]._signedValue;
	UNUSED(slot);
...
// DynamicSprite: import static DynamicSprite* CreateFromScreenShot(int width=0,
// int height=0) Creates a dynamic sprite as a copy of the current screen.
RuntimeValue Script_DynamicSprite_CreateFromScreenShot(
    AGSEngine *vm, ScriptObject *, const Common::Array<RuntimeValue> &params) {
	int width = params[0]._signedValue;
	UNUSED(width);
	int height = params[1]._signedValue;
...
// DynamicSprite: import void ChangeCanvasSize(int width, int height, int x, int
// y) Enlarges the size of the sprite, but does not resize the image.
RuntimeValue Script_DynamicSprite_ChangeCanvasSize(
...
// Copies the transparency mask and/or alpha channel from the specified sprite
// onto this dynamic sprite.
RuntimeValue Script_DynamicSprite_CopyTransparencyMask(
    AGSEngine *vm, DynamicSprite *self,
    const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_DynamicSprite_Crop(AGSEngine *vm, DynamicSprite *self,
                          const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
RuntimeValue
Script_DynamicSprite_Flip(AGSEngine *vm, DynamicSprite *self,
                          const Common::Array<RuntimeValue> &params) {
...
// Resizes the sprite.
RuntimeValue
Script_DynamicSprite_Resize(AGSEngine *vm, DynamicSprite *self,
                            const Common::Array<RuntimeValue> &params) {
	int width = params[0]._signedValue;
	UNUSED(width);
	int height = params[1]._signedValue;
...
// DynamicSprite: import void Rotate(int angle, int width=SCR_NO_VALUE, int
// height=SCR_NO_VALUE) Rotates the sprite by the specified number of degrees.
RuntimeValue
Script_DynamicSprite_Rotate(AGSEngine *vm, DynamicSprite *self,
                            const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_DynamicSprite_SaveToFile(AGSEngine *vm, DynamicSprite *self,
                                const Common::Array<RuntimeValue> &params) {
	ScriptString *filename = (ScriptString *) params[0]._object;
	UNUSED(filename);
...
// DynamicSprite: import void Tint(int red, int green, int blue, int saturation,
// int luminance) Permanently tints the sprite to the specified colour.
RuntimeValue
Script_DynamicSprite_Tint(AGSEngine *vm, DynamicSprite *self,
                          const Common::Array<RuntimeValue> &params) {
	int red = params[0]._signedValue;
	UNUSED(red);
	int green = params[1]._signedValue;
...
RuntimeValue
Script_DynamicSprite_get_ColorDepth(AGSEngine *vm, DynamicSprite *self,
                                    const Common::Array<RuntimeValue> &params) {
	// FIXME
...
// Gets the sprite number of this dynamic sprite, which you can use to display
// it in the game.
RuntimeValue
...
RuntimeValue
Script_DynamicSprite_get_Height(AGSEngine *vm, DynamicSprite *self,
                                const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_DynamicSprite_get_Width(AGSEngine *vm, DynamicSprite *self,
                               const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue Script_FadeIn(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_FadeOut(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
	int speed = params[0]._signedValue;
	UNUSED(speed);
...
RuntimeValue Script_CyclePalette(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_SetPalRGB(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
	int slot = params[0]._signedValue;
	UNUSED(slot);
...
RuntimeValue Script_UpdatePalette(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue Script_TintScreen(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
	int red = params[0]._signedValue;
	UNUSED(red);
	int green = params[1]._signedValue;
...
// import void SetAmbientTint(int red, int green, int blue, int saturation, int
// luminance) Sets an ambient tint that affects all objects and characters in
// the room.
RuntimeValue Script_SetAmbientTint(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	int red = params[0]._signedValue;
	UNUSED(red);
	int green = params[1]._signedValue;
...
RuntimeValue
Script_SetBackgroundFrame(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	...
	if (frame >= vm->getCurrentRoom()->_backgroundScenes.size())
		error("SetBackgroundFrame: frame %d is too high (only have %d "
		      "backgrounds)",
...
RuntimeValue
Script_GetBackgroundFrame(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_ShakeScreen(AGSEngine *vm, ScriptObject *,
                                const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_ShakeScreenBackground(AGSEngine *vm, ScriptObject *,
                             const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_SetScreenTransition(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
	uint transitionStyle = params[0]._value;

	if (transitionStyle > FADE_LAST)
		error("SetScreenTransition: invalid transition type %d",
		      transitionStyle);
...
RuntimeValue
Script_SetNextScreenTransition(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_SetFadeColor(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
	int red = params[0]._signedValue;
	UNUSED(red);
	int green = params[1]._signedValue;
...
RuntimeValue Script_SaveScreenShot(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	ScriptString *filename = (ScriptString *) params[0]._object;
	UNUSED(filename);
...
// Undocumented.
RuntimeValue Script_FlipScreen(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue Script_SetFrameSound(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
...
static const ScriptSystemFunctionInfo ourFunctionList[] = {
    {"ViewFrame::get_Flipped",
     (ScriptAPIFunction *) &Script_ViewFrame_get_Flipped, "", sotViewFrame},
    {"ViewFrame::get_Frame", (ScriptAPIFunction *) &Script_ViewFrame_get_Frame,
     "", sotViewFrame},
    {"ViewFrame::get_Graphic",
     (ScriptAPIFunction *) &Script_ViewFrame_get_Graphic, "", sotViewFrame},
    {"ViewFrame::set_Graphic",
     (ScriptAPIFunction *) &Script_ViewFrame_set_Graphic, "i", sotViewFrame},
    {"ViewFrame::get_LinkedAudio",
     (ScriptAPIFunction *) &Script_ViewFrame_get_LinkedAudio, "", sotViewFrame},
    {"ViewFrame::set_LinkedAudio",
     (ScriptAPIFunction *) &Script_ViewFrame_set_LinkedAudio, "o",
     sotViewFrame},
    {"ViewFrame::get_Loop", (ScriptAPIFunction *) &Script_ViewFrame_get_Loop,
     "", sotViewFrame},
    {"ViewFrame::get_Sound", (ScriptAPIFunction *) &Script_ViewFrame_get_Sound,
     "", sotViewFrame},
    {"ViewFrame::set_Sound", (ScriptAPIFunction *) &Script_ViewFrame_set_Sound,
     "i", sotViewFrame},
    {"ViewFrame::get_Speed", (ScriptAPIFunction *) &Script_ViewFrame_get_Speed,
     "", sotViewFrame},
    {"ViewFrame::get_View", (ScriptAPIFunction *) &Script_ViewFrame_get_View,
     "", sotViewFrame},
    {"DrawingSurface::Clear^1",
     (ScriptAPIFunction *) &Script_DrawingSurface_Clear, "i",
     sotDrawingSurface},
    {"DrawingSurface::CreateCopy^0",
     (ScriptAPIFunction *) &Script_DrawingSurface_CreateCopy, "",
     sotDrawingSurface},
    {"DrawingSurface::DrawCircle^3",
     (ScriptAPIFunction *) &Script_DrawingSurface_DrawCircle, "iii",
     sotDrawingSurface},
    {"DrawingSurface::DrawImage^6",
     (ScriptAPIFunction *) &Script_DrawingSurface_DrawImage, "iiiiii",
     sotDrawingSurface},
    {"DrawingSurface::DrawLine^5",
     (ScriptAPIFunction *) &Script_DrawingSurface_DrawLine, "iiiii",
     sotDrawingSurface},
    {"DrawingSurface::DrawMessageWrapped^5",
     (ScriptAPIFunction *) &Script_DrawingSurface_DrawMessageWrapped, "iiiii",
     sotDrawingSurface},
    {"DrawingSurface::DrawPixel^2",
     (ScriptAPIFunction *) &Script_DrawingSurface_DrawPixel, "ii",
     sotDrawingSurface},
    {"DrawingSurface::DrawRectangle^4",
    ...
    {"DrawingSurface::DrawStringWrapped^6",
     ...
     sotDrawingSurface},
    {"DrawingSurface::DrawSurface^2",
     (ScriptAPIFunction *) &Script_DrawingSurface_DrawSurface, "oi",
     sotDrawingSurface},
    {"DrawingSurface::DrawTriangle^6",
     (ScriptAPIFunction *) &Script_DrawingSurface_DrawTriangle, "iiiiii",
     sotDrawingSurface},
    {"DrawingSurface::GetPixel^2",
     (ScriptAPIFunction *) &Script_DrawingSurface_GetPixel, "ii",
     sotDrawingSurface},
    {"DrawingSurface::Release^0",
     (ScriptAPIFunction *) &Script_DrawingSurface_Release, "",
     sotDrawingSurface},
    {"DrawingSurface::get_DrawingColor",
     (ScriptAPIFunction *) &Script_DrawingSurface_get_DrawingColor, "",
     sotDrawingSurface},
    {"DrawingSurface::set_DrawingColor",
     (ScriptAPIFunction *) &Script_DrawingSurface_set_DrawingColor, "i",
     sotDrawingSurface},
    {"DrawingSurface::get_Height",
     (ScriptAPIFunction *) &Script_DrawingSurface_get_Height, "",
     sotDrawingSurface},
    {"DrawingSurface::get_UseHighResCoordinates",
     (ScriptAPIFunction *) &Script_DrawingSurface_get_UseHighResCoordinates, "",
     sotDrawingSurface},
    {"DrawingSurface::set_UseHighResCoordinates",
     (ScriptAPIFunction *) &Script_DrawingSurface_set_UseHighResCoordinates,
     "i", sotDrawingSurface},
    {"DrawingSurface::get_Width",
    ...
    {"ReleaseViewport", (ScriptAPIFunction *) &Script_ReleaseViewport, "",
     sotNone},
    ...
    {"IsOverlayValid", (ScriptAPIFunction *) &Script_IsOverlayValid, "i",
     sotNone},
    {"LoadImageFile", (ScriptAPIFunction *) &Script_LoadImageFile, "s",
     sotNone},
    {"DeleteSprite", (ScriptAPIFunction *) &Script_DeleteSprite, "i", sotNone},
    {"SetSpeechFont", (ScriptAPIFunction *) &Script_SetSpeechFont, "i",
     sotNone},
    {"SetNormalFont", (ScriptAPIFunction *) &Script_SetNormalFont, "i",
     sotNone},
    {"RawClearScreen", (ScriptAPIFunction *) &Script_RawClearScreen, "i",
     sotNone},
    {"RawDrawCircle", (ScriptAPIFunction *) &Script_RawDrawCircle, "iii",
     sotNone},
    {"RawDrawImage", (ScriptAPIFunction *) &Script_RawDrawImage, "iii",
     sotNone},
    {"RawDrawImageOffset", (ScriptAPIFunction *) &Script_RawDrawImageOffset,
     "iii", sotNone},
    {"RawDrawImageResized", (ScriptAPIFunction *) &Script_RawDrawImageResized,
     "iiiii", sotNone},
    {"RawDrawImageTransparent",
     (ScriptAPIFunction *) &Script_RawDrawImageTransparent, "iiii", sotNone},
    {"RawDrawLine", (ScriptAPIFunction *) &Script_RawDrawLine, "iiii", sotNone},
    {"RawDrawRectangle", (ScriptAPIFunction *) &Script_RawDrawRectangle, "iiii",
     sotNone},
    ...
    {"RawSetColorRGB", (ScriptAPIFunction *) &Script_RawSetColorRGB, "iii",
     sotNone},
    {"RawDrawFrameTransparent",
     (ScriptAPIFunction *) &Script_RawDrawFrameTransparent, "ii", sotNone},
    {"RawSaveScreen", (ScriptAPIFunction *) &Script_RawSaveScreen, "", sotNone},
    {"RawRestoreScreen", (ScriptAPIFunction *) &Script_RawRestoreScreen, "",
     sotNone},
    {"RawRestoreScreenTinted",
     (ScriptAPIFunction *) &Script_RawRestoreScreenTinted, "iiii", sotNone},
    {"GetTextWidth", (ScriptAPIFunction *) &Script_GetTextWidth, "si", sotNone},
    {"GetTextHeight", (ScriptAPIFunction *) &Script_GetTextHeight, "sii",
     sotNone},
    {"Overlay::CreateGraphical^4",
     (ScriptAPIFunction *) &Script_Overlay_CreateGraphical, "iiii", sotNone},
    {"Overlay::CreateTextual^106",
     (ScriptAPIFunction *) &Script_Overlay_CreateTextual, "iiiiis", sotNone},
    {"Overlay::SetText^104", (ScriptAPIFunction *) &Script_Overlay_SetText,
     "iiis", sotOverlay},
    {"Overlay::Remove^0", (ScriptAPIFunction *) &Script_Overlay_Remove, "",
     sotOverlay},
    {"Overlay::get_Valid", (ScriptAPIFunction *) &Script_Overlay_get_Valid, "",
     sotOverlay},
    {"Overlay::get_X", (ScriptAPIFunction *) &Script_Overlay_get_X, "",
     sotOverlay},
    {"Overlay::set_X", (ScriptAPIFunction *) &Script_Overlay_set_X, "i",
     sotOverlay},
    {"Overlay::get_Y", (ScriptAPIFunction *) &Script_Overlay_get_Y, "",
     sotOverlay},
    {"Overlay::set_Y", (ScriptAPIFunction *) &Script_Overlay_set_Y, "i",
     sotOverlay},
    {"DynamicSprite::Create^3",
     (ScriptAPIFunction *) &Script_DynamicSprite_Create, "iii", sotNone},
    {"DynamicSprite::CreateFromBackground^5",
     (ScriptAPIFunction *) &Script_DynamicSprite_CreateFromBackground, "iiiii",
     sotNone},
    ...
    {"DynamicSprite::CreateFromExistingSprite^2",
     ...
     sotNone},
    {"DynamicSprite::CreateFromFile^1",
     (ScriptAPIFunction *) &Script_DynamicSprite_CreateFromFile, "s", sotNone},
    {"DynamicSprite::CreateFromSaveGame^3",
     (ScriptAPIFunction *) &Script_DynamicSprite_CreateFromSaveGame, "iii",
     sotNone},
    {"DynamicSprite::CreateFromScreenShot^2",
     (ScriptAPIFunction *) &Script_DynamicSprite_CreateFromScreenShot, "ii",
     sotNone},
    {"DynamicSprite::ChangeCanvasSize^4",
     (ScriptAPIFunction *) &Script_DynamicSprite_ChangeCanvasSize, "iiii",
     sotDynamicSprite},
    {"DynamicSprite::CopyTransparencyMask^1",
     (ScriptAPIFunction *) &Script_DynamicSprite_CopyTransparencyMask, "i",
     sotDynamicSprite},
    {"DynamicSprite::Crop^4", (ScriptAPIFunction *) &Script_DynamicSprite_Crop,
     "iiii", sotDynamicSprite},
    {"DynamicSprite::Delete^0",
     (ScriptAPIFunction *) &Script_DynamicSprite_Delete, "", sotDynamicSprite},
    {"DynamicSprite::Flip^1", (ScriptAPIFunction *) &Script_DynamicSprite_Flip,
     "i", sotDynamicSprite},
    {"DynamicSprite::GetDrawingSurface^0",
     (ScriptAPIFunction *) &Script_DynamicSprite_GetDrawingSurface, "",
     sotDynamicSprite},
    {"DynamicSprite::Resize^2",
     (ScriptAPIFunction *) &Script_DynamicSprite_Resize, "ii",
     sotDynamicSprite},
    {"DynamicSprite::Rotate^3",
     (ScriptAPIFunction *) &Script_DynamicSprite_Rotate, "iii",
     sotDynamicSprite},
    {"DynamicSprite::SaveToFile^1",
     (ScriptAPIFunction *) &Script_DynamicSprite_SaveToFile, "s",
     sotDynamicSprite},
    {"DynamicSprite::Tint^5", (ScriptAPIFunction *) &Script_DynamicSprite_Tint,
     "iiiii", sotDynamicSprite},
    {"DynamicSprite::get_ColorDepth",
     (ScriptAPIFunction *) &Script_DynamicSprite_get_ColorDepth, "",
     sotDynamicSprite},
    {"DynamicSprite::get_Graphic",
     (ScriptAPIFunction *) &Script_DynamicSprite_get_Graphic, "",
     sotDynamicSprite},
    {"DynamicSprite::get_Height",
     (ScriptAPIFunction *) &Script_DynamicSprite_get_Height, "",
     sotDynamicSprite},
    {"DynamicSprite::get_Width",
     (ScriptAPIFunction *) &Script_DynamicSprite_get_Width, "",
     sotDynamicSprite},
    ...
    {"SetAmbientTint", (ScriptAPIFunction *) &Script_SetAmbientTint, "iiiii",
     sotNone},
    {"SetBackgroundFrame", (ScriptAPIFunction *) &Script_SetBackgroundFrame,
     "i", sotNone},
    {"GetBackgroundFrame", (ScriptAPIFunction *) &Script_GetBackgroundFrame, "",
     sotNone},
    {"ShakeScreen", (ScriptAPIFunction *) &Script_ShakeScreen, "i", sotNone},
    {"ShakeScreenBackground",
     (ScriptAPIFunction *) &Script_ShakeScreenBackground, "iii", sotNone},
    {"SetScreenTransition", (ScriptAPIFunction *) &Script_SetScreenTransition,
     "i", sotNone},
    {"SetNextScreenTransition",
     (ScriptAPIFunction *) &Script_SetNextScreenTransition, "i", sotNone},
    {"SetFadeColor", (ScriptAPIFunction *) &Script_SetFadeColor, "iii",
     sotNone},
    {"SaveScreenShot", (ScriptAPIFunction *) &Script_SaveScreenShot, "s",
     sotNone},
    {"FlipScreen", (ScriptAPIFunction *) &Script_FlipScreen, "i", sotNone},
    {"SetFrameSound", (ScriptAPIFunction *) &Script_SetFrameSound, "iiii",
     sotNone},
};
