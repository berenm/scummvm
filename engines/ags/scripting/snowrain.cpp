...
public:
	...
	void update(bool withDrift = false) {
		...
		for (uint i = 0; i < _amount * 2; ++i) {
			...
			p.y += p.speed;
			float drift = p.drift * sin((float) (p.y + p.driftOffset) *
			                            p.driftSpeed * 2.0f * M_PI / 360.0f);
			...
			if (p.y > p.maxY) {
				...
				p.speed = (rnd->getRandomNumber(_deltaFallSpeed - 1) +
				           _minFallSpeed) /
				          50.0f;
				p.maxY =
				    rnd->getRandomNumber(_deltaBaseline - 1) + _topBaseline;
				if (withDrift) {
					p.drift = rnd->getRandomNumber(_deltaDrift - 1) + _minDrift;
					p.driftSpeed = (rnd->getRandomNumber(_deltaDriftSpeed - 1) +
					                _minDriftSpeed) /
					               50.0f;
				}
				...
				_vm->_graphics->internalDraw(_views[p.type].surface,
				                             Common::Point(p.x, p.y), p.alpha);
			}
		}
	}
	...
	bool isActive() { return (_amount > 0 || _targetAmount != _amount); }

	void enterRoom() { _amount = _targetAmount; }
	...
	void initParticles() {
		...
		for (uint i = 0; i < _particles.size(); ++i) {
			...
			p.alpha = rnd->getRandomNumber(_deltaAlpha - 1) + _minAlpha;
			p.speed =
			    (rnd->getRandomNumber(_deltaFallSpeed - 1) + _minFallSpeed) /
			    50.0f;
			p.maxY = rnd->getRandomNumber(_deltaBaseline - 1) + _topBaseline;
			p.drift = rnd->getRandomNumber(_deltaDrift - 1) + _minDrift;
			p.driftSpeed =
			    (rnd->getRandomNumber(_deltaDriftSpeed - 1) + _minDriftSpeed) /
			    50.0f;
	...
	void setTransparency(uint min, uint max) {
		...
		for (uint i = 0; i < _particles.size(); ++i)
			_particles[i].alpha =
			    rnd->getRandomNumber(_deltaAlpha - 1) + _minAlpha;
	}
...
RuntimeValue
Script_srSetSnowDriftRange(AGSEngine *vm, ScriptObject *,
...
RuntimeValue
Script_srSetSnowDriftSpeed(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
	uint minValue = params[0]._value;
	uint maxValue = params[1]._value;
...
RuntimeValue
Script_srSetSnowFallSpeed(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	uint minValue = params[0]._value;
	uint maxValue = params[1]._value;
...
// Undocumented.
RuntimeValue
Script_srSetRainFallSpeed(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	uint minValue = params[0]._value;
	uint maxValue = params[1]._value;
...
RuntimeValue
Script_srChangeSnowAmount(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue
Script_srChangeRainAmount(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	uint amount = params[0]._value;
...
RuntimeValue Script_srSetSnowAmount(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	uint amount = params[0]._value;
...
// Undocumented.
RuntimeValue Script_srSetRainAmount(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	uint amount = params[0]._value;
...
RuntimeValue
Script_srSetSnowBaseline(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue
Script_srSetRainBaseline(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	uint top = params[0]._value;
	uint bottom = params[1]._value;
...
RuntimeValue Script_srSetBaseline(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	uint top = params[0]._value;
	uint bottom = params[1]._value;
...
RuntimeValue
Script_srSetSnowTransparency(AGSEngine *vm, ScriptObject *,
                             const Common::Array<RuntimeValue> &params) {
	uint minValue = params[0]._value;
	uint maxValue = params[1]._value;
...
// Undocumented.
RuntimeValue
Script_srSetRainTransparency(AGSEngine *vm, ScriptObject *,
                             const Common::Array<RuntimeValue> &params) {
	uint minValue = params[0]._value;
	uint maxValue = params[1]._value;
...
// Set the default snow view/loop, used for 'normal' flakes which haven't had
// srSetSnowView called.
RuntimeValue
Script_srSetSnowDefaultView(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue
Script_srSetRainDefaultView(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
	uint view = params[0]._value;
	uint loop = params[1]._value;
...
RuntimeValue Script_srSetSnowView(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue Script_srSetRainView(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	uint kindId = params[0]._signedValue;
	uint event = params[1]._signedValue;
	UNUSED(event);
...
RuntimeValue
Script_srSetSnowWindSpeed(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue
Script_srSetRainWindSpeed(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	int value = params[0]._signedValue;
...
RuntimeValue Script_srSetWindSpeed(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	int value = params[0]._signedValue;
...
static const ScriptSystemFunctionInfo ourFunctionList[] = {
    {"srSetSnowDriftRange", (ScriptAPIFunction *) &Script_srSetSnowDriftRange,
     "ii", sotNone},
    {"srSetSnowDriftSpeed", (ScriptAPIFunction *) &Script_srSetSnowDriftSpeed,
     "ii", sotNone},
    {"srSetSnowFallSpeed", (ScriptAPIFunction *) &Script_srSetSnowFallSpeed,
     "ii", sotNone},
    {"srSetRainFallSpeed", (ScriptAPIFunction *) &Script_srSetRainFallSpeed,
     "ii", sotNone},
    {"srChangeSnowAmount", (ScriptAPIFunction *) &Script_srChangeSnowAmount,
     "i", sotNone},
    {"srChangeRainAmount", (ScriptAPIFunction *) &Script_srChangeRainAmount,
     "i", sotNone},
    {"srSetSnowAmount", (ScriptAPIFunction *) &Script_srSetSnowAmount, "i",
     sotNone},
    {"srSetRainAmount", (ScriptAPIFunction *) &Script_srSetRainAmount, "i",
     sotNone},
    {"srSetSnowBaseline", (ScriptAPIFunction *) &Script_srSetSnowBaseline, "ii",
     sotNone},
    {"srSetRainBaseline", (ScriptAPIFunction *) &Script_srSetRainBaseline, "ii",
     sotNone},
    {"srSetBaseline", (ScriptAPIFunction *) &Script_srSetBaseline, "ii",
     sotNone},
    {"srSetSnowTransparency",
     (ScriptAPIFunction *) &Script_srSetSnowTransparency, "ii", sotNone},
    {"srSetRainTransparency",
     (ScriptAPIFunction *) &Script_srSetRainTransparency, "ii", sotNone},
    {"srSetSnowDefaultView", (ScriptAPIFunction *) &Script_srSetSnowDefaultView,
     "ii", sotNone},
    {"srSetRainDefaultView", (ScriptAPIFunction *) &Script_srSetRainDefaultView,
     "ii", sotNone},
    {"srSetSnowView", (ScriptAPIFunction *) &Script_srSetSnowView, "iiii",
     sotNone},
    {"srSetRainView", (ScriptAPIFunction *) &Script_srSetRainView, "iiii",
     sotNone},
    {"srSetSnowWindSpeed", (ScriptAPIFunction *) &Script_srSetSnowWindSpeed,
     "i", sotNone},
    {"srSetRainWindSpeed", (ScriptAPIFunction *) &Script_srSetRainWindSpeed,
     "i", sotNone},
    {"srSetWindSpeed", (ScriptAPIFunction *) &Script_srSetWindSpeed, "i",
     sotNone},
};
