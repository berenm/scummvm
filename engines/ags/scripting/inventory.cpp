...
// Undocumented.
RuntimeValue Script_GetInvAt(AGSEngine *vm, ScriptObject *,
...
RuntimeValue Script_GetInvProperty(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	uint invItem = params[0]._value;
	ScriptString *property = (ScriptString *) params[1]._object;

	if (invItem >= vm->_gameFile->_invItemInfo.size())
		error("GetInvProperty: item %d is too high (only have %d)", invItem,
		      vm->_gameFile->_invItemInfo.size());

	return vm->getIntProperty(property->getString(),
	                          vm->_gameFile->_invItemInfo[invItem]._properties);
}

// import void GetInvPropertyText(int invItem, const string property, string
// buffer) Obsolete inventory function.
RuntimeValue
Script_GetInvPropertyText(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	uint invItem = params[0]._value;
	ScriptString *property = (ScriptString *) params[1]._object;
	...
	if (invItem >= vm->_gameFile->_invItemInfo.size())
		error("GetInvPropertyText: item %d is too high (only have %d)", invItem,
		      vm->_gameFile->_invItemInfo.size());
...
// Obsolete inventory function.
RuntimeValue Script_GetInvName(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
	uint item = params[0]._value;
	ScriptString *buffer = (ScriptString *) params[1]._object;

	if (item >= vm->_gameFile->_invItemInfo.size())
		error("GetInvName: item %d is too high (only have %d)", item,
		      vm->_gameFile->_invItemInfo.size());

	buffer->setString(
	    vm->getTranslation(vm->_gameFile->_invItemInfo[item]._name));
...
// Obsolete inventory function.
RuntimeValue Script_GetInvGraphic(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	uint item = params[0]._value;
	UNUSED(item);

	if (item >= vm->_gameFile->_invItemInfo.size())
		error("GetInvGraphic: item %d is too high (only have %d)", item,
		      vm->_gameFile->_invItemInfo.size());
...
// Obsolete inventory function.
RuntimeValue Script_SetInvItemPic(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	uint item = params[0]._value;
	uint spriteSlot = params[1]._value;

	if (item >= vm->_gameFile->_invItemInfo.size())
		error("SetInvItemPic: item %d is too high (only have %d)", item,
		      vm->_gameFile->_invItemInfo.size());
	...
	if (invItem._pic == invItem._cursorPic) {
		...
		if (vm->_graphics->getCurrentCursor() == MODE_USE &&
		    vm->getPlayerChar()->_activeInv == item) {
...
// Obsolete inventory function.
RuntimeValue Script_SetInvItemName(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	uint item = params[0]._value;
	ScriptString *name = (ScriptString *) params[1]._object;

	if (item >= vm->_gameFile->_invItemInfo.size())
		error("SetInvItemName: item %d is too high (only have %d)", item,
		      vm->_gameFile->_invItemInfo.size());
...
// Obsolete inventory function.
RuntimeValue Script_IsInventoryInteractionAvailable(
    AGSEngine *vm, ScriptObject *, const Common::Array<RuntimeValue> &params) {
	uint item = params[0]._value;
	uint32 cursormode = params[1]._value;

	if (item >= vm->_gameFile->_invItemInfo.size())
		error("IsInventoryInteractionAvailable: item %d is too high (only have "
		      "%d)",
		      item, vm->_gameFile->_invItemInfo.size());
...
// Obsolete inventory function.
RuntimeValue
Script_RunInventoryInteraction(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
	uint item = params[0]._value;
	uint32 cursormode = params[1]._value;

	if (item >= vm->_gameFile->_invItemInfo.size())
		error("RunInventoryInteraction: item %d is too high (only have %d)",
		      item, vm->_gameFile->_invItemInfo.size());
...
RuntimeValue Script_UpdateInventory(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	vm->updateInvOrder();
...
RuntimeValue
Script_InventoryItem_GetAtScreenXY(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;
...
RuntimeValue
Script_InventoryItem_GetProperty(AGSEngine *vm, InventoryItem *self,
                                 const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_InventoryItem_GetTextProperty(
    AGSEngine *vm, InventoryItem *self,
    const Common::Array<RuntimeValue> &params) {
	ScriptString *property = (ScriptString *) params[0]._object;
...
// Checks whether an event handler has been registered for clicking on this item
// in the specified cursor mode.
RuntimeValue Script_InventoryItem_IsInteractionAvailable(
    AGSEngine *vm, InventoryItem *self,
    const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_InventoryItem_RunInteraction(AGSEngine *vm, InventoryItem *self,
                                    const Common::Array<RuntimeValue> &params) {
	uint32 cursormode = params[0]._value;
...
RuntimeValue Script_InventoryItem_get_CursorGraphic(
    AGSEngine *vm, InventoryItem *self,
    const Common::Array<RuntimeValue> &params) {
...
// InventoryItem: import attribute int CursorGraphic
// Gets/sets the sprite used as the item's mouse cursor.
RuntimeValue Script_InventoryItem_set_CursorGraphic(
    AGSEngine *vm, InventoryItem *self,
    const Common::Array<RuntimeValue> &params) {
	...
	if (vm->_graphics->getCurrentCursor() == MODE_USE &&
	    vm->getPlayerChar()->_activeInv == self->_id) {
...
RuntimeValue
Script_InventoryItem_get_Graphic(AGSEngine *vm, InventoryItem *self,
                                 const Common::Array<RuntimeValue> &params) {
	return self->_pic;
}
...
// Gets/sets the sprite used to display the inventory item.
RuntimeValue
Script_InventoryItem_set_Graphic(AGSEngine *vm, InventoryItem *self,
                                 const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_InventoryItem_get_ID(AGSEngine *vm, InventoryItem *self,
                            const Common::Array<RuntimeValue> &params) {
	return self->_id;
}
...
RuntimeValue
Script_InventoryItem_get_Name(AGSEngine *vm, InventoryItem *self,
                              const Common::Array<RuntimeValue> &params) {
...
// Gets/sets the name of the inventory item.
RuntimeValue
Script_InventoryItem_set_Name(AGSEngine *vm, InventoryItem *self,
                              const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue
Script_InventoryItem_GetName(AGSEngine *vm, InventoryItem *self,
                             const Common::Array<RuntimeValue> &params) {
...
// InventoryItem: import void GetPropertyText(const string property, string
// buffer) Undocumented.
RuntimeValue Script_InventoryItem_GetPropertyText(
    AGSEngine *vm, InventoryItem *self,
    const Common::Array<RuntimeValue> &params) {
	ScriptString *property = (ScriptString *) params[0]._object;
	ScriptString *buffer = (ScriptString *) params[1]._object;
...
// Undocumented.
RuntimeValue
Script_InventoryItem_SetName(AGSEngine *vm, InventoryItem *self,
                             const Common::Array<RuntimeValue> &params) {
...
static const ScriptSystemFunctionInfo ourFunctionList[] = {
    ...
    {"GetInvProperty", (ScriptAPIFunction *) &Script_GetInvProperty, "is",
     sotNone},
    {"GetInvPropertyText", (ScriptAPIFunction *) &Script_GetInvPropertyText,
     "iss", sotNone},
    {"GetInvName", (ScriptAPIFunction *) &Script_GetInvName, "is", sotNone},
    {"GetInvGraphic", (ScriptAPIFunction *) &Script_GetInvGraphic, "i",
     sotNone},
    {"SetInvItemPic", (ScriptAPIFunction *) &Script_SetInvItemPic, "ii",
     sotNone},
    {"SetInvItemName", (ScriptAPIFunction *) &Script_SetInvItemName, "is",
     sotNone},
    {"IsInventoryInteractionAvailable",
     (ScriptAPIFunction *) &Script_IsInventoryInteractionAvailable, "ii",
     sotNone},
    {"RunInventoryInteraction",
     (ScriptAPIFunction *) &Script_RunInventoryInteraction, "ii", sotNone},
    {"UpdateInventory", (ScriptAPIFunction *) &Script_UpdateInventory, "",
     sotNone},
    {"InventoryItem::GetAtScreenXY^2",
     (ScriptAPIFunction *) &Script_InventoryItem_GetAtScreenXY, "ii", sotNone},
    {"InventoryItem::GetProperty^1",
     (ScriptAPIFunction *) &Script_InventoryItem_GetProperty, "s",
     sotInventoryItem},
    {"InventoryItem::GetTextProperty^1",
     (ScriptAPIFunction *) &Script_InventoryItem_GetTextProperty, "s",
     sotInventoryItem},
    {"InventoryItem::IsInteractionAvailable^1",
     (ScriptAPIFunction *) &Script_InventoryItem_IsInteractionAvailable, "i",
     sotInventoryItem},
    {"InventoryItem::RunInteraction^1",
     (ScriptAPIFunction *) &Script_InventoryItem_RunInteraction, "i",
     sotInventoryItem},
    {"InventoryItem::get_CursorGraphic",
     (ScriptAPIFunction *) &Script_InventoryItem_get_CursorGraphic, "",
     sotInventoryItem},
    {"InventoryItem::set_CursorGraphic",
     (ScriptAPIFunction *) &Script_InventoryItem_set_CursorGraphic, "i",
     sotInventoryItem},
    {"InventoryItem::get_Graphic",
     (ScriptAPIFunction *) &Script_InventoryItem_get_Graphic, "",
     sotInventoryItem},
    {"InventoryItem::set_Graphic",
     (ScriptAPIFunction *) &Script_InventoryItem_set_Graphic, "i",
     sotInventoryItem},
    {"InventoryItem::get_ID",
     (ScriptAPIFunction *) &Script_InventoryItem_get_ID, "", sotInventoryItem},
    {"InventoryItem::get_Name",
     (ScriptAPIFunction *) &Script_InventoryItem_get_Name, "",
     sotInventoryItem},
    {"InventoryItem::set_Name",
     (ScriptAPIFunction *) &Script_InventoryItem_set_Name, "s",
     sotInventoryItem},
    {"InventoryItem::GetName^1",
     (ScriptAPIFunction *) &Script_InventoryItem_GetName, "s",
     sotInventoryItem},
    {"InventoryItem::GetPropertyText^2",
     (ScriptAPIFunction *) &Script_InventoryItem_GetPropertyText, "ss",
     sotInventoryItem},
    {"InventoryItem::SetName^1",
     (ScriptAPIFunction *) &Script_InventoryItem_SetName, "s",
     sotInventoryItem},
};
