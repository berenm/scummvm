...
RuntimeValue Script_CDAudio(AGSEngine *vm, ScriptObject *,
...
RuntimeValue Script_PlayFlic(AGSEngine *vm, ScriptObject *,
                             const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_PlayVideo(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
...
// Plays a MIDI, but mutes the channel. This allows you to use it to time game
// events.
RuntimeValue Script_PlaySilentMIDI(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	int musicNumber = params[0]._signedValue;
	UNUSED(musicNumber);
...
// Plays a specified MP3 or OGG file, that is not part of the normal game
// package.
RuntimeValue Script_PlayMP3File(AGSEngine *vm, ScriptObject *,
                                const Common::Array<RuntimeValue> &params) {
	ScriptString *filename = (ScriptString *) params[0]._object;
	UNUSED(filename);
...
RuntimeValue Script_PlaySound(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
...
// y) Starts an ambient looping sound playing.
RuntimeValue
Script_PlayAmbientSound(AGSEngine *vm, ScriptObject *,
                        const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_StopAmbientSound(AGSEngine *vm, ScriptObject *,
                        const Common::Array<RuntimeValue> &params) {
	uint channel = params[0]._value;
...
RuntimeValue Script_GetCurrentMusic(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_SetSoundVolume(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_SetMusicMasterVolume(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
	uint volume = params[0]._value;

	if (volume > 100)
		error("SetMusicMasterVolume: volume %d is too high (must be 0-100)",
		      volume);
...
RuntimeValue Script_SeekMODPattern(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_GetMIDIPosition(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_SeekMIDIPosition(AGSEngine *vm, ScriptObject *,
                        const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_SetSpeechVolume(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	uint volume = params[0]._value;
...
RuntimeValue
Script_IsMusicVoxAvailable(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
...
// Sets the audio to have its location at (x,y); it will get quieter the further
// away the player is.
RuntimeValue
Script_AudioChannel_SetRoomLocation(AGSEngine *vm, AudioChannel *self,
                                    const Common::Array<RuntimeValue> &params) {
...
// The audio clip that is currently being played on this channel, or null if
// none.
RuntimeValue
...
// The current offset into the sound. What this represents depends on the audio
// type.
RuntimeValue
...
// AudioClip: import AudioChannel* Play(AudioPriority=SCR_NO_VALUE,
...
RuntimeValue Script_AudioClip_Play(AGSEngine *vm, AudioClip *self,
	...
	uint channelId =
	    vm->_audio->playAudioClip(*self, audioPriority, repeatStyle);
	if (channelId == (uint) -1)
		return 0;
	else
...
// AudioPriority=SCR_NO_VALUE, RepeatStyle=SCR_NO_VALUE) Plays this audio clip,
...
Script_AudioClip_PlayFrom(AGSEngine *vm, AudioClip *self,
	...
	uint channelId =
	    vm->_audio->playAudioClip(*self, audioPriority, repeatStyle, position);
	if (channelId == (uint) -1)
		return 0;
	else
		return vm->_audio->_channels[channelId];
...
// RepeatStyle=SCR_NO_VALUE) Plays this audio clip, or queues it if all channels
// are busy.
RuntimeValue
Script_AudioClip_PlayQueued(AGSEngine *vm, AudioClip *self,
	...
	uint channelId =
	    vm->_audio->playAudioClip(*self, audioPriority, repeatStyle, 0, true);
	if (channelId == (uint) -1)
		return 0;
	else
		return vm->_audio->_channels[channelId];
...
RuntimeValue
Script_System_geti_AudioChannels(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
	uint index = params[0]._value;

	if (index >= vm->_audio->_channels.size() - 1)
		error("System::geti_AudioChannels: channel %d too high (only %d "
		      "channels)",
		      index, vm->_audio->_channels.size());
...
RuntimeValue
Script_System_get_AudioChannelCount(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
...
static const ScriptSystemFunctionInfo ourFunctionList[] = {
    ...
    {"PlayMusicQueued", (ScriptAPIFunction *) &Script_PlayMusicQueued, "i",
     sotNone},
    {"PlaySilentMIDI", (ScriptAPIFunction *) &Script_PlaySilentMIDI, "i",
     sotNone},
    ...
    {"PlayAmbientSound", (ScriptAPIFunction *) &Script_PlayAmbientSound,
     "iiiii", sotNone},
    {"StopAmbientSound", (ScriptAPIFunction *) &Script_StopAmbientSound, "i",
     sotNone},
    {"GetCurrentMusic", (ScriptAPIFunction *) &Script_GetCurrentMusic, "",
     sotNone},
    {"SetMusicRepeat", (ScriptAPIFunction *) &Script_SetMusicRepeat, "i",
     sotNone},
    {"SetMusicVolume", (ScriptAPIFunction *) &Script_SetMusicVolume, "i",
     sotNone},
    {"SetSoundVolume", (ScriptAPIFunction *) &Script_SetSoundVolume, "i",
     sotNone},
    {"SetMusicMasterVolume", (ScriptAPIFunction *) &Script_SetMusicMasterVolume,
     "i", sotNone},
    {"SetDigitalMasterVolume",
     (ScriptAPIFunction *) &Script_SetDigitalMasterVolume, "i", sotNone},
    {"SeekMODPattern", (ScriptAPIFunction *) &Script_SeekMODPattern, "i",
     sotNone},
    {"IsChannelPlaying", (ScriptAPIFunction *) &Script_IsChannelPlaying, "i",
     sotNone},
    {"IsSoundPlaying", (ScriptAPIFunction *) &Script_IsSoundPlaying, "",
     sotNone},
    {"IsMusicPlaying", (ScriptAPIFunction *) &Script_IsMusicPlaying, "",
     sotNone},
    {"GetMIDIPosition", (ScriptAPIFunction *) &Script_GetMIDIPosition, "",
     sotNone},
    {"SeekMIDIPosition", (ScriptAPIFunction *) &Script_SeekMIDIPosition, "i",
     sotNone},
    {"GetMP3PosMillis", (ScriptAPIFunction *) &Script_GetMP3PosMillis, "",
     sotNone},
    {"SeekMP3PosMillis", (ScriptAPIFunction *) &Script_SeekMP3PosMillis, "i",
     sotNone},
    {"SetChannelVolume", (ScriptAPIFunction *) &Script_SetChannelVolume, "ii",
     sotNone},
    {"StopChannel", (ScriptAPIFunction *) &Script_StopChannel, "i", sotNone},
    {"StopMusic", (ScriptAPIFunction *) &Script_StopMusic, "", sotNone},
    {"IsVoxAvailable", (ScriptAPIFunction *) &Script_IsVoxAvailable, "",
     sotNone},
    {"SetSpeechVolume", (ScriptAPIFunction *) &Script_SetSpeechVolume, "i",
     sotNone},
    {"IsMusicVoxAvailable", (ScriptAPIFunction *) &Script_IsMusicVoxAvailable,
     "", sotNone},
    {"AudioChannel::Seek^1", (ScriptAPIFunction *) &Script_AudioChannel_Seek,
     "i", sotAudioChannel},
    {"AudioChannel::SetRoomLocation^2",
     (ScriptAPIFunction *) &Script_AudioChannel_SetRoomLocation, "ii",
     sotAudioChannel},
    {"AudioChannel::Stop^0", (ScriptAPIFunction *) &Script_AudioChannel_Stop,
     "", sotAudioChannel},
    {"AudioChannel::get_ID", (ScriptAPIFunction *) &Script_AudioChannel_get_ID,
     "", sotAudioChannel},
    {"AudioChannel::get_IsPlaying",
     (ScriptAPIFunction *) &Script_AudioChannel_get_IsPlaying, "",
     sotAudioChannel},
    {"AudioChannel::get_LengthMs",
     (ScriptAPIFunction *) &Script_AudioChannel_get_LengthMs, "",
     sotAudioChannel},
    {"AudioChannel::get_Panning",
     (ScriptAPIFunction *) &Script_AudioChannel_get_Panning, "",
     sotAudioChannel},
    {"AudioChannel::set_Panning",
     (ScriptAPIFunction *) &Script_AudioChannel_set_Panning, "i",
     sotAudioChannel},
    {"AudioChannel::get_PlayingClip",
     (ScriptAPIFunction *) &Script_AudioChannel_get_PlayingClip, "",
     sotAudioChannel},
    {"AudioChannel::get_Position",
     (ScriptAPIFunction *) &Script_AudioChannel_get_Position, "",
     sotAudioChannel},
    {"AudioChannel::get_PositionMs",
     (ScriptAPIFunction *) &Script_AudioChannel_get_PositionMs, "",
     sotAudioChannel},
    {"AudioChannel::get_Volume",
     (ScriptAPIFunction *) &Script_AudioChannel_get_Volume, "",
     sotAudioChannel},
    {"AudioChannel::set_Volume",
     (ScriptAPIFunction *) &Script_AudioChannel_set_Volume, "i",
     sotAudioChannel},
    {"AudioClip::Play^2", (ScriptAPIFunction *) &Script_AudioClip_Play, "ii",
     sotAudioClip},
    {"AudioClip::PlayFrom^3", (ScriptAPIFunction *) &Script_AudioClip_PlayFrom,
     "iii", sotAudioClip},
    {"AudioClip::PlayQueued^2",
     (ScriptAPIFunction *) &Script_AudioClip_PlayQueued, "ii", sotAudioClip},
    {"AudioClip::Stop^0", (ScriptAPIFunction *) &Script_AudioClip_Stop, "",
     sotAudioClip},
    {"AudioClip::get_FileType",
     (ScriptAPIFunction *) &Script_AudioClip_get_FileType, "", sotAudioClip},
    {"AudioClip::get_IsAvailable",
     (ScriptAPIFunction *) &Script_AudioClip_get_IsAvailable, "", sotAudioClip},
    {"AudioClip::get_Type", (ScriptAPIFunction *) &Script_AudioClip_get_Type,
     "", sotAudioClip},
    {"System::geti_AudioChannels",
     (ScriptAPIFunction *) &Script_System_geti_AudioChannels, "i", sotNone},
    {"System::get_AudioChannelCount",
     (ScriptAPIFunction *) &Script_System_get_AudioChannelCount, "", sotNone},
};
