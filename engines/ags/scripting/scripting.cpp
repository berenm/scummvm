...
void GlobalScriptState::addImport(const Common::String &name,
                                  ...
                                  bool forceReplace) {
	// Original ignores attempts by scripts to import symbols with empty
	// strings, so there's no point adding any such symbols to the global list.
	if (name.empty())
		return;
		...
		if (forceReplace) {
			...
			if (oldImport._type == sitSystemObject) {
				...
				if (oldImport._object->getRefCount() != 1)
					error("replacing referenced (%d) import '%s'",
					      oldImport._object->getRefCount(), name.c_str());
...
void GlobalScriptState::addSystemFunctionImport(
    const ScriptSystemFunctionInfo *function) {
	ScriptImport import;
...
void GlobalScriptState::addSystemFunctionImportList(
    const ScriptSystemFunctionInfo *list, uint32 count) {
