...
RuntimeValue Script_FileOpen(AGSEngine *vm, ScriptObject *,
...
// Obsolete file I/O function.
RuntimeValue Script_FileWrite(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
...
// Obsolete file I/O function.
RuntimeValue
Script_FileWriteRawLine(AGSEngine *vm, ScriptObject *,
                        const Common::Array<RuntimeValue> &params) {
	int fileHandle = params[0]._signedValue;
	UNUSED(fileHandle);
	ScriptString *text = (ScriptString *) params[1]._object;
...
// Obsolete file I/O function.
RuntimeValue Script_FileRead(AGSEngine *vm, ScriptObject *,
                             const Common::Array<RuntimeValue> &params) {
	int fileHandle = params[0]._signedValue;
	UNUSED(fileHandle);
...
// Obsolete file I/O function.
RuntimeValue Script_FileClose(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
	int fileHandle = params[0]._signedValue;
	UNUSED(fileHandle);
...
// Obsolete file I/O function.
RuntimeValue Script_FileWriteInt(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
	int fileHandle = params[0]._signedValue;
	UNUSED(fileHandle);
...
// Obsolete file I/O function.
RuntimeValue Script_FileReadInt(AGSEngine *vm, ScriptObject *,
                                const Common::Array<RuntimeValue> &params) {
	int fileHandle = params[0]._signedValue;
	UNUSED(fileHandle);
...
// Obsolete file I/O function.
RuntimeValue Script_FileReadRawChar(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	int fileHandle = params[0]._signedValue;
	UNUSED(fileHandle);
...
// Obsolete file I/O function.
RuntimeValue
Script_FileWriteRawChar(AGSEngine *vm, ScriptObject *,
                        const Common::Array<RuntimeValue> &params) {
	int fileHandle = params[0]._signedValue;
	UNUSED(fileHandle);
	int value = params[1]._signedValue;
...
// Obsolete file I/O function.
RuntimeValue Script_FileReadRawInt(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	int fileHandle = params[0]._signedValue;
	UNUSED(fileHandle);
...
// Obsolete file I/O function.
RuntimeValue Script_FileIsEOF(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
	int fileHandle = params[0]._signedValue;
	UNUSED(fileHandle);
...
// Obsolete file I/O function.
RuntimeValue Script_FileIsError(AGSEngine *vm, ScriptObject *,
                                const Common::Array<RuntimeValue> &params) {
	int fileHandle = params[0]._signedValue;
	UNUSED(fileHandle);
...
RuntimeValue Script_File_Delete(AGSEngine *vm, ScriptObject *,
                                const Common::Array<RuntimeValue> &params) {
	ScriptString *filename = (ScriptString *) params[0]._object;
	UNUSED(filename);
...
RuntimeValue Script_File_Open(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
	ScriptString *filename = (ScriptString *) params[0]._object;
	UNUSED(filename);
...
// Closes the file.
RuntimeValue Script_File_Close(AGSEngine *vm, ScriptFile *self,
                               const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue Script_File_ReadInt(AGSEngine *vm, ScriptFile *self,
                                 const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_File_ReadRawChar(AGSEngine *vm, ScriptFile *self,
                        const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue Script_File_ReadRawInt(AGSEngine *vm, ScriptFile *self,
                                    const Common::Array<RuntimeValue> &params) {
	// FIXME
...
// Undocumented.
RuntimeValue
Script_File_ReadRawLine(AGSEngine *vm, ScriptFile *self,
                        const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue Script_File_ReadString(AGSEngine *vm, ScriptFile *self,
                                    const Common::Array<RuntimeValue> &params) {
	ScriptString *buffer = (ScriptString *) params[0]._object;
	UNUSED(buffer);
...
RuntimeValue
Script_File_ReadStringBack(AGSEngine *vm, ScriptFile *self,
                           const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue Script_File_WriteInt(AGSEngine *vm, ScriptFile *self,
                                  const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_File_WriteRawChar(AGSEngine *vm, ScriptFile *self,
                         const Common::Array<RuntimeValue> &params) {
	int value = params[0]._signedValue;
	UNUSED(value);
...
RuntimeValue
Script_File_WriteRawLine(AGSEngine *vm, ScriptFile *self,
                         const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_File_WriteString(AGSEngine *vm, ScriptFile *self,
                        const Common::Array<RuntimeValue> &params) {
	ScriptString *text = (ScriptString *) params[0]._object;
	UNUSED(text);
...
RuntimeValue Script_File_get_Error(AGSEngine *vm, ScriptFile *self,
                                   const Common::Array<RuntimeValue> &params) {
	// FIXME
...
static const ScriptSystemFunctionInfo ourFunctionList[] = {
    ...
    {"FileWriteRawLine", (ScriptAPIFunction *) &Script_FileWriteRawLine, "is",
     sotNone},
    ...
    {"FileReadRawChar", (ScriptAPIFunction *) &Script_FileReadRawChar, "i",
     sotNone},
    {"FileWriteRawChar", (ScriptAPIFunction *) &Script_FileWriteRawChar, "ii",
     sotNone},
    {"FileReadRawInt", (ScriptAPIFunction *) &Script_FileReadRawInt, "i",
     sotNone},
    ...
    {"File::Close^0", (ScriptAPIFunction *) &Script_File_Close, "", sotNone},
    {"File::ReadInt^0", (ScriptAPIFunction *) &Script_File_ReadInt, "",
     sotNone},
    {"File::ReadRawChar^0", (ScriptAPIFunction *) &Script_File_ReadRawChar, "",
     sotNone},
    {"File::ReadRawInt^0", (ScriptAPIFunction *) &Script_File_ReadRawInt, "",
     sotNone},
    {"File::ReadRawLine^1", (ScriptAPIFunction *) &Script_File_ReadRawLine, "s",
     sotNone},
    {"File::ReadString^1", (ScriptAPIFunction *) &Script_File_ReadString, "s",
     sotNone},
    {"File::ReadRawLineBack^0",
     (ScriptAPIFunction *) &Script_File_ReadRawLineBack, "", sotNone},
    {"File::ReadStringBack^0",
     (ScriptAPIFunction *) &Script_File_ReadStringBack, "", sotNone},
    {"File::WriteInt^1", (ScriptAPIFunction *) &Script_File_WriteInt, "i",
     sotNone},
    {"File::WriteRawChar^1", (ScriptAPIFunction *) &Script_File_WriteRawChar,
     "i", sotNone},
    {"File::WriteRawLine^1", (ScriptAPIFunction *) &Script_File_WriteRawLine,
     "s", sotNone},
    {"File::WriteString^1", (ScriptAPIFunction *) &Script_File_WriteString, "s",
     sotNone},
    {"File::get_EOF", (ScriptAPIFunction *) &Script_File_get_EOF, "", sotNone},
    {"File::get_Error", (ScriptAPIFunction *) &Script_File_get_Error, "",
     sotNone},
};
