...
RuntimeValue Script_ProcessClick(AGSEngine *vm, ScriptObject *,
	...
	if (cursorMode == MODE_WALK && !vm->getGameOption(OPT_NOWALKMODE)) {
		...
		if (hotspotId && vm->_state->_autoUseWalkToPoints) {
			...
			const RoomHotspot &hotspot =
			    vm->getCurrentRoom()->_hotspots[hotspotId];
	...
	case LOCTYPE_OBJ:
		...
		break;
	case LOCTYPE_HOTSPOT: vm->runHotspotInteraction(locId, cursorMode); break;
	default:
...
RuntimeValue
Script_Mouse_DisableMode(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	uint32 cursormode = params[0]._value;

	if (cursormode >= vm->_gameFile->_cursors.size())
		error("Mouse::DisableMode: mode %d is too high (only %d cursors)",
		      cursormode, vm->_gameFile->_cursors.size());
...
RuntimeValue
Script_Mouse_EnableMode(AGSEngine *vm, ScriptObject *,
                        const Common::Array<RuntimeValue> &params) {
	uint32 cursormode = params[0]._value;

	if (cursormode >= vm->_gameFile->_cursors.size())
		error("Mouse::EnableMode: mode %d is too high (only %d cursors)",
		      cursormode, vm->_gameFile->_cursors.size());
...
RuntimeValue
Script_Mouse_GetModeGraphic(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
	uint32 cursormode = params[0]._value;

	if (cursormode >= vm->_gameFile->_cursors.size())
		error("Mouse::GetModeGraphic: mode %d is too high (only %d cursors)",
		      cursormode, vm->_gameFile->_cursors.size());
...
// Remembers the current mouse cursor and restores it when the mouse leaves the
// current area.
RuntimeValue Script_Mouse_SaveCursorUntilItLeaves(
...
RuntimeValue
Script_Mouse_SelectNextMode(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
	vm->setNextCursor();
...
RuntimeValue Script_Mouse_SetBounds(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Mouse_SetPosition(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;
...
// Changes the mouse cursor to use the graphic for a different non-active cursor
// mode.
RuntimeValue
...
// Mouse: import static attribute CursorMode Mode
// Gets/sets the current mouse cursor mode.
RuntimeValue Script_Mouse_set_Mode(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
...
// Gets/sets whether the mouse cursor is visible.
RuntimeValue
Script_Mouse_set_Visible(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
...
// Mouse function.
RuntimeValue Script_SetCursorMode(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	uint32 cursormode = params[0]._value;
...
// Mouse function.
RuntimeValue
Script_SetNextCursorMode(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	vm->setNextCursor();
...
// Mouse function.
RuntimeValue Script_SetMouseBounds(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	int left = params[0]._signedValue;
	UNUSED(left);
	int top = params[1]._signedValue;
...
// Mouse function.
RuntimeValue
Script_SetMousePosition(AGSEngine *vm, ScriptObject *,
                        const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;
	vm->_system->warpMouse(x, y);
...
// Mouse function.
RuntimeValue Script_ShowMouseCursor(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	// FIXME
...
// Mouse function.
RuntimeValue Script_HideMouseCursor(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	// FIXME
...
// Mouse function.
RuntimeValue Script_RefreshMouse(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
	// Do nothing.
...
// Mouse function.
RuntimeValue
Script_DisableCursorMode(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	uint32 cursormode = params[0]._value;

	if (cursormode >= vm->_gameFile->_cursors.size())
		error("DisableCursorMode: mode %d is too high (only %d cursors)",
		      cursormode, vm->_gameFile->_cursors.size());
...
// Mouse function.
RuntimeValue
Script_EnableCursorMode(AGSEngine *vm, ScriptObject *,
                        const Common::Array<RuntimeValue> &params) {
	uint32 cursormode = params[0]._value;

	if (cursormode >= vm->_gameFile->_cursors.size())
		error("EnableCursorMode: mode %d is too high (only %d cursors)",
		      cursormode, vm->_gameFile->_cursors.size());
...
// Mouse function.
RuntimeValue
Script_SaveCursorForLocationChange(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue Script_IsButtonDown(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
	uint32 mouseButton = params[0]._value;
...
// Blocks the script for the specified number of game loops, unless a key is
// pressed.
...
RuntimeValue Script_IsKeyPressed(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
...
static const ScriptSystemFunctionInfo ourFunctionList[] = {
    {"ProcessClick", (ScriptAPIFunction *) &Script_ProcessClick, "iii",
     sotNone},
    {"Mouse::ChangeModeGraphic^2",
     (ScriptAPIFunction *) &Script_Mouse_ChangeModeGraphic, "ii", sotNone},
    {"Mouse::ChangeModeHotspot^3",
     (ScriptAPIFunction *) &Script_Mouse_ChangeModeHotspot, "iii", sotNone},
    {"Mouse::ChangeModeView^2",
     (ScriptAPIFunction *) &Script_Mouse_ChangeModeView, "ii", sotNone},
    {"Mouse::DisableMode^1", (ScriptAPIFunction *) &Script_Mouse_DisableMode,
     "i", sotNone},
    {"Mouse::EnableMode^1", (ScriptAPIFunction *) &Script_Mouse_EnableMode, "i",
     sotNone},
    {"Mouse::GetModeGraphic^1",
     (ScriptAPIFunction *) &Script_Mouse_GetModeGraphic, "i", sotNone},
    {"Mouse::IsButtonDown^1", (ScriptAPIFunction *) &Script_Mouse_IsButtonDown,
     "i", sotNone},
    {"Mouse::SaveCursorUntilItLeaves^0",
     (ScriptAPIFunction *) &Script_Mouse_SaveCursorUntilItLeaves, "", sotNone},
    {"Mouse::SelectNextMode^0",
     (ScriptAPIFunction *) &Script_Mouse_SelectNextMode, "", sotNone},
    {"Mouse::SetBounds^4", (ScriptAPIFunction *) &Script_Mouse_SetBounds,
     "iiii", sotNone},
    {"Mouse::SetPosition^2", (ScriptAPIFunction *) &Script_Mouse_SetPosition,
     "ii", sotNone},
    {"Mouse::Update^0", (ScriptAPIFunction *) &Script_Mouse_Update, "",
     sotNone},
    {"Mouse::UseDefaultGraphic^0",
     (ScriptAPIFunction *) &Script_Mouse_UseDefaultGraphic, "", sotNone},
    {"Mouse::UseModeGraphic^1",
     (ScriptAPIFunction *) &Script_Mouse_UseModeGraphic, "i", sotNone},
    {"Mouse::get_Mode", (ScriptAPIFunction *) &Script_Mouse_get_Mode, "",
     sotNone},
    {"Mouse::set_Mode", (ScriptAPIFunction *) &Script_Mouse_set_Mode, "i",
     sotNone},
    {"Mouse::get_Visible", (ScriptAPIFunction *) &Script_Mouse_get_Visible, "",
     sotNone},
    {"Mouse::set_Visible", (ScriptAPIFunction *) &Script_Mouse_set_Visible, "i",
     sotNone},
    {"ChangeCursorGraphic", (ScriptAPIFunction *) &Script_ChangeCursorGraphic,
     "ii", sotNone},
    {"ChangeCursorHotspot", (ScriptAPIFunction *) &Script_ChangeCursorHotspot,
     "iii", sotNone},
    {"GetCursorMode", (ScriptAPIFunction *) &Script_GetCursorMode, "", sotNone},
    {"SetCursorMode", (ScriptAPIFunction *) &Script_SetCursorMode, "i",
     sotNone},
    {"SetNextCursorMode", (ScriptAPIFunction *) &Script_SetNextCursorMode, "",
     sotNone},
    {"SetDefaultCursor", (ScriptAPIFunction *) &Script_SetDefaultCursor, "",
     sotNone},
    {"SetMouseCursor", (ScriptAPIFunction *) &Script_SetMouseCursor, "i",
     sotNone},
    {"SetMouseBounds", (ScriptAPIFunction *) &Script_SetMouseBounds, "iiii",
     sotNone},
    {"SetMousePosition", (ScriptAPIFunction *) &Script_SetMousePosition, "ii",
     sotNone},
    {"ShowMouseCursor", (ScriptAPIFunction *) &Script_ShowMouseCursor, "",
     sotNone},
    {"HideMouseCursor", (ScriptAPIFunction *) &Script_HideMouseCursor, "",
     sotNone},
    {"RefreshMouse", (ScriptAPIFunction *) &Script_RefreshMouse, "", sotNone},
    {"DisableCursorMode", (ScriptAPIFunction *) &Script_DisableCursorMode, "i",
     sotNone},
    {"EnableCursorMode", (ScriptAPIFunction *) &Script_EnableCursorMode, "i",
     sotNone},
    {"SaveCursorForLocationChange",
     (ScriptAPIFunction *) &Script_SaveCursorForLocationChange, "", sotNone},
