...
// newTranslationFileName) Changes the active translation.
RuntimeValue
Script_Game_ChangeTranslation(AGSEngine *vm, ScriptObject *,
...
RuntimeValue Script_Game_DoOnceOnly(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	ScriptString *token = (ScriptString *) params[0]._object;

	Common::String string = token->getString();

	if (Common::find(vm->_state->_doOnceTokens.begin(),
	                 vm->_state->_doOnceTokens.end(),
	                 string) != vm->_state->_doOnceTokens.end())
		return 0;
...
RuntimeValue
Script_Game_GetColorFromRGB(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Game_GetFrameCountForLoop(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
	uint view = params[0]._value;
	uint loop = params[1]._value;

	if (view < 1 || view > vm->_gameFile->_views.size())
		error("Game::GetFrameCountForLoop: view %d is invalid or too high "
		      "(only have %d)",
		      view, vm->_gameFile->_views.size());
	view--;
	if (loop >= vm->_gameFile->_views[view]._loops.size())
		error("Game::GetFrameCountForLoop: loop %d (in view %d) is invalid or "
		      "too high (only have %d)",
...
RuntimeValue
Script_Game_GetLocationName(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;

	RuntimeValue ret =
	    new ScriptMutableString(vm->getLocationName(Common::Point(x, y)));
	ret._object->DecRef();
	return ret;
}
...
RuntimeValue
Script_Game_GetLoopCountForView(AGSEngine *vm, ScriptObject *,
                                const Common::Array<RuntimeValue> &params) {
	uint view = params[0]._value;

	if (view == 0 || view - 1 >= vm->_gameFile->_views.size())
		error("GetLoopCountForView: view %d is invalid or too high (only have "
		      "%d)",
		      view, vm->_gameFile->_views.size());
...
RuntimeValue
Script_Game_GetMODPattern(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	// FIXME
...
// Gets whether the "Run next loop after this" setting is checked for the
// specified loop.
RuntimeValue Script_Game_GetRunNextSettingForLoop(
...
RuntimeValue
Script_Game_GetViewFrame(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	...
	if (viewId == 0 || viewId - 1 >= vm->_gameFile->_views.size())
		error(
		    "Game::GetViewFrame: view %d is invalid or too high (only have %d)",
		    viewId, vm->_gameFile->_views.size());
	if (loopId >= vm->_gameFile->_views[viewId - 1]._loops.size())
		error("Game::GetViewFrame: loop %d is too high (only have %d)", loopId,
		      vm->_gameFile->_views[viewId - 1]._loops.size());

	ViewLoopNew &loop = vm->_gameFile->_views[viewId - 1]._loops[loopId];
	if (frameId >= loop._frames.size())
		error("Game::GetViewFrame: frame %d is too high (only have %d)",
		      frameId, loop._frames.size());
...
RuntimeValue Script_Game_InputBox(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Game_IsAudioPlaying(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
...
// Game: import static void SetAudioTypeSpeechVolumeDrop(AudioType, int
// volumeDrop) Changes the volume drop applied to this audio type when speech is
// played
RuntimeValue Script_Game_SetAudioTypeSpeechVolumeDrop(
    AGSEngine *vm, ScriptObject *, const Common::Array<RuntimeValue> &params) {
	uint audioType = params[0]._value;
	int volumeDrop = params[1]._signedValue;

	if (audioType >= vm->_audio->_audioClipTypes.size())
		error("Game::SetAudioTypeSpeechVolumeDrop: clip type %d is too high "
		      "(only have %d)",
		      audioType, vm->_audio->_audioClipTypes.size());

	vm->_audio->_audioClipTypes[audioType]._volumeReductionWhileSpeechPlaying =
	    volumeDrop;

	return RuntimeValue();
}

// Game: import static void SetAudioTypeVolume(AudioType, int volume,
// ChangeVolumeType) Changes the default volume of audio clips of the specified
// type.
RuntimeValue
Script_Game_SetAudioTypeVolume(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
	uint32 audiotype = params[0]._value;
...
RuntimeValue
Script_Game_SetSaveGameDirectory(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_Game_StopAudio(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	uint32 audioType = params[0]._value;
	UNUSED(audioType);
...
RuntimeValue Script_Game_StopSound(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Game_get_CharacterCount(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Game_get_DialogCount(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_Game_get_FontCount(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Game_geti_GlobalMessages(AGSEngine *vm, ScriptObject *,
                                const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Game_geti_GlobalStrings(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
	uint index = params[0]._value;

	if (index >= vm->_state->_globalStrings.size())
		error(
		    "Game::geti_GlobalStrings: invalid string id %d (only %d present)",
		    index, vm->_state->_globalStrings.size());

	RuntimeValue ret =
	    new ScriptMutableString(vm->_state->_globalStrings[index]);
	ret._object->DecRef();
	return ret;
}
...
// Accesses the global strings collection. This is obsolete.
RuntimeValue
Script_Game_seti_GlobalStrings(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
	uint index = params[0]._value;
	...
	if (index >= vm->_state->_globalStrings.size())
		error(
		    "Game::seti_GlobalStrings: invalid string id %d (only %d present)",
		    index, vm->_state->_globalStrings.size());
...
RuntimeValue
Script_Game_get_GUICount(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	return vm->_gameFile->_guiGroups.size();
}

// Game: import static attribute int IgnoreUserInputAfterTextTimeoutMs
// Gets/sets the time for which user input is ignored after some text is
...
RuntimeValue
Script_Game_get_InSkippableCutscene(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Game_get_InventoryItemCount(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	// Item 0 isn't valid in 3.x.
	// "because of the dummy item 0, this is always one higher than it should
	// be"
	return vm->_gameFile->_invItemInfo.size() - 1;
}

// Game: import static attribute int MinimumTextDisplayTimeMs
// Gets/sets the minimum time that a piece of speech text stays on screen (in
// milliseconds)
...
RuntimeValue
Script_Game_get_MouseCursorCount(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_Game_get_Name(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
...
// Game: import static attribute String Name
// Gets/sets the game name.
RuntimeValue Script_Game_set_Name(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Game_get_NormalFont(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
...
// Gets/sets the normal font used for displaying text.
RuntimeValue
Script_Game_set_NormalFont(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
	uint value = params[0]._value;
...
RuntimeValue
Script_Game_get_SkippingCutscene(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Game_get_SpeechFont(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
...
// Gets/sets the font used for displaying speech text.
RuntimeValue
Script_Game_set_SpeechFont(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
	uint value = params[0]._value;
...
RuntimeValue
Script_Game_get_TextReadingSpeed(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
...
// Gets/sets how fast speech text is removed from the screen.
RuntimeValue
Script_Game_set_TextReadingSpeed(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
	uint value = params[0]._value;
...
RuntimeValue
Script_Game_get_TranslationFilename(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue Script_AbortGame(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_GetGameOption(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_IsTranslationAvailable(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_RestoreGameDialog(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue Script_SaveGameDialog(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue Script_RestartGame(AGSEngine *vm, ScriptObject *,
                                const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue Script_DeleteSaveSlot(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	int slot = params[0]._signedValue;
	UNUSED(slot);
...
// Undocumented.
RuntimeValue
Script_GetGraphicalVariable(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue
Script_SetGraphicalVariable(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
	ScriptString *variableName = (ScriptString *) params[0]._object;
	UNUSED(variableName);
	int value = params[1]._signedValue;
...
// Old string buffer function.
RuntimeValue Script_GetGlobalString(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	uint stringID = params[0]._value;
	ScriptString *buffer = (ScriptString *) params[1]._object;

	if (stringID >= vm->_state->_globalStrings.size())
		error("GetGlobalString: invalid string id %d (only %d present)",
		      stringID, vm->_state->_globalStrings.size());
...
// Old string buffer function.
RuntimeValue Script_InputBox(AGSEngine *vm, ScriptObject *,
                             const Common::Array<RuntimeValue> &params) {
	ScriptString *prompt = (ScriptString *) params[0]._object;
	UNUSED(prompt);
	ScriptString *buffer = (ScriptString *) params[1]._object;
...
// Old string buffer function.
RuntimeValue
Script_GetTranslationName(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
...
// Old string buffer function.
RuntimeValue
Script_GetSaveSlotDescription(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
	int slot = params[0]._signedValue;
	UNUSED(slot);
	ScriptString *buffer = (ScriptString *) params[1]._object;
...
// Undocumented.
RuntimeValue Script_GetMessageText(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue
Script_LoadSaveSlotScreenshot(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
	int saveSlot = params[0]._signedValue;
	UNUSED(saveSlot);
...
// import int GetGameParameter(int parameter, int data1=0, int data2=0, int
// data3=0) Obsolete function for retrieving game parameters. Since replaced by
// individual functions.
RuntimeValue
Script_GetGameParameter(AGSEngine *vm, ScriptObject *,
                        const Common::Array<RuntimeValue> &params) {
	...
	switch (parameter) {
	case GP_SPRITEWIDTH: return vm->getSprites()->getSpriteWidth(data1);
	case GP_SPRITEHEIGHT: return vm->getSprites()->getSpriteHeight(data1);
	case GP_NUMLOOPS:
	case GP_NUMFRAMES:
	case GP_FRAMESPEED:
	case GP_FRAMEIMAGE:
	...
	case GP_NUMGUIS: return vm->_gameFile->_guiGroups.size();
	case GP_NUMOBJECTS: return vm->getCurrentRoom()->_objects.size();
	case GP_NUMCHARACTERS: return vm->_characters.size();
	case GP_NUMINVITEMS: return vm->_gameFile->_invItemInfo.size();
	default: error("GetGameParameter: unknown parameter %d", parameter);
	}
}
...
RuntimeValue Script_SetVoiceMode(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
	...
	if (newMode < 0 || newMode > 2)
		error("SetVoiceMode: invalid mode number %d (must be 0, 1 or 2)",
		      newMode);
...
RuntimeValue Script_SetSpeechStyle(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue Script_SetGlobalInt(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue Script_GetGlobalInt(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
	uint globalInt = params[0]._value;
...
static const ScriptSystemFunctionInfo ourFunctionList[] = {
    {"Game::ChangeTranslation^1",
     (ScriptAPIFunction *) &Script_Game_ChangeTranslation, "s", sotNone},
    {"Game::DoOnceOnly^1", (ScriptAPIFunction *) &Script_Game_DoOnceOnly, "s",
     sotNone},
    {"Game::GetColorFromRGB^3",
     (ScriptAPIFunction *) &Script_Game_GetColorFromRGB, "iii", sotNone},
    {"Game::GetFrameCountForLoop^2",
     (ScriptAPIFunction *) &Script_Game_GetFrameCountForLoop, "ii", sotNone},
    {"Game::GetLocationName^2",
     (ScriptAPIFunction *) &Script_Game_GetLocationName, "ii", sotNone},
    {"Game::GetLoopCountForView^1",
     (ScriptAPIFunction *) &Script_Game_GetLoopCountForView, "i", sotNone},
    {"Game::GetMODPattern^0", (ScriptAPIFunction *) &Script_Game_GetMODPattern,
     "", sotNone},
    {"Game::GetRunNextSettingForLoop^2",
     (ScriptAPIFunction *) &Script_Game_GetRunNextSettingForLoop, "ii",
     sotNone},
    {"Game::GetSaveSlotDescription^1",
     (ScriptAPIFunction *) &Script_Game_GetSaveSlotDescription, "i", sotNone},
    {"Game::GetViewFrame^3", (ScriptAPIFunction *) &Script_Game_GetViewFrame,
     "iii", sotNone},
    {"Game::InputBox^1", (ScriptAPIFunction *) &Script_Game_InputBox, "s",
     sotNone},
    {"Game::IsAudioPlaying^1",
     (ScriptAPIFunction *) &Script_Game_IsAudioPlaying, "i", sotNone},
    {"Game::SetAudioTypeSpeechVolumeDrop^2",
     (ScriptAPIFunction *) &Script_Game_SetAudioTypeSpeechVolumeDrop, "ii",
     sotNone},
    {"Game::SetAudioTypeVolume^3",
     (ScriptAPIFunction *) &Script_Game_SetAudioTypeVolume, "iii", sotNone},
    {"Game::SetSaveGameDirectory^1",
     (ScriptAPIFunction *) &Script_Game_SetSaveGameDirectory, "s", sotNone},
    {"Game::StopAudio^1", (ScriptAPIFunction *) &Script_Game_StopAudio, "i",
     sotNone},
    {"Game::StopSound^1", (ScriptAPIFunction *) &Script_Game_StopSound, "i",
     sotNone},
    {"Game::get_CharacterCount",
     (ScriptAPIFunction *) &Script_Game_get_CharacterCount, "", sotNone},
    {"Game::get_DialogCount",
     (ScriptAPIFunction *) &Script_Game_get_DialogCount, "", sotNone},
    {"Game::get_FileName", (ScriptAPIFunction *) &Script_Game_get_FileName, "",
     sotNone},
    {"Game::get_FontCount", (ScriptAPIFunction *) &Script_Game_get_FontCount,
     "", sotNone},
    {"Game::geti_GlobalMessages",
     (ScriptAPIFunction *) &Script_Game_geti_GlobalMessages, "i", sotNone},
    {"Game::geti_GlobalStrings",
     (ScriptAPIFunction *) &Script_Game_geti_GlobalStrings, "i", sotNone},
    {"Game::seti_GlobalStrings",
     (ScriptAPIFunction *) &Script_Game_seti_GlobalStrings, "is", sotNone},
    {"Game::get_GUICount", (ScriptAPIFunction *) &Script_Game_get_GUICount, "",
     sotNone},
    {"Game::get_IgnoreUserInputAfterTextTimeoutMs",
     (ScriptAPIFunction *) &Script_Game_get_IgnoreUserInputAfterTextTimeoutMs,
     "", sotNone},
    {"Game::set_IgnoreUserInputAfterTextTimeoutMs",
     (ScriptAPIFunction *) &Script_Game_set_IgnoreUserInputAfterTextTimeoutMs,
     "i", sotNone},
    {"Game::get_InSkippableCutscene",
     (ScriptAPIFunction *) &Script_Game_get_InSkippableCutscene, "", sotNone},
    {"Game::get_InventoryItemCount",
     (ScriptAPIFunction *) &Script_Game_get_InventoryItemCount, "", sotNone},
    {"Game::get_MinimumTextDisplayTimeMs",
     (ScriptAPIFunction *) &Script_Game_get_MinimumTextDisplayTimeMs, "",
     sotNone},
    {"Game::set_MinimumTextDisplayTimeMs",
     (ScriptAPIFunction *) &Script_Game_set_MinimumTextDisplayTimeMs, "i",
     sotNone},
    {"Game::get_MouseCursorCount",
     (ScriptAPIFunction *) &Script_Game_get_MouseCursorCount, "", sotNone},
    {"Game::get_Name", (ScriptAPIFunction *) &Script_Game_get_Name, "",
     sotNone},
    {"Game::set_Name", (ScriptAPIFunction *) &Script_Game_set_Name, "s",
     sotNone},
    {"Game::get_NormalFont", (ScriptAPIFunction *) &Script_Game_get_NormalFont,
     "", sotNone},
    {"Game::set_NormalFont", (ScriptAPIFunction *) &Script_Game_set_NormalFont,
     "i", sotNone},
    {"Game::get_SkippingCutscene",
     (ScriptAPIFunction *) &Script_Game_get_SkippingCutscene, "", sotNone},
    {"Game::get_SpeechFont", (ScriptAPIFunction *) &Script_Game_get_SpeechFont,
     "", sotNone},
    {"Game::set_SpeechFont", (ScriptAPIFunction *) &Script_Game_set_SpeechFont,
     "i", sotNone},
    {"Game::geti_SpriteHeight",
     (ScriptAPIFunction *) &Script_Game_geti_SpriteHeight, "i", sotNone},
    {"Game::geti_SpriteWidth",
     (ScriptAPIFunction *) &Script_Game_geti_SpriteWidth, "i", sotNone},
    {"Game::get_TextReadingSpeed",
     (ScriptAPIFunction *) &Script_Game_get_TextReadingSpeed, "", sotNone},
    {"Game::set_TextReadingSpeed",
     (ScriptAPIFunction *) &Script_Game_set_TextReadingSpeed, "i", sotNone},
    {"Game::get_TranslationFilename",
     (ScriptAPIFunction *) &Script_Game_get_TranslationFilename, "", sotNone},
    {"Game::get_UseNativeCoordinates",
     (ScriptAPIFunction *) &Script_Game_get_UseNativeCoordinates, "", sotNone},
    {"Game::get_ViewCount", (ScriptAPIFunction *) &Script_Game_get_ViewCount,
     "", sotNone},
    ...
    {"SetGameOption", (ScriptAPIFunction *) &Script_SetGameOption, "ii",
     sotNone},
    {"GetGameOption", (ScriptAPIFunction *) &Script_GetGameOption, "i",
     sotNone},
    {"GetTranslation", (ScriptAPIFunction *) &Script_GetTranslation, "s",
     sotNone},
    {"IsTranslationAvailable",
     (ScriptAPIFunction *) &Script_IsTranslationAvailable, "", sotNone},
    {"RestoreGameDialog", (ScriptAPIFunction *) &Script_RestoreGameDialog, "",
     sotNone},
    {"SaveGameDialog", (ScriptAPIFunction *) &Script_SaveGameDialog, "",
     sotNone},
    {"RestartGame", (ScriptAPIFunction *) &Script_RestartGame, "", sotNone},
    {"SaveGameSlot", (ScriptAPIFunction *) &Script_SaveGameSlot, "is", sotNone},
    {"RestoreGameSlot", (ScriptAPIFunction *) &Script_RestoreGameSlot, "i",
     sotNone},
    {"DeleteSaveSlot", (ScriptAPIFunction *) &Script_DeleteSaveSlot, "i",
     sotNone},
    {"SetRestartPoint", (ScriptAPIFunction *) &Script_SetRestartPoint, "",
     sotNone},
    {"IsGamePaused", (ScriptAPIFunction *) &Script_IsGamePaused, "", sotNone},
    {"GetGraphicalVariable", (ScriptAPIFunction *) &Script_GetGraphicalVariable,
     "s", sotNone},
    {"SetGraphicalVariable", (ScriptAPIFunction *) &Script_SetGraphicalVariable,
     "si", sotNone},
    {"SetGlobalString", (ScriptAPIFunction *) &Script_SetGlobalString, "is",
     sotNone},
    {"GetGlobalString", (ScriptAPIFunction *) &Script_GetGlobalString, "is",
     sotNone},
    {"InputBox", (ScriptAPIFunction *) &Script_InputBox, "ss", sotNone},
    {"GetTranslationName", (ScriptAPIFunction *) &Script_GetTranslationName,
     "s", sotNone},
    {"GetSaveSlotDescription",
     (ScriptAPIFunction *) &Script_GetSaveSlotDescription, "is", sotNone},
    {"GetMessageText", (ScriptAPIFunction *) &Script_GetMessageText, "is",
     sotNone},
    {"LoadSaveSlotScreenshot",
     (ScriptAPIFunction *) &Script_LoadSaveSlotScreenshot, "iii", sotNone},
    {"GetGameParameter", (ScriptAPIFunction *) &Script_GetGameParameter, "iiii",
     sotNone},
    {"GiveScore", (ScriptAPIFunction *) &Script_GiveScore, "i", sotNone},
    {"SetVoiceMode", (ScriptAPIFunction *) &Script_SetVoiceMode, "i", sotNone},
    {"SetSkipSpeech", (ScriptAPIFunction *) &Script_SetSkipSpeech, "i",
     sotNone},
    {"SetSpeechStyle", (ScriptAPIFunction *) &Script_SetSpeechStyle, "i",
     sotNone},
