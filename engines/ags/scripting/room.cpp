...
RuntimeValue
Script_Room_GetTextProperty(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
	ScriptString *property = (ScriptString *) params[0]._object;

	Common::String string = vm->getStringProperty(
	    property->getString(), vm->getCurrentRoom()->_properties);
	RuntimeValue ret = new ScriptMutableString(string);
	ret._object->DecRef();
	return ret;
}

// Room: import static DrawingSurface* GetDrawingSurfaceForBackground(int
// backgroundNumber=SCR_NO_VALUE) Gets a drawing surface that allows you to
// manipulate the room background.
RuntimeValue Script_Room_GetDrawingSurfaceForBackground(
	...
	if (backgroundNumber >= room->_backgroundScenes.size())
		error("Room::GetDrawingSurfaceForBackground: background %d is too high "
		      "(only %d backgrounds)",
...
RuntimeValue
Script_Room_get_BottomEdge(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_Room_get_ColorDepth(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue Script_Room_get_Height(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Room_get_LeftEdge(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_Room_geti_Messages(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Room_get_MusicOnLoad(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_Room_get_RightEdge(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_Room_get_TopEdge(AGSEngine *vm, ScriptObject *,
                        const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue Script_Room_get_Width(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_ResetRoom(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_HasPlayerBeenInRoom(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
	int roomNumber = params[0]._signedValue;
	UNUSED(roomNumber);
...
RuntimeValue Script_CallRoomScript(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
...
// Returns which walkable area is at the specified position relative to the
// current viewport.
RuntimeValue
Script_GetWalkableAreaAt(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;
...
RuntimeValue Script_GetRoomProperty(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	ScriptString *property = (ScriptString *) params[0]._object;
...
RuntimeValue Script_GetLocationName(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;
...
// Undocumented.
RuntimeValue Script_GetObjectAt(AGSEngine *vm, ScriptObject *,
                                const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;
...
RuntimeValue Script_DisableHotspot(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	uint hotspot = params[0]._value;

	if (hotspot >= vm->getCurrentRoom()->_hotspots.size())
		error("DisableHotspot: hotspot %d is too high (only have %d)", hotspot,
...
// Obsolete hotspot function.
RuntimeValue Script_EnableHotspot(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	uint hotspot = params[0]._value;

	if (hotspot >= vm->getCurrentRoom()->_hotspots.size())
		error("EnableHotspot: hotspot %d is too high (only have %d)", hotspot,
		      vm->getCurrentRoom()->_hotspots.size());
...
// Obsolete hotspot function.
RuntimeValue
Script_GetHotspotProperty(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	uint hotspot = params[0]._value;
	...
	return vm->getIntProperty(
	    property->getString(),
	    vm->getCurrentRoom()->_hotspots[hotspot]._properties);
}

// import void GetHotspotPropertyText(int hotspot, const string property, string
// buffer) Obsolete hotspot function.
RuntimeValue
Script_GetHotspotPropertyText(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
	uint hotspot = params[0]._value;
	ScriptString *property = (ScriptString *) params[1]._object;
	ScriptString *buffer = (ScriptString *) params[2]._object;
...
// Obsolete hotspot function.
RuntimeValue
Script_RunHotspotInteraction(AGSEngine *vm, ScriptObject *,
                             const Common::Array<RuntimeValue> &params) {
	uint hotspot = params[0]._value;
...
RuntimeValue Script_DisableRegion(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	uint region = params[0]._value;

	if (region >= vm->getCurrentRoom()->_regions.size())
		error("DisableRegion: region %d is too high (only have %d)", region,
		      vm->getCurrentRoom()->_regions.size());
...
// Obsolete region function.
RuntimeValue Script_EnableRegion(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
	uint region = params[0]._value;

	if (region >= vm->getCurrentRoom()->_regions.size())
		error("EnableRegion: region %d is too high (only have %d)", region,
		      vm->getCurrentRoom()->_regions.size());
...
// Obsolete region function.
RuntimeValue
Script_RunRegionInteraction(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
...
// Obsolete region function.
RuntimeValue
Script_SetAreaLightLevel(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
...
// Obsolete region function.
RuntimeValue Script_SetRegionTint(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	int area = params[0]._signedValue;
	UNUSED(area);
...
RuntimeValue
Script_RemoveWalkableArea(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	uint area = params[0]._value;

	if (area < 1 || area > 15)
		error("RemoveWalkableArea: invalid walkable area %d specified (must be "
		      "1-15).",
		      area);
...
RuntimeValue
Script_RestoreWalkableArea(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
	uint area = params[0]._value;
...
RuntimeValue Script_SetAreaScaling(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	int area = params[0]._signedValue;
	UNUSED(area);
...
RuntimeValue
Script_DisableGroundLevelAreas(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_EnableGroundLevelAreas(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_SetWalkBehindBase(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	uint area = params[0]._value;
	...
	if (baseline != vm->getCurrentRoom()->_walkBehinds[area]._baseline) {
		...
		debugC(kDebugLevelGame, "walk-behind %d baseline changed to %d", area,
		       baseline);
	}
...
RuntimeValue
Script_Hotspot_GetAtScreenXY(AGSEngine *vm, ScriptObject *,
                             const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;
...
// Undocumented.
RuntimeValue
Script_Hotspot_GetPropertyText(AGSEngine *vm, RoomHotspot *self,
                               const Common::Array<RuntimeValue> &params) {
	ScriptString *property = (ScriptString *) params[0]._object;
	ScriptString *buffer = (ScriptString *) params[1]._object;
...
RuntimeValue
Script_Hotspot_GetProperty(AGSEngine *vm, RoomHotspot *self,
                           const Common::Array<RuntimeValue> &params) {
	ScriptString *property = (ScriptString *) params[0]._object;
...
RuntimeValue
Script_Hotspot_GetTextProperty(AGSEngine *vm, RoomHotspot *self,
                               const Common::Array<RuntimeValue> &params) {
	ScriptString *property = (ScriptString *) params[0]._object;
...
RuntimeValue
Script_Hotspot_RunInteraction(AGSEngine *vm, RoomHotspot *self,
                              const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_Hotspot_get_ID(AGSEngine *vm, RoomHotspot *self,
                                   const Common::Array<RuntimeValue> &params) {
	return self->_id;
}
...
RuntimeValue
Script_Hotspot_get_Name(AGSEngine *vm, RoomHotspot *self,
                        const Common::Array<RuntimeValue> &params) {
...
// Sets the region tint which will apply to characters that are standing on the
// region.
RuntimeValue Script_Region_Tint(AGSEngine *vm, RoomRegion *self,
                                const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Region_get_Enabled(AGSEngine *vm, RoomRegion *self,
                          const Common::Array<RuntimeValue> &params) {
	return self->_enabled ? 1 : 0;
}
...
// Gets/sets whether this region is enabled.
RuntimeValue
Script_Region_set_Enabled(AGSEngine *vm, RoomRegion *self,
                          const Common::Array<RuntimeValue> &params) {
	uint32 value = params[0]._value;
...
RuntimeValue Script_Region_get_ID(AGSEngine *vm, RoomRegion *self,
                                  const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_Region_get_TintEnabled(AGSEngine *vm, RoomRegion *self,
                              const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_Region_get_TintBlue(AGSEngine *vm, RoomRegion *self,
                           const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_Region_get_TintGreen(AGSEngine *vm, RoomRegion *self,
                            const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_Region_get_TintRed(AGSEngine *vm, RoomRegion *self,
                          const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue
Script_Region_get_TintSaturation(AGSEngine *vm, RoomRegion *self,
                                 const Common::Array<RuntimeValue> &params) {
	// FIXME
...
static const ScriptSystemFunctionInfo ourFunctionList[] = {
    {"Room::GetTextProperty^1",
     (ScriptAPIFunction *) &Script_Room_GetTextProperty, "s", sotNone},
    {"Room::GetDrawingSurfaceForBackground^1",
     (ScriptAPIFunction *) &Script_Room_GetDrawingSurfaceForBackground, "i",
     sotNone},
    {"Room::get_BottomEdge", (ScriptAPIFunction *) &Script_Room_get_BottomEdge,
     "", sotNone},
    {"Room::get_ColorDepth", (ScriptAPIFunction *) &Script_Room_get_ColorDepth,
     "", sotNone},
    {"Room::get_Height", (ScriptAPIFunction *) &Script_Room_get_Height, "",
     sotNone},
    {"Room::get_LeftEdge", (ScriptAPIFunction *) &Script_Room_get_LeftEdge, "",
     sotNone},
    {"Room::geti_Messages", (ScriptAPIFunction *) &Script_Room_geti_Messages,
     "i", sotNone},
    {"Room::get_MusicOnLoad",
     (ScriptAPIFunction *) &Script_Room_get_MusicOnLoad, "", sotNone},
    {"Room::get_ObjectCount",
     (ScriptAPIFunction *) &Script_Room_get_ObjectCount, "", sotNone},
    {"Room::get_RightEdge", (ScriptAPIFunction *) &Script_Room_get_RightEdge,
     "", sotNone},
    {"Room::get_TopEdge", (ScriptAPIFunction *) &Script_Room_get_TopEdge, "",
     sotNone},
    {"Room::get_Width", (ScriptAPIFunction *) &Script_Room_get_Width, "",
     sotNone},
    {"ResetRoom", (ScriptAPIFunction *) &Script_ResetRoom, "i", sotNone},
    {"HasPlayerBeenInRoom", (ScriptAPIFunction *) &Script_HasPlayerBeenInRoom,
     "i", sotNone},
    {"CallRoomScript", (ScriptAPIFunction *) &Script_CallRoomScript, "i",
     sotNone},
    {"GetLocationType", (ScriptAPIFunction *) &Script_GetLocationType, "ii",
     sotNone},
    {"GetWalkableAreaAt", (ScriptAPIFunction *) &Script_GetWalkableAreaAt, "ii",
     sotNone},
    {"GetScalingAt", (ScriptAPIFunction *) &Script_GetScalingAt, "ii", sotNone},
    {"GetRoomProperty", (ScriptAPIFunction *) &Script_GetRoomProperty, "s",
     sotNone},
    {"GetLocationName", (ScriptAPIFunction *) &Script_GetLocationName, "iis",
     sotNone},
    {"GetRoomPropertyText", (ScriptAPIFunction *) &Script_GetRoomPropertyText,
     "ss", sotNone},
    ...
    {"DisableHotspot", (ScriptAPIFunction *) &Script_DisableHotspot, "i",
     sotNone},
    {"EnableHotspot", (ScriptAPIFunction *) &Script_EnableHotspot, "i",
     sotNone},
    {"GetHotspotName", (ScriptAPIFunction *) &Script_GetHotspotName, "is",
     sotNone},
    {"GetHotspotPointX", (ScriptAPIFunction *) &Script_GetHotspotPointX, "i",
     sotNone},
    {"GetHotspotPointY", (ScriptAPIFunction *) &Script_GetHotspotPointY, "i",
     sotNone},
    {"GetHotspotProperty", (ScriptAPIFunction *) &Script_GetHotspotProperty,
     "is", sotNone},
    {"GetHotspotPropertyText",
     (ScriptAPIFunction *) &Script_GetHotspotPropertyText, "iss", sotNone},
    {"RunHotspotInteraction",
     (ScriptAPIFunction *) &Script_RunHotspotInteraction, "ii", sotNone},
    {"DisableRegion", (ScriptAPIFunction *) &Script_DisableRegion, "i",
     sotNone},
    {"EnableRegion", (ScriptAPIFunction *) &Script_EnableRegion, "i", sotNone},
    {"RunRegionInteraction", (ScriptAPIFunction *) &Script_RunRegionInteraction,
     "ii", sotNone},
    {"SetAreaLightLevel", (ScriptAPIFunction *) &Script_SetAreaLightLevel, "ii",
     sotNone},
    {"SetRegionTint", (ScriptAPIFunction *) &Script_SetRegionTint, "iiiii",
     sotNone},
    {"RemoveWalkableArea", (ScriptAPIFunction *) &Script_RemoveWalkableArea,
     "i", sotNone},
    {"RestoreWalkableArea", (ScriptAPIFunction *) &Script_RestoreWalkableArea,
     "i", sotNone},
    {"SetAreaScaling", (ScriptAPIFunction *) &Script_SetAreaScaling, "iii",
     sotNone},
    {"DisableGroundLevelAreas",
     (ScriptAPIFunction *) &Script_DisableGroundLevelAreas, "i", sotNone},
    {"EnableGroundLevelAreas",
     (ScriptAPIFunction *) &Script_EnableGroundLevelAreas, "", sotNone},
    {"SetWalkBehindBase", (ScriptAPIFunction *) &Script_SetWalkBehindBase, "ii",
     sotNone},
    {"Hotspot::GetAtScreenXY^2",
     (ScriptAPIFunction *) &Script_Hotspot_GetAtScreenXY, "ii", sotNone},
    {"Hotspot::GetName^1", (ScriptAPIFunction *) &Script_Hotspot_GetName, "s",
     sotHotspot},
    {"Hotspot::GetPropertyText^2",
     (ScriptAPIFunction *) &Script_Hotspot_GetPropertyText, "ss", sotHotspot},
    {"Hotspot::GetProperty^1",
     (ScriptAPIFunction *) &Script_Hotspot_GetProperty, "s", sotHotspot},
    {"Hotspot::GetTextProperty^1",
     (ScriptAPIFunction *) &Script_Hotspot_GetTextProperty, "s", sotHotspot},
    {"Hotspot::RunInteraction^1",
     (ScriptAPIFunction *) &Script_Hotspot_RunInteraction, "i", sotHotspot},
    {"Hotspot::get_Enabled", (ScriptAPIFunction *) &Script_Hotspot_get_Enabled,
     "", sotHotspot},
    {"Hotspot::set_Enabled", (ScriptAPIFunction *) &Script_Hotspot_set_Enabled,
     "i", sotHotspot},
    {"Hotspot::get_ID", (ScriptAPIFunction *) &Script_Hotspot_get_ID, "",
     sotHotspot},
    {"Hotspot::get_Name", (ScriptAPIFunction *) &Script_Hotspot_get_Name, "",
     sotHotspot},
    {"Hotspot::get_WalkToX", (ScriptAPIFunction *) &Script_Hotspot_get_WalkToX,
     "", sotHotspot},
    {"Hotspot::get_WalkToY", (ScriptAPIFunction *) &Script_Hotspot_get_WalkToY,
     "", sotHotspot},
    {"Region::GetAtRoomXY^2", (ScriptAPIFunction *) &Script_Region_GetAtRoomXY,
     "ii", sotNone},
    {"Region::RunInteraction^1",
     (ScriptAPIFunction *) &Script_Region_RunInteraction, "i", sotRegion},
    {"Region::Tint^4", (ScriptAPIFunction *) &Script_Region_Tint, "iiii",
     sotRegion},
    {"Region::get_Enabled", (ScriptAPIFunction *) &Script_Region_get_Enabled,
     "", sotRegion},
    {"Region::set_Enabled", (ScriptAPIFunction *) &Script_Region_set_Enabled,
     "i", sotRegion},
    {"Region::get_ID", (ScriptAPIFunction *) &Script_Region_get_ID, "",
     sotRegion},
    {"Region::get_LightLevel",
     (ScriptAPIFunction *) &Script_Region_get_LightLevel, "", sotRegion},
    {"Region::set_LightLevel",
     (ScriptAPIFunction *) &Script_Region_set_LightLevel, "i", sotRegion},
    {"Region::get_TintEnabled",
     (ScriptAPIFunction *) &Script_Region_get_TintEnabled, "", sotRegion},
    {"Region::get_TintBlue", (ScriptAPIFunction *) &Script_Region_get_TintBlue,
     "", sotRegion},
    {"Region::get_TintGreen",
     (ScriptAPIFunction *) &Script_Region_get_TintGreen, "", sotRegion},
    {"Region::get_TintRed", (ScriptAPIFunction *) &Script_Region_get_TintRed,
     "", sotRegion},
    {"Region::get_TintSaturation",
     (ScriptAPIFunction *) &Script_Region_get_TintSaturation, "", sotRegion},
};
