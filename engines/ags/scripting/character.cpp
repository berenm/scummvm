...
// Undocumented.
RuntimeValue Script_GetCharacterAt(AGSEngine *vm, ScriptObject *,
...
RuntimeValue Script_AddInventory(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
...
// Obsolete character function.
RuntimeValue Script_LoseInventory(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	uint item = params[0]._value;
...
// Obsolete character function.
RuntimeValue
Script_SetActiveInventory(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	uint item = params[0]._value;
...
// Obsolete character function.
RuntimeValue Script_NewRoomEx(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
...
// Obsolete character function.
RuntimeValue Script_NewRoomNPC(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
	...
	if (charid >= vm->_characters.size())
		error("NewRoomNPC: character %d is too high (only have %d)", charid,
		      vm->_characters.size());
...
// buffer) Obsolete character function.
RuntimeValue
...
// Obsolete character function.
RuntimeValue
Script_RunCharacterInteraction(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	...
	if (charid >= vm->_characters.size())
		error(
		    "RunCharacterInteraction: character %d is too high (only have %d)",
		    charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue
Script_DisplaySpeechBackground(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	ScriptString *message = (ScriptString *) params[1]._object;
...
// import void DisplaySpeechAt (int x, int y, int width, CHARID, const string
// message) Obsolete character function.
RuntimeValue Script_DisplaySpeechAt(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;
...
// Obsolete character function.
RuntimeValue Script_DisplayThought(AGSEngine *vm, ScriptObject *,
                                   const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	UNUSED(charid);
	ScriptString *message = (ScriptString *) params[1]._object;
...
// Obsolete character function.
RuntimeValue Script_FollowCharacter(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
...
// import void FollowCharacterEx(CHARID sheep, CHARID shepherd, int dist, int
// eagerness) Obsolete character function.
RuntimeValue
Script_FollowCharacterEx(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	uint32 sheep = params[0]._value;
	UNUSED(sheep);
	uint32 shepherd = params[1]._value;
...
// Obsolete character function.
RuntimeValue
Script_SetPlayerCharacter(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;

	if (charid >= vm->_characters.size())
		error("SetPlayerCharacter: character %d is too high (only have %d)",
		      charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue
Script_AddInventoryToCharacter(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	...
	if (charid >= vm->_characters.size())
		error(
		    "AddInventoryToCharacter: character %d is too high (only have %d)",
		    charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue
Script_LoseInventoryFromCharacter(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	uint item = params[1]._value;

	if (charid >= vm->_characters.size())
		error("LoseInventoryFromCharacter: character %d is too high (only have "
		      "%d)",
		      charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue
Script_AnimateCharacter(AGSEngine *vm, ScriptObject *,
                        const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	...
	if (charid >= vm->_characters.size())
		error("AnimateCharacter: character %d is too high (only have %d)",
		      charid, vm->_characters.size());
...
// import void AnimateCharacterEx (CHARID, int loop, int delay, int repeat, int
// direction, int blocking) Obsolete character function.
RuntimeValue
Script_AnimateCharacterEx(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	uint loop = params[1]._value;
	uint delay = params[2]._value;
	...
	if (charid >= vm->_characters.size())
		error("AnimateCharacterEx: character %d is too high (only have %d)",
		      charid, vm->_characters.size());

	vm->_characters[charid]->animate(loop, delay, repeat, false,
	                                 direction ? 1 : 0);
...
// Obsolete character function.
RuntimeValue Script_MoveCharacter(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	int x = params[1]._signedValue;
	int y = params[2]._signedValue;
	...
	if (charid >= vm->_characters.size())
		error("MoveCharacter: character %d is too high (only have %d)", charid,
		      vm->_characters.size());
...
// Obsolete character function.
RuntimeValue
Script_MoveToWalkableArea(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;

	if (charid >= vm->_characters.size())
		error("MoveToWalkableArea: character %d is too high (only have %d)",
		      charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue Script_FaceCharacter(AGSEngine *vm, ScriptObject *,
                                  const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	uint32 toFace = params[1]._value;

	if (charid >= vm->_characters.size())
		error("FaceCharacter: character %d is too high (only have %d)", charid,
		      vm->_characters.size());
	if (toFace >= vm->_characters.size())
		error("FaceCharacter: target character %d is too high (only have %d)",
		      toFace, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue Script_FaceLocation(AGSEngine *vm, ScriptObject *,
                                 const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	int x = params[1]._signedValue;
	int y = params[2]._signedValue;
...
// Obsolete character function.
RuntimeValue
Script_SetCharacterView(AGSEngine *vm, ScriptObject *,
                        const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	int view = params[1]._signedValue;

	if (charid >= vm->_characters.size())
		error("SetCharacterView: character %d is too high (only have %d)",
		      charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue
Script_SetCharacterViewEx(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	UNUSED(charid);
	int view = params[1]._signedValue;
...
// import void SetCharacterViewOffset(CHARID, int view, int x_offset, int
// y_offset) Obsolete character function.
RuntimeValue
Script_SetCharacterViewOffset(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	UNUSED(charid);
	int view = params[1]._signedValue;
...
// Obsolete character function.
RuntimeValue
Script_SetCharacterFrame(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	int view = params[1]._signedValue;
	int loop = params[2]._signedValue;
	...
	if (charid >= vm->_characters.size())
		error("SetCharacterFrame: character %d is too high (only have %d)",
		      charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue
Script_ReleaseCharacterView(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;

	if (charid >= vm->_characters.size())
		error("ReleaseCharacterView: character %d is too high (only have %d)",
		      charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue
Script_ChangeCharacterView(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	int view = params[1]._signedValue;

	if (charid >= vm->_characters.size())
		error("ChangeCharacterView: character %d is too high (only have %d)",
		      charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue
Script_SetCharacterSpeechView(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	int view = params[1]._signedValue;

	if (charid >= vm->_characters.size())
		error("SetCharacterSpeechView: character %d is too high (only have %d)",
		      charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue
Script_SetCharacterBlinkView(AGSEngine *vm, ScriptObject *,
                             const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	int view = params[1]._signedValue;
...
// Obsolete character function.
RuntimeValue
Script_SetCharacterIdle(AGSEngine *vm, ScriptObject *,
                        const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	...
	if (charid >= vm->_characters.size())
		error("SetCharacterIdle: character %d is too high (only have %d)",
		      charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue Script_StopMoving(AGSEngine *vm, ScriptObject *,
                               const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;

	if (charid >= vm->_characters.size())
		error("StopMoving: character %d is too high (only have %d)", charid,
		      vm->_characters.size());
...
// Obsolete character function.
RuntimeValue
Script_AreCharObjColliding(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	UNUSED(charid);
...
// Obsolete character function.
RuntimeValue
Script_AreCharactersColliding(AGSEngine *vm, ScriptObject *,
                              const Common::Array<RuntimeValue> &params) {
...
// Obsolete character function.
RuntimeValue
Script_SetCharacterSpeed(AGSEngine *vm, ScriptObject *,
                         const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	...
	if (charid >= vm->_characters.size())
		error("SetCharacterSpeed: character %d is too high (only have %d)",
		      charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue
Script_SetCharacterSpeedEx(AGSEngine *vm, ScriptObject *,
                           const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	...
	if (xSpeed == 0 || xSpeed > 50 || ySpeed == 0 || ySpeed > 50)
		error("SetCharacterSpeedEx: invalid speed values %d, %d", xSpeed,
		      ySpeed);

	if (charid >= vm->_characters.size())
		error("SetCharacterSpeedEx: character %d is too high (only have %d)",
		      charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue Script_SetTalkingColor(AGSEngine *vm, ScriptObject *,
                                    const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	uint colour = params[1]._value;

	if (charid >= vm->_characters.size())
		error("SetTalkingColor: character %d is too high (only have %d)",
		      charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue
Script_SetCharacterTransparency(AGSEngine *vm, ScriptObject *,
                                const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	...
	if (charid >= vm->_characters.size())
		error(
		    "SetCharacterTransparency: character %d is too high (only have %d)",
		    charid, vm->_characters.size());

	if (transparency > 100)
		error("SetCharacterTransparency: transparency value must be between 0 "
		      "and 100, but got %d",
		      transparency);
	...
	if (transparency == 0)
		...
		vm->_characters[charid]->_transparency =
		    ((100 - transparency) * 25) / 10;
...
// Obsolete character function.
RuntimeValue
Script_SetCharacterClickable(AGSEngine *vm, ScriptObject *,
                             const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	uint clickable = params[1]._value;

	if (charid >= vm->_characters.size())
		error("SetCharacterClickable: character %d is too high (only have %d)",
		      charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue
Script_SetCharacterBaseline(AGSEngine *vm, ScriptObject *,
                            const Common::Array<RuntimeValue> &params) {
	uint32 charid = params[0]._value;
	int baseline = params[1]._signedValue;

	if (charid >= vm->_characters.size())
		error("SetCharacterBaseline: character %d is too high (only have %d)",
		      charid, vm->_characters.size());
...
// Obsolete character function.
RuntimeValue
Script_GetPlayerCharacter(AGSEngine *vm, ScriptObject *,
                          const Common::Array<RuntimeValue> &params) {
	return vm->_gameFile->_playerChar;
}

// Character: import function AddInventory(InventoryItem *item, int
// addAtIndex=SCR_NO_VALUE) Adds the specified item to the character's
// inventory.
RuntimeValue
Script_Character_AddInventory(AGSEngine *vm, Character *self,
                              const Common::Array<RuntimeValue> &params) {
	if (!params[0]._object->isOfType(sotInventoryItem))
		error("Character::AddInventory got incorrect object type (expected a "
...
// Character: import function Animate(int loop, int delay, RepeatStyle=eOnce,
// BlockingStyle=eBlock, Direction=eForwards) Animates the character using its
// current locked view.
RuntimeValue
Script_Character_Animate(AGSEngine *vm, Character *self,
                         const Common::Array<RuntimeValue> &params) {
...
// Character: import function ChangeRoom(int room, int x=SCR_NO_VALUE, int
// y=SCR_NO_VALUE) Moves the character to another room. If this is the player
// character, the game will also switch to that room.
RuntimeValue
Script_Character_ChangeRoom(AGSEngine *vm, Character *self,
                            const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_Character_ChangeRoomAutoPosition(
...
RuntimeValue
Script_Character_ChangeView(AGSEngine *vm, Character *self,
                            const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Character_FaceCharacter(AGSEngine *vm, Character *self,
                               const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Character_FaceLocation(AGSEngine *vm, Character *self,
                              const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;
...
RuntimeValue
Script_Character_FaceObject(AGSEngine *vm, Character *self,
                            const Common::Array<RuntimeValue> &params) {
	if (!params[0]._object->isOfType(sotRoomObject))
		error("Character::FaceObject got incorrect object type (expected a "
		      "RoomObject) for parameter 1");
...
// Character: import function FollowCharacter(Character*, int dist=10, int
// eagerness=97) Starts this character following the other character.
RuntimeValue
...
RuntimeValue
Script_Character_GetProperty(AGSEngine *vm, Character *self,
                             const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Character_HasInventory(AGSEngine *vm, Character *self,
                              const Common::Array<RuntimeValue> &params) {
	if (!params[0]._object->isOfType(sotInventoryItem))
...
RuntimeValue Script_Character_IsCollidingWithChar(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
	if (!params[0]._object->isOfType(sotCharacter))
		error("Character::IsCollidingWithChar got incorrect object type "
		      "(expected a Character) for parameter 1");
	Character *character = (Character *) params[0]._object;
	UNUSED(character);
...
RuntimeValue Script_Character_IsCollidingWithObject(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
	if (!params[0]._object->isOfType(sotRoomObject))
		error("Character::IsCollidingWithObject got incorrect object type "
		      "(expected a RoomObject) for parameter 1");
	RoomObject *object = (RoomObject *) params[0]._object;
	UNUSED(object);
...
RuntimeValue
Script_Character_LockView(AGSEngine *vm, Character *self,
                          const Common::Array<RuntimeValue> &params) {
...
// Locks the character to this view, and aligns it against one side of the
// existing sprite.
RuntimeValue
Script_Character_LockViewAligned(AGSEngine *vm, Character *self,
                                 const Common::Array<RuntimeValue> &params) {
	int view = params[0]._signedValue;
	UNUSED(view);
...
RuntimeValue
Script_Character_LockViewFrame(AGSEngine *vm, Character *self,
                               const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Character_LockViewOffset(AGSEngine *vm, Character *self,
                                const Common::Array<RuntimeValue> &params) {
	uint view = params[0]._value;
...
RuntimeValue
Script_Character_LoseInventory(AGSEngine *vm, Character *self,
                               const Common::Array<RuntimeValue> &params) {
	if (!params[0]._object->isOfType(sotInventoryItem))
		error("Character::LoseInventory got incorrect object type (expected a "
		      "InventoryItem) for parameter 1");
	InventoryItem *item = (InventoryItem *) params[0]._object;
...
// Character: import function Move(int x, int y, BlockingStyle=eNoBlock,
// WalkWhere=eWalkableAreas) Moves the character to the destination, without
// playing his walking animation.
RuntimeValue Script_Character_Move(AGSEngine *vm, Character *self,
                                   const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	UNUSED(x);
	int y = params[1]._signedValue;
...
RuntimeValue Script_Character_PlaceOnWalkableArea(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_Character_Say(AGSEngine *vm, Character *self,
                                  const Common::Array<RuntimeValue> &params) {
...
// Character: import function SayAt(int x, int y, int width, const string
// message) Says the specified text at the specified position on the screen
// using the character's speech settings.
RuntimeValue Script_Character_SayAt(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;
	int width = params[2]._signedValue;
...
RuntimeValue
Script_Character_SayBackground(AGSEngine *vm, Character *self,
                               const Common::Array<RuntimeValue> &params) {
	ScriptString *message = (ScriptString *) params[0]._object;

	uint overlayType =
	    vm->displaySpeechBackground(self->_indexId, message->getString());
...
RuntimeValue
Script_Character_SetAsPlayer(AGSEngine *vm, Character *self,
                             const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_Character_Think(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
	ScriptString *message = (ScriptString *) params[0]._object;
	UNUSED(message);
...
// Character: import void Tint(int red, int green, int blue, int saturation, int
// luminance) Tints the character to the specified colour.
RuntimeValue Script_Character_Tint(AGSEngine *vm, Character *self,
                                   const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Character_UnlockView(AGSEngine *vm, Character *self,
                            const Common::Array<RuntimeValue> &params) {
	self->unlockView();
...
// Character: import function Walk(int x, int y, BlockingStyle=eNoBlock,
// WalkWhere=eWalkableAreas) Moves the character to the destination,
// automatically playing his walking animation.
RuntimeValue Script_Character_Walk(AGSEngine *vm, Character *self,
                                   const Common::Array<RuntimeValue> &params) {
	int x = params[0]._signedValue;
	int y = params[1]._signedValue;
	uint32 blockingStyle = params[2]._value;
	...
	if (walkWhere == ANYWHERE)
		...
		error("Character::Walk: %d is not a valid value for WalkWhere",
		      walkWhere);
	...
	if (blockingStyle == BLOCKING || blockingStyle == 1)
		...
		error("Character::Walk: %d is not a valid value for BlockingStyle",
		      blockingStyle);
...
// Moves the character in a straight line as far as possible towards the
// co-ordinates. Useful for keyboard movement.
RuntimeValue
...
RuntimeValue Script_Character_get_ActiveInventory(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Character_get_AnimationSpeed(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
...
// Gets/sets the character's animation speed.
RuntimeValue
Script_Character_set_AnimationSpeed(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
...
// Gets/sets a specific baseline for the character. 0 means character's Y-pos
// will be used.
RuntimeValue
Script_Character_get_Baseline(AGSEngine *vm, Character *self,
                              const Common::Array<RuntimeValue> &params) {
	if (self->_baseline < 1)
		return 0;
...
// will be used.
RuntimeValue
Script_Character_set_Baseline(AGSEngine *vm, Character *self,
                              const Common::Array<RuntimeValue> &params) {
...
// Gets/sets the interval at which the character will blink while talking, in
// game loops.
RuntimeValue
Script_Character_get_BlinkInterval(AGSEngine *vm, Character *self,
                                   const Common::Array<RuntimeValue> &params) {
...
// game loops.
RuntimeValue
Script_Character_set_BlinkInterval(AGSEngine *vm, Character *self,
                                   const Common::Array<RuntimeValue> &params) {
...
// Gets/sets the view used for the character's blinking animation. -1 to
// disable.
RuntimeValue
Script_Character_get_BlinkView(AGSEngine *vm, Character *self,
                               const Common::Array<RuntimeValue> &params) {
...
// disable.
RuntimeValue
Script_Character_set_BlinkView(AGSEngine *vm, Character *self,
                               const Common::Array<RuntimeValue> &params) {
	int value = params[0]._signedValue;
...
RuntimeValue Script_Character_get_BlinkWhileThinking(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
...
// Character: import attribute bool BlinkWhileThinking
// Gets/sets whether the character will blink while thinking as well as talking.
RuntimeValue Script_Character_set_BlinkWhileThinking(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
...
// Allows you to manually specify the height of the blocking area at the
// character's feet.
RuntimeValue
Script_Character_get_BlockingHeight(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
...
// character's feet.
RuntimeValue
Script_Character_set_BlockingHeight(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
	uint value = params[0]._value;
...
// Allows you to manually specify the width of the blocking area at the
// character's feet.
RuntimeValue
Script_Character_get_BlockingWidth(AGSEngine *vm, Character *self,
                                   const Common::Array<RuntimeValue> &params) {
...
// character's feet.
RuntimeValue
Script_Character_set_BlockingWidth(AGSEngine *vm, Character *self,
                                   const Common::Array<RuntimeValue> &params) {
	uint value = params[0]._value;
...
// Gets/sets whether the mouse can be clicked on the character, or whether it
// passes straight through.
RuntimeValue
Script_Character_get_Clickable(AGSEngine *vm, Character *self,
                               const Common::Array<RuntimeValue> &params) {
...
// passes straight through.
RuntimeValue
Script_Character_set_Clickable(AGSEngine *vm, Character *self,
                               const Common::Array<RuntimeValue> &params) {
	uint32 value = params[0]._value;
...
// Gets/sets whether this character has an 8-loop walking view with diagonal
// loops.
RuntimeValue
Script_Character_get_DiagonalLoops(AGSEngine *vm, Character *self,
                                   const Common::Array<RuntimeValue> &params) {
...
// loops.
RuntimeValue
Script_Character_set_DiagonalLoops(AGSEngine *vm, Character *self,
                                   const Common::Array<RuntimeValue> &params) {
	uint32 value = params[0]._value;
...
RuntimeValue
Script_Character_get_Frame(AGSEngine *vm, Character *self,
                           const Common::Array<RuntimeValue> &params) {
	return self->_frame;
}
...
// Gets/sets the character's current frame number within its current view.
RuntimeValue
Script_Character_set_Frame(AGSEngine *vm, Character *self,
                           const Common::Array<RuntimeValue> &params) {
	int value = params[0]._signedValue;
...
RuntimeValue
Script_Character_get_ID(AGSEngine *vm, Character *self,
                        const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Character_get_IdleView(AGSEngine *vm, Character *self,
                              const Common::Array<RuntimeValue> &params) {
	if (self->_idleView < 1)
		return -1;
...
RuntimeValue
Script_Character_get_IgnoreLighting(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
...
// Gets/sets whether the character ignores region tints and lighting.
RuntimeValue
Script_Character_set_IgnoreLighting(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
	uint32 value = params[0]._value;
...
// Undocumented.
RuntimeValue
Script_Character_get_IgnoreScaling(AGSEngine *vm, Character *self,
                                   const Common::Array<RuntimeValue> &params) {
...
// Undocumented.
RuntimeValue
Script_Character_set_IgnoreScaling(AGSEngine *vm, Character *self,
                                   const Common::Array<RuntimeValue> &params) {
	uint32 value = params[0]._value;
...
// Gets/sets whether the character ignores walk-behind areas and is always
// placed on top.
RuntimeValue Script_Character_get_IgnoreWalkbehinds(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
...
// Gets/sets whether the character ignores walk-behind areas and is always
// placed on top.
RuntimeValue Script_Character_set_IgnoreWalkbehinds(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
	uint32 value = params[0]._value;
...
RuntimeValue Script_Character_geti_InventoryQuantity(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
	uint index = params[0]._value;

	if (index < 1 || index >= vm->_gameFile->_invItemInfo.size())
		error("Character::geti_InventoryQuantity: invalid inventory item index "
		      "%d (only %d items)",
...
// Character: import attribute int InventoryQuantity[]
// Accesses the number of each inventory item that the character currently has.
RuntimeValue Script_Character_seti_InventoryQuantity(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
	uint index = params[0]._value;
	uint value = params[1]._value;

	if (index < 1 || index >= vm->_gameFile->_invItemInfo.size())
		error("Character::seti_InventoryQuantity: invalid inventory item index "
		      "%d (only %d items)",
		      index, vm->_gameFile->_invItemInfo.size());
...
// Gets/sets the character's current loop number within its current view.
RuntimeValue
Script_Character_set_Loop(AGSEngine *vm, Character *self,
                          const Common::Array<RuntimeValue> &params) {
	uint value = params[0]._value;

	if (value >= vm->_gameFile->_views[self->_view]._loops.size())
		error("Character::set_Loop: loop %d is invalid for view %d, which only "
		      "has %d loops",
		      value, self->_view,
		      vm->_gameFile->_views[self->_view]._loops.size());

	self->_loop = value;

	if (self->_frame >=
	    vm->_gameFile->_views[self->_view]._loops[self->_loop]._frames.size())
		self->_frame = 0;
...
// Gets/sets whether the character uses manually specified scaling instead of
// using walkable area scaling.
RuntimeValue
Script_Character_get_ManualScaling(AGSEngine *vm, Character *self,
                                   const Common::Array<RuntimeValue> &params) {
	return (self->_flags & CHF_MANUALSCALING) ? 1 : 0;
}
...
// using walkable area scaling.
RuntimeValue
Script_Character_set_ManualScaling(AGSEngine *vm, Character *self,
                                   const Common::Array<RuntimeValue> &params) {
	uint32 value = params[0]._value;
...
// Gets/sets whether the character only moves when their animation frame
// changes.
RuntimeValue Script_Character_get_MovementLinkedToAnimation(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
...
// Gets/sets whether the character only moves when their animation frame
// changes.
RuntimeValue Script_Character_set_MovementLinkedToAnimation(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
	uint32 value = params[0]._value;
...
RuntimeValue
Script_Character_get_Name(AGSEngine *vm, Character *self,
                          const Common::Array<RuntimeValue> &params) {
...
// Gets/sets the character's name.
RuntimeValue
Script_Character_set_Name(AGSEngine *vm, Character *self,
                          const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Character_get_NormalView(AGSEngine *vm, Character *self,
                                const Common::Array<RuntimeValue> &params) {
...
// Gets/sets whether the character's movement speed is adjusted in line with its
// scaling level.
RuntimeValue
Script_Character_get_ScaleMoveSpeed(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
...
// scaling level.
RuntimeValue
Script_Character_set_ScaleMoveSpeed(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
	uint adjustSpeed = params[0]._value;

	if (adjustSpeed > 1)
		error("Character::set_ScaleMoveSpeed: value must be true or false (1 "
		      "or 0), but got %d",
		      adjustSpeed);
...
// Gets/sets whether the volume of frame-linked sounds for the character are
// adjusted in line with its scaling level.
RuntimeValue
Script_Character_get_ScaleVolume(AGSEngine *vm, Character *self,
                                 const Common::Array<RuntimeValue> &params) {
...
// adjusted in line with its scaling level.
RuntimeValue
Script_Character_set_ScaleVolume(AGSEngine *vm, Character *self,
                                 const Common::Array<RuntimeValue> &params) {
	uint32 value = params[0]._value;
...
RuntimeValue
Script_Character_get_Scaling(AGSEngine *vm, Character *self,
                             const Common::Array<RuntimeValue> &params) {
	return self->_zoom;
}
...
// Gets/sets the character's current scaling level.
RuntimeValue
Script_Character_set_Scaling(AGSEngine *vm, Character *self,
                             const Common::Array<RuntimeValue> &params) {
	uint value = params[0]._value;

	if (!(self->_flags & CHF_MANUALSCALING))
		error("Character::set_Scaling: scaling can only be changed on manually "
		      "scaled characters");
	if (value < 5 || value > 200)
		error("Character::set_Scaling: scaling level must be from 5-200%%, but "
		      "got %d%%",
		      value);
...
// Gets/sets whether this character blocks other objects and characters from
// moving through it.
RuntimeValue
Script_Character_get_Solid(AGSEngine *vm, Character *self,
                           const Common::Array<RuntimeValue> &params) {
...
// moving through it.
RuntimeValue
Script_Character_set_Solid(AGSEngine *vm, Character *self,
                           const Common::Array<RuntimeValue> &params) {
	uint32 value = params[0]._value;
...
RuntimeValue
Script_Character_get_Speaking(AGSEngine *vm, Character *self,
                              const Common::Array<RuntimeValue> &params) {
	// FIXME
...
// Gets the current frame of the character's speaking animation (only valid when
// Speaking is true)
RuntimeValue
Script_Character_get_SpeakingFrame(AGSEngine *vm, Character *self,
                                   const Common::Array<RuntimeValue> &params) {
	// FIXME
...
RuntimeValue Script_Character_get_SpeechAnimationDelay(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
...
// Character: import attribute int SpeechAnimationDelay
// Gets/sets the character's speech animation delay.
RuntimeValue Script_Character_set_SpeechAnimationDelay(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
	uint value = params[0]._value;

	if (vm->getGameOption(OPT_OLDTALKANIMSPD))
		error("Character::set_SpeechAnimationDelay: can't set speech animation "
		      "delay when legacy speech animation speed is enabled");
...
RuntimeValue
Script_Character_get_SpeechColor(AGSEngine *vm, Character *self,
                                 const Common::Array<RuntimeValue> &params) {
...
// Gets/sets the character's speech text colour.
RuntimeValue
Script_Character_set_SpeechColor(AGSEngine *vm, Character *self,
                                 const Common::Array<RuntimeValue> &params) {
	uint value = params[0]._value;
...
// Gets/sets the character's speech view.
RuntimeValue
Script_Character_set_SpeechView(AGSEngine *vm, Character *self,
                                const Common::Array<RuntimeValue> &params) {
	int value = params[0]._signedValue;
...
RuntimeValue
Script_Character_get_ThinkView(AGSEngine *vm, Character *self,
                               const Common::Array<RuntimeValue> &params) {
...
// Gets/sets the character's thinking view.
RuntimeValue
Script_Character_set_ThinkView(AGSEngine *vm, Character *self,
                               const Common::Array<RuntimeValue> &params) {
	int value = params[0]._signedValue;
...
RuntimeValue
Script_Character_get_Transparency(AGSEngine *vm, Character *self,
                                  const Common::Array<RuntimeValue> &params) {
...
// Gets/sets the character's current transparency level.
RuntimeValue
Script_Character_set_Transparency(AGSEngine *vm, Character *self,
                                  const Common::Array<RuntimeValue> &params) {
	uint trans = params[0]._value;

	if (trans > 100)
		error("Character::set_Transparency: transparency value must be between "
		      "0 and 100, but got %d",
		      trans);
...
// Gets/sets whether the character turns on the spot to face the correct
// direction before walking.
RuntimeValue Script_Character_get_TurnBeforeWalking(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
...
// Gets/sets whether the character turns on the spot to face the correct
// direction before walking.
RuntimeValue Script_Character_set_TurnBeforeWalking(
    AGSEngine *vm, Character *self, const Common::Array<RuntimeValue> &params) {
	uint32 value = params[0]._value;
...
RuntimeValue
Script_Character_get_WalkSpeedX(AGSEngine *vm, Character *self,
                                const Common::Array<RuntimeValue> &params) {
...
RuntimeValue
Script_Character_get_WalkSpeedY(AGSEngine *vm, Character *self,
                                const Common::Array<RuntimeValue> &params) {
...
RuntimeValue Script_Character_get_x(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
	return self->_x;
}
...
// Character: import attribute int x
// The character's current X-position.
RuntimeValue Script_Character_set_x(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
	int value = params[0]._signedValue;
...
RuntimeValue Script_Character_get_y(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
	return self->_y;
}
...
// Character: import attribute int y
// The character's current Y-position.
RuntimeValue Script_Character_set_y(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
	int value = params[0]._signedValue;
...
RuntimeValue Script_Character_get_z(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
	return self->_z;
}
...
// Character: import attribute int z
// The character's current Z-position.
RuntimeValue Script_Character_set_z(AGSEngine *vm, Character *self,
                                    const Common::Array<RuntimeValue> &params) {
	int value = params[0]._signedValue;
...
static const ScriptSystemFunctionInfo ourFunctionList[] = {
    {"GetCharacterAt", (ScriptAPIFunction *) &Script_GetCharacterAt, "ii",
     sotNone},
    {"AddInventory", (ScriptAPIFunction *) &Script_AddInventory, "i", sotNone},
    {"LoseInventory", (ScriptAPIFunction *) &Script_LoseInventory, "i",
     sotNone},
    {"SetActiveInventory", (ScriptAPIFunction *) &Script_SetActiveInventory,
     "i", sotNone},
    ...
    {"GetCharacterProperty", (ScriptAPIFunction *) &Script_GetCharacterProperty,
     "is", sotNone},
    {"GetCharacterPropertyText",
     (ScriptAPIFunction *) &Script_GetCharacterPropertyText, "iss", sotNone},
    {"RunCharacterInteraction",
     (ScriptAPIFunction *) &Script_RunCharacterInteraction, "ii", sotNone},
    {"DisplaySpeech", (ScriptAPIFunction *) &Script_DisplaySpeech, "is.",
     sotNone},
    {"DisplaySpeechBackground",
     (ScriptAPIFunction *) &Script_DisplaySpeechBackground, "is", sotNone},
    {"DisplaySpeechAt", (ScriptAPIFunction *) &Script_DisplaySpeechAt, "iiiis",
     sotNone},
    {"DisplayThought", (ScriptAPIFunction *) &Script_DisplayThought, "is",
     sotNone},
    {"FollowCharacter", (ScriptAPIFunction *) &Script_FollowCharacter, "ii",
     sotNone},
    {"FollowCharacterEx", (ScriptAPIFunction *) &Script_FollowCharacterEx,
     "iiii", sotNone},
    {"SetPlayerCharacter", (ScriptAPIFunction *) &Script_SetPlayerCharacter,
     "i", sotNone},
    {"AddInventoryToCharacter",
     (ScriptAPIFunction *) &Script_AddInventoryToCharacter, "ii", sotNone},
    {"LoseInventoryFromCharacter",
     (ScriptAPIFunction *) &Script_LoseInventoryFromCharacter, "ii", sotNone},
    {"AnimateCharacter", (ScriptAPIFunction *) &Script_AnimateCharacter, "iiii",
     sotNone},
    {"AnimateCharacterEx", (ScriptAPIFunction *) &Script_AnimateCharacterEx,
     "iiiiii", sotNone},
    {"MoveCharacter", (ScriptAPIFunction *) &Script_MoveCharacter, "iii",
     sotNone},
    {"MoveCharacterDirect", (ScriptAPIFunction *) &Script_MoveCharacterDirect,
     "iii", sotNone},
    {"MoveCharacterPath", (ScriptAPIFunction *) &Script_MoveCharacterPath,
     "iii", sotNone},
    {"MoveCharacterStraight",
     (ScriptAPIFunction *) &Script_MoveCharacterStraight, "iii", sotNone},
    {"MoveCharacterToHotspot",
     (ScriptAPIFunction *) &Script_MoveCharacterToHotspot, "ii", sotNone},
    {"MoveCharacterToObject",
     (ScriptAPIFunction *) &Script_MoveCharacterToObject, "ii", sotNone},
    {"MoveCharacterBlocking",
     (ScriptAPIFunction *) &Script_MoveCharacterBlocking, "iiii", sotNone},
    {"MoveToWalkableArea", (ScriptAPIFunction *) &Script_MoveToWalkableArea,
     "i", sotNone},
    {"FaceCharacter", (ScriptAPIFunction *) &Script_FaceCharacter, "ii",
     sotNone},
    {"FaceLocation", (ScriptAPIFunction *) &Script_FaceLocation, "iii",
     sotNone},
    {"SetCharacterView", (ScriptAPIFunction *) &Script_SetCharacterView, "ii",
     sotNone},
    {"SetCharacterViewEx", (ScriptAPIFunction *) &Script_SetCharacterViewEx,
     "iiii", sotNone},
    {"SetCharacterViewOffset",
     (ScriptAPIFunction *) &Script_SetCharacterViewOffset, "iiii", sotNone},
    {"SetCharacterFrame", (ScriptAPIFunction *) &Script_SetCharacterFrame,
     "iiii", sotNone},
    {"ReleaseCharacterView", (ScriptAPIFunction *) &Script_ReleaseCharacterView,
     "i", sotNone},
    {"ChangeCharacterView", (ScriptAPIFunction *) &Script_ChangeCharacterView,
     "ii", sotNone},
    {"SetCharacterSpeechView",
     (ScriptAPIFunction *) &Script_SetCharacterSpeechView, "ii", sotNone},
    {"SetCharacterBlinkView",
     (ScriptAPIFunction *) &Script_SetCharacterBlinkView, "iii", sotNone},
    {"SetCharacterIdle", (ScriptAPIFunction *) &Script_SetCharacterIdle, "iii",
     sotNone},
    {"StopMoving", (ScriptAPIFunction *) &Script_StopMoving, "i", sotNone},
    {"AreCharObjColliding", (ScriptAPIFunction *) &Script_AreCharObjColliding,
     "ii", sotNone},
    {"AreCharactersColliding",
     (ScriptAPIFunction *) &Script_AreCharactersColliding, "ii", sotNone},
    {"SetCharacterSpeed", (ScriptAPIFunction *) &Script_SetCharacterSpeed, "ii",
     sotNone},
    {"SetCharacterSpeedEx", (ScriptAPIFunction *) &Script_SetCharacterSpeedEx,
     "iii", sotNone},
    {"SetTalkingColor", (ScriptAPIFunction *) &Script_SetTalkingColor, "ii",
     sotNone},
    {"SetCharacterTransparency",
     (ScriptAPIFunction *) &Script_SetCharacterTransparency, "ii", sotNone},
    {"SetCharacterClickable",
     (ScriptAPIFunction *) &Script_SetCharacterClickable, "ii", sotNone},
    {"SetCharacterBaseline", (ScriptAPIFunction *) &Script_SetCharacterBaseline,
     "ii", sotNone},
    {"SetCharacterIgnoreLight",
     (ScriptAPIFunction *) &Script_SetCharacterIgnoreLight, "ii", sotNone},
    {"SetCharacterIgnoreWalkbehinds",
     (ScriptAPIFunction *) &Script_SetCharacterIgnoreWalkbehinds, "ii",
     sotNone},
    {"SetCharacterProperty", (ScriptAPIFunction *) &Script_SetCharacterProperty,
     "iii", sotNone},
    {"GetPlayerCharacter", (ScriptAPIFunction *) &Script_GetPlayerCharacter, "",
     sotNone},
    {"Character::AddInventory^2",
     (ScriptAPIFunction *) &Script_Character_AddInventory, "oi", sotCharacter},
    {"Character::AddWaypoint^2",
     (ScriptAPIFunction *) &Script_Character_AddWaypoint, "ii", sotCharacter},
    {"Character::Animate^5", (ScriptAPIFunction *) &Script_Character_Animate,
     "iiiii", sotCharacter},
    {"Character::ChangeRoom^3",
     (ScriptAPIFunction *) &Script_Character_ChangeRoom, "iii", sotCharacter},
    {"Character::ChangeRoomAutoPosition^2",
     (ScriptAPIFunction *) &Script_Character_ChangeRoomAutoPosition, "ii",
     sotCharacter},
    {"Character::ChangeView^1",
     (ScriptAPIFunction *) &Script_Character_ChangeView, "i", sotCharacter},
    {"Character::FaceCharacter^2",
     (ScriptAPIFunction *) &Script_Character_FaceCharacter, "oi", sotCharacter},
    {"Character::FaceLocation^3",
     (ScriptAPIFunction *) &Script_Character_FaceLocation, "iii", sotCharacter},
    ...
    {"Character::GetProperty^1",
     (ScriptAPIFunction *) &Script_Character_GetProperty, "s", sotCharacter},
    {"Character::GetPropertyText^2",
     (ScriptAPIFunction *) &Script_Character_GetPropertyText, "ss",
     sotCharacter},
    {"Character::GetTextProperty^1",
     (ScriptAPIFunction *) &Script_Character_GetTextProperty, "s",
     sotCharacter},
    {"Character::HasInventory^1",
     (ScriptAPIFunction *) &Script_Character_HasInventory, "o", sotCharacter},
    {"Character::IsCollidingWithChar^1",
     (ScriptAPIFunction *) &Script_Character_IsCollidingWithChar, "o",
     sotCharacter},
    {"Character::IsCollidingWithObject^1",
     (ScriptAPIFunction *) &Script_Character_IsCollidingWithObject, "o",
     sotCharacter},
    {"Character::LockView^1", (ScriptAPIFunction *) &Script_Character_LockView,
     "i", sotCharacter},
    {"Character::LockViewAligned^3",
     (ScriptAPIFunction *) &Script_Character_LockViewAligned, "iii",
     sotCharacter},
    {"Character::LockViewFrame^3",
     (ScriptAPIFunction *) &Script_Character_LockViewFrame, "iii",
     sotCharacter},
    {"Character::LockViewOffset^3",
     (ScriptAPIFunction *) &Script_Character_LockViewOffset, "iii",
     sotCharacter},
    {"Character::LoseInventory^1",
     (ScriptAPIFunction *) &Script_Character_LoseInventory, "o", sotCharacter},
    {"Character::Move^4", (ScriptAPIFunction *) &Script_Character_Move, "iiii",
     sotCharacter},
    {"Character::PlaceOnWalkableArea^0",
     (ScriptAPIFunction *) &Script_Character_PlaceOnWalkableArea, "",
     sotCharacter},
    {"Character::RemoveTint^0",
     (ScriptAPIFunction *) &Script_Character_RemoveTint, "", sotCharacter},
    {"Character::RunInteraction^1",
     (ScriptAPIFunction *) &Script_Character_RunInteraction, "i", sotCharacter},
    {"Character::Say^101", (ScriptAPIFunction *) &Script_Character_Say, "s.",
     sotCharacter},
    {"Character::SayAt^4", (ScriptAPIFunction *) &Script_Character_SayAt,
     "iiis", sotCharacter},
    {"Character::SayBackground^1",
     (ScriptAPIFunction *) &Script_Character_SayBackground, "s", sotCharacter},
    {"Character::SetAsPlayer^0",
     (ScriptAPIFunction *) &Script_Character_SetAsPlayer, "", sotCharacter},
    {"Character::SetIdleView^2",
     (ScriptAPIFunction *) &Script_Character_SetIdleView, "ii", sotCharacter},
    {"Character::SetWalkSpeed^2",
     (ScriptAPIFunction *) &Script_Character_SetWalkSpeed, "ii", sotCharacter},
    {"Character::StopMoving^0",
     (ScriptAPIFunction *) &Script_Character_StopMoving, "", sotCharacter},
    {"Character::Think^101", (ScriptAPIFunction *) &Script_Character_Think, "s",
     sotCharacter},
    {"Character::Tint^5", (ScriptAPIFunction *) &Script_Character_Tint, "iiiii",
     sotCharacter},
    {"Character::UnlockView^0",
     (ScriptAPIFunction *) &Script_Character_UnlockView, "", sotCharacter},
    {"Character::Walk^4", (ScriptAPIFunction *) &Script_Character_Walk, "iiii",
     sotCharacter},
    {"Character::WalkStraight^3",
     (ScriptAPIFunction *) &Script_Character_WalkStraight, "iii", sotCharacter},
    {"Character::get_ActiveInventory",
    ...
    {"Character::get_AnimationSpeed",
     (ScriptAPIFunction *) &Script_Character_get_AnimationSpeed, "",
     sotCharacter},
    {"Character::set_AnimationSpeed",
     (ScriptAPIFunction *) &Script_Character_set_AnimationSpeed, "i",
     sotCharacter},
    {"Character::get_Baseline",
     (ScriptAPIFunction *) &Script_Character_get_Baseline, "", sotCharacter},
    {"Character::set_Baseline",
     (ScriptAPIFunction *) &Script_Character_set_Baseline, "i", sotCharacter},
    {"Character::get_BlinkInterval",
     (ScriptAPIFunction *) &Script_Character_get_BlinkInterval, "",
     sotCharacter},
    {"Character::set_BlinkInterval",
     (ScriptAPIFunction *) &Script_Character_set_BlinkInterval, "i",
     sotCharacter},
    {"Character::get_BlinkView",
     (ScriptAPIFunction *) &Script_Character_get_BlinkView, "", sotCharacter},
    {"Character::set_BlinkView",
     (ScriptAPIFunction *) &Script_Character_set_BlinkView, "i", sotCharacter},
    {"Character::get_BlinkWhileThinking",
     (ScriptAPIFunction *) &Script_Character_get_BlinkWhileThinking, "",
     sotCharacter},
    {"Character::set_BlinkWhileThinking",
     (ScriptAPIFunction *) &Script_Character_set_BlinkWhileThinking, "i",
     sotCharacter},
    {"Character::get_BlockingHeight",
     (ScriptAPIFunction *) &Script_Character_get_BlockingHeight, "",
     sotCharacter},
    {"Character::set_BlockingHeight",
     (ScriptAPIFunction *) &Script_Character_set_BlockingHeight, "i",
     sotCharacter},
    {"Character::get_BlockingWidth",
     (ScriptAPIFunction *) &Script_Character_get_BlockingWidth, "",
     sotCharacter},
    {"Character::set_BlockingWidth",
     (ScriptAPIFunction *) &Script_Character_set_BlockingWidth, "i",
     sotCharacter},
    {"Character::get_Clickable",
     (ScriptAPIFunction *) &Script_Character_get_Clickable, "", sotCharacter},
    {"Character::set_Clickable",
     (ScriptAPIFunction *) &Script_Character_set_Clickable, "i", sotCharacter},
    {"Character::get_DiagonalLoops",
     (ScriptAPIFunction *) &Script_Character_get_DiagonalLoops, "",
     sotCharacter},
    {"Character::set_DiagonalLoops",
     (ScriptAPIFunction *) &Script_Character_set_DiagonalLoops, "i",
     sotCharacter},
    {"Character::get_Frame", (ScriptAPIFunction *) &Script_Character_get_Frame,
     "", sotCharacter},
    {"Character::set_Frame", (ScriptAPIFunction *) &Script_Character_set_Frame,
     "i", sotCharacter},
    {"Character::get_HasExplicitTint",
     (ScriptAPIFunction *) &Script_Character_get_HasExplicitTint, "",
     sotCharacter},
    {"Character::get_ID", (ScriptAPIFunction *) &Script_Character_get_ID, "",
     sotCharacter},
    {"Character::get_IdleView",
     (ScriptAPIFunction *) &Script_Character_get_IdleView, "", sotCharacter},
    {"Character::get_IgnoreLighting",
     (ScriptAPIFunction *) &Script_Character_get_IgnoreLighting, "",
     sotCharacter},
    {"Character::set_IgnoreLighting",
     (ScriptAPIFunction *) &Script_Character_set_IgnoreLighting, "i",
     sotCharacter},
    {"Character::get_IgnoreScaling",
     (ScriptAPIFunction *) &Script_Character_get_IgnoreScaling, "",
     sotCharacter},
    {"Character::set_IgnoreScaling",
     (ScriptAPIFunction *) &Script_Character_set_IgnoreScaling, "i",
     sotCharacter},
    {"Character::get_IgnoreWalkbehinds",
     (ScriptAPIFunction *) &Script_Character_get_IgnoreWalkbehinds, "",
     sotCharacter},
    {"Character::set_IgnoreWalkbehinds",
     (ScriptAPIFunction *) &Script_Character_set_IgnoreWalkbehinds, "i",
     sotCharacter},
    {"Character::geti_InventoryQuantity",
     (ScriptAPIFunction *) &Script_Character_geti_InventoryQuantity, "i",
     sotCharacter},
    {"Character::seti_InventoryQuantity",
     (ScriptAPIFunction *) &Script_Character_seti_InventoryQuantity, "ii",
     sotCharacter},
    {"Character::get_Loop", (ScriptAPIFunction *) &Script_Character_get_Loop,
     "", sotCharacter},
    {"Character::set_Loop", (ScriptAPIFunction *) &Script_Character_set_Loop,
     "i", sotCharacter},
    {"Character::get_ManualScaling",
     (ScriptAPIFunction *) &Script_Character_get_ManualScaling, "",
     sotCharacter},
    {"Character::set_ManualScaling",
     (ScriptAPIFunction *) &Script_Character_set_ManualScaling, "i",
     sotCharacter},
    {"Character::get_MovementLinkedToAnimation",
     (ScriptAPIFunction *) &Script_Character_get_MovementLinkedToAnimation, "",
     sotCharacter},
    {"Character::set_MovementLinkedToAnimation",
     (ScriptAPIFunction *) &Script_Character_set_MovementLinkedToAnimation, "i",
     sotCharacter},
    {"Character::get_Moving",
     (ScriptAPIFunction *) &Script_Character_get_Moving, "", sotCharacter},
    {"Character::get_Name", (ScriptAPIFunction *) &Script_Character_get_Name,
     "", sotCharacter},
    {"Character::set_Name", (ScriptAPIFunction *) &Script_Character_set_Name,
     "s", sotCharacter},
    {"Character::get_NormalView",
     (ScriptAPIFunction *) &Script_Character_get_NormalView, "", sotCharacter},
    {"Character::get_PreviousRoom",
     (ScriptAPIFunction *) &Script_Character_get_PreviousRoom, "",
     sotCharacter},
    {"Character::get_Room", (ScriptAPIFunction *) &Script_Character_get_Room,
     "", sotCharacter},
    {"Character::get_ScaleMoveSpeed",
     (ScriptAPIFunction *) &Script_Character_get_ScaleMoveSpeed, "",
     sotCharacter},
    {"Character::set_ScaleMoveSpeed",
     (ScriptAPIFunction *) &Script_Character_set_ScaleMoveSpeed, "i",
     sotCharacter},
    {"Character::get_ScaleVolume",
     (ScriptAPIFunction *) &Script_Character_get_ScaleVolume, "", sotCharacter},
    {"Character::set_ScaleVolume",
     (ScriptAPIFunction *) &Script_Character_set_ScaleVolume, "i",
     sotCharacter},
    {"Character::get_Scaling",
     (ScriptAPIFunction *) &Script_Character_get_Scaling, "", sotCharacter},
    {"Character::set_Scaling",
     (ScriptAPIFunction *) &Script_Character_set_Scaling, "i", sotCharacter},
    {"Character::get_Solid", (ScriptAPIFunction *) &Script_Character_get_Solid,
     "", sotCharacter},
    {"Character::set_Solid", (ScriptAPIFunction *) &Script_Character_set_Solid,
     "i", sotCharacter},
    {"Character::get_Speaking",
     (ScriptAPIFunction *) &Script_Character_get_Speaking, "", sotCharacter},
    {"Character::get_SpeakingFrame",
     (ScriptAPIFunction *) &Script_Character_get_SpeakingFrame, "",
     sotCharacter},
    {"Character::get_SpeechAnimationDelay",
     (ScriptAPIFunction *) &Script_Character_get_SpeechAnimationDelay, "",
     sotCharacter},
    {"Character::set_SpeechAnimationDelay",
     (ScriptAPIFunction *) &Script_Character_set_SpeechAnimationDelay, "i",
     sotCharacter},
    {"Character::get_SpeechColor",
     (ScriptAPIFunction *) &Script_Character_get_SpeechColor, "", sotCharacter},
    {"Character::set_SpeechColor",
     (ScriptAPIFunction *) &Script_Character_set_SpeechColor, "i",
     sotCharacter},
    {"Character::get_SpeechView",
     (ScriptAPIFunction *) &Script_Character_get_SpeechView, "", sotCharacter},
    {"Character::set_SpeechView",
     (ScriptAPIFunction *) &Script_Character_set_SpeechView, "i", sotCharacter},
    {"Character::get_ThinkView",
     (ScriptAPIFunction *) &Script_Character_get_ThinkView, "", sotCharacter},
    {"Character::set_ThinkView",
     (ScriptAPIFunction *) &Script_Character_set_ThinkView, "i", sotCharacter},
    {"Character::get_Transparency",
     (ScriptAPIFunction *) &Script_Character_get_Transparency, "",
     sotCharacter},
    {"Character::set_Transparency",
     (ScriptAPIFunction *) &Script_Character_set_Transparency, "i",
     sotCharacter},
    {"Character::get_TurnBeforeWalking",
     (ScriptAPIFunction *) &Script_Character_get_TurnBeforeWalking, "",
     sotCharacter},
    {"Character::set_TurnBeforeWalking",
     (ScriptAPIFunction *) &Script_Character_set_TurnBeforeWalking, "i",
     sotCharacter},
    {"Character::get_View", (ScriptAPIFunction *) &Script_Character_get_View,
     "", sotCharacter},
    {"Character::get_WalkSpeedX",
     (ScriptAPIFunction *) &Script_Character_get_WalkSpeedX, "", sotCharacter},
    {"Character::get_WalkSpeedY",
     (ScriptAPIFunction *) &Script_Character_get_WalkSpeedY, "", sotCharacter},
    {"Character::get_x", (ScriptAPIFunction *) &Script_Character_get_x, "",
     sotCharacter},
    {"Character::set_x", (ScriptAPIFunction *) &Script_Character_set_x, "i",
     sotCharacter},
    {"Character::get_y", (ScriptAPIFunction *) &Script_Character_get_y, "",
     sotCharacter},
    {"Character::set_y", (ScriptAPIFunction *) &Script_Character_set_y, "i",
     sotCharacter},
    {"Character::get_z", (ScriptAPIFunction *) &Script_Character_get_z, "",
     sotCharacter},
    {"Character::set_z", (ScriptAPIFunction *) &Script_Character_set_z, "i",
     sotCharacter},
};
...
void addCharacterSystemScripting(AGSEngine *vm) {
	...
	state->addSystemFunctionImportList(ourFunctionList,
	                                   ARRAYSIZE(ourFunctionList));
}
