...
ScriptDynamicArray::ScriptDynamicArray(uint32 elementSize, uint32 elementCount,
                                       bool isManaged) {
...
uint32 ScriptDynamicArray::readUint32(uint offset) {
	...
	if (objectId >= _array.size())
		error("readUint32: offset %d is beyond end of dynamic array (%d) (size "
		      "%d)",
		      offset, _elementSize, _array.size());
...
bool ScriptDynamicArray::writeUint32(uint offset, uint value) {
	...
	uint32 objectId = offset / _elementSize;
	if (objectId >= _array.size())
		error("writeUint32: offset %d is beyond end of dynamic array (%d) "
		      "(size %d)",
		      offset, _elementSize, _array.size());
...
uint16 ScriptDynamicArray::readUint16(uint offset) {
	...
	uint32 objectId = offset / _elementSize;
	if (objectId >= _array.size())
		error("readUint16: offset %d is beyond end of dynamic array (%d) (size "
		      "%d)",
		      offset, _elementSize, _array.size());
	return _array[objectId]._value;
}
...
bool ScriptDynamicArray::writeUint16(uint offset, uint16 value) {
	...
	uint32 objectId = offset / _elementSize;
	if (objectId >= _array.size())
		error("writeUint16: offset %d is beyond end of dynamic array (%d) "
		      "(size %d)",
		      offset, _elementSize, _array.size());
	_array[objectId] = value;
	return true;
...
byte ScriptDynamicArray::readByte(uint offset) {
	...
	if (objectId >= _array.size())
		error(
		    "readByte: offset %d is beyond end of dynamic array (%d) (size %d)",
		    offset, _elementSize, _array.size());
	return _array[objectId]._value;
}
...
bool ScriptDynamicArray::writeByte(uint offset, byte value) {
	...
	uint32 objectId = offset;
	if (objectId >= _array.size())
		error("writeByte: offset %d is beyond end of dynamic array (%d) (size "
		      "%d)",
		      offset, _elementSize, _array.size());
	_array[objectId] = value;
	return true;
