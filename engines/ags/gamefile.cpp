...
bool GameFile::init() {
	...
	if (_version <= kAGSVer272) {
		...
		for (uint i = 0; i < _chars.size(); ++i) {
			...
			_chars[i]->_scriptName.setChar(toupper(_chars[i]->_scriptName[0]),
			                               0);
...
void GameFile::readGui(Common::SeekableReadStream *dta) {
	...
	for (uint n = 0; n < _guiGroups.size(); ++n) {
		...
		for (uint i = 0; i < group._objects.size(); ++i) {
			...
			switch (type) {
			case GOBJ_BUTTON: group._objects[i] = _guiButtons[id]; break;
			case GOBJ_LABEL: group._objects[i] = _guiLabels[id]; break;
			case GOBJ_INVENTORY: group._objects[i] = _guiInvControls[id]; break;
			case GOBJ_SLIDER: group._objects[i] = _guiSliders[id]; break;
			case GOBJ_TEXTBOX: group._objects[i] = _guiTextBoxes[id]; break;
			case GOBJ_LISTBOX: group._objects[i] = _guiListBoxes[id]; break;
