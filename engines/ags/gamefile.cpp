...
bool GameFile::init() {
	...
	if (_version > kAGSVer321)
		error("AGS version %d ('%s') is not supported", _version,
	...
	debug(1, "AGS game file for '%s' has version %d ('%s')", _gameName.c_str(),
	      _version, _versionString.c_str());
	...
	debug(2,
	      "%d views, %d characters (player is %d), %d inventory items, %d "
	      "dialog topics",
	...
	if (_version <= kAGSVer272) {
		...
		for (uint i = 0; i < _vm->_characters.size(); ++i) {
			...
			_vm->_characters[i]->_scriptName.setChar(
			    toupper(_vm->_characters[i]->_scriptName[0]), 0);
	...
	for (uint i = 0; i < _invItemInfo.size(); ++i) {
		...
		debug(5, "inventory item %d has script '%s'", i,
		      _invItemInfo[i]._scriptName.c_str());
	}
	...
	if (_version >= kAGSVer320) {
		...
		if (_options[OPT_SCORESOUND] > 0) {
			AudioClip *clip =
			    _vm->_audio->getClipByIndex(false, _options[OPT_SCORESOUND]);
			if (clip)
	...
	if ((_version >= kAGSVer300b) && _options[OPT_DEBUGMODE]) {
		...
		for (uint i = 0; i < roomCount; ++i) {
			...
			debug(5, "room %d (number %d) is '%s'", i, roomNumber,
			      roomName.c_str());
		}
	}
...
NewInteraction::readCommandList(Common::SeekableReadStream *dta) {
...
void InteractionScript::readFrom(Common::SeekableReadStream *dta) {
	...
	for (uint i = 0; i < eventCount; ++i) {
		...
		debug(8, "interaction script: event %d: script '%s'", i,
		      _eventScriptNames[i].c_str());
	}
}
...
void GameFile::readOldViews(Common::SeekableReadStream *dta) {
	for (uint n = 0; n < _views.size(); ++n) {
		...
		for (uint i = 0; i < numLoops; ++i) {
			...
			if (numFrames[i] > 0 &&
			    frames[i][numFrames[i] - 1]._pic == 0xffffffff) {
...
void GameFile::setDefaultMessages() {
	...
	// We only use the first and the last three (for the quit confirmation
	// dialog).
...
void GameFile::readGui(Common::SeekableReadStream *dta) {
	...
	for (uint n = 0; n < _guiGroups.size(); ++n) {
		...
		for (uint i = 0; i < group._controls.size(); ++i) {
			...
			switch (type) {
			case GOBJ_BUTTON: group._controls[i] = _guiButtons[id]; break;
			case GOBJ_LABEL: group._controls[i] = _guiLabels[id]; break;
			case GOBJ_INVENTORY:
				group._controls[i] = _guiInvControls[id];
				break;
			case GOBJ_SLIDER: group._controls[i] = _guiSliders[id]; break;
			case GOBJ_TEXTBOX: group._controls[i] = _guiTextBoxes[id]; break;
			...
			if (group._controls[i]->_parent)
				error("GUI control was assigned to more than one group "
				      "(corrupt game file?)");
...
void GameFile::readPropertyData(Common::SeekableReadStream *dta) {
	...
	for (uint i = 0; i < numProperties; ++i) {
		...
		debug(7, "schema property '%s' ('%s'), default '%s'",
		      property._name.c_str(), property._description.c_str(),
		      property._defaultValue.c_str());
	}
