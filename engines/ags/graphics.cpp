...
public:
	WFNFont(Common::SeekableReadStream *stream, uint multiplier) :
	    _maxCharWidth(0), _maxCharHeight(0), _multiplier(multiplier) {
		assert(_multiplier);
		...
		for (uint i = 0; i < 128; ++i) {
			...
			uint32 dataSize =
			    _glyphs[i].height * (((_glyphs[i].width - 1) / 8) + 1);
	...
	void drawChar(Graphics::Surface *surface, uint32 chr, int x, int y,
	              uint32 color) const {
		if (chr >= 128)
			chr = '?';
		...
		for (uint chrY = 0; chrY < _glyphs[chr].height * _multiplier; ++chrY) {
			...
			for (uint chrX = 0; chrX < _glyphs[chr].width * _multiplier;
			     ++chrX) {
				...
				// FIXME: we should obey the configured text multiply factor
				// here
				switch (surface->format.bytesPerPixel) {
				case 1: *(byte *) dest = color; break;
				case 2: *(uint16 *) dest = color; break;
				case 4: *(uint32 *) dest = color; break;
				}
			}
		}
	}
...
public:
	CursorDrawable(AGSEngine *vm) :
	    _vm(vm), _mouseFrame(0), _mouseDelay(0), _currentCursor(0xffffffff),
	    _cursorSprite(NULL) {}
	...
	void setMouseCursor(uint32 cursor) {
		...
		if ((cursor == MODE_USE) && cursorInfo._pic &&
		    (_vm->_gameFile->_hotDot || _vm->_gameFile->_invHotDotSprite)) {
	...
	void setCursorGraphic(uint32 spriteId) {
		...
		_alphaBlendCursor =
		    (bool) (_vm->_gameFile->_spriteFlags[spriteId] & SPF_ALPHACHANNEL);
	...
	void setMouseDelay(uint32 delay) { _mouseDelay = delay; }

	void tick() {
		assert(_currentCursor < _vm->_gameFile->_cursors.size());
		const MouseCursor &cursorInfo =
		    _vm->_gameFile->_cursors[_currentCursor];
		...
		if (view._loops.empty())
			error("cursor %d is using view %d which has no loops",
			      _currentCursor, cursorInfo._view);
		if (view._loops[0]._frames.empty())
			error("cursor %d is using view %d which has no frames in loop 0",
			      _currentCursor, cursorInfo._view);
...
bool AGSGraphics::getScreenSize() {
	...
	if ((_vm->_gameFile->_defaultResolution > 2) &&
...
uint32 AGSGraphics::resolveHardcodedColor(uint32 color) const {
	...
	const uint32 hardcodedColorTable[32] = {
	    // 16 system colors
	    0x000000, 0x0000A0, 0x00A000, 0x00A0A0, 0xA00000, 0xA000A0, 0xA05000,
	    0xA0A0A0, 0x505050, 0x5050FF, 0x50FF50, 0x50FFFF, 0xFF5050, 0xFF50FF,
	    0xFFFF50, 0xFFFFFF,
	    // 16 shades of grey
	    0x000000, 0x101010, 0x202020, 0x303030, 0x404040, 0x505050, 0x606060,
	    0x707070, 0x808080, 0x909090, 0xA0A0A0, 0xB0B0B0, 0xC0C0C0, 0xD0D0D0,
	    0xE0E0E0, 0xF0F0F0};

	const Graphics::PixelFormat inFormat(3, 8, 8, 8, 0, 16, 8, 0, 0);
	uint8 r, g, b;
	inFormat.colorToRGB(hardcodedColorTable[color], r, g, b);
	// FIXME: set alpha, add check above to make sure we don't reconvert if
	// alpha got set?
	return outFormat.RGBToColor(r, g, b);
}
...
void AGSGraphics::loadFonts() {
	...
	for (uint i = 0; i < _fonts.size(); ++i) {
		...
		Common::SeekableReadStream *stream =
		    _vm->getFile(Common::String::format("agsfnt%d.ttf", i));
		if (stream) {
			bool antialias = (_vm->_gameFile->_colorDepth != 1) &&
...
void AGSGraphics::initPalette() {
	for (uint i = 0; i < 256; ++i) {
		if (_vm->_gameFile->_paletteUses[i] != PAL_BACKGROUND) {
			_palette[i * 3 + 0] =
			    _vm->_gameFile->_defaultPalette[i * 3 + 0] * 4;
			_palette[i * 3 + 1] =
			    _vm->_gameFile->_defaultPalette[i * 3 + 1] * 4;
			_palette[i * 3 + 2] =
			    _vm->_gameFile->_defaultPalette[i * 3 + 2] * 4;
		}
	}
}

void AGSGraphics::newRoomPalette() {
	const byte *roomPal = _vm->getCurrentRoom()
	                          ->_backgroundScenes[_vm->_state->_bgFrame]
	                          ._palette;
	for (uint i = 0; i < 256; ++i) {
		if (_vm->_gameFile->_paletteUses[i] == PAL_BACKGROUND) {
...
void AGSGraphics::drawOutlinedString(uint fontId, Graphics::Surface *surface,
	...
	if (_vm->_gameFile->_fonts[fontId]._outline >= 0) {
		...
		font->drawString(surface, text, x - outlineDist, y, width,
		                 outlineColor);
		font->drawString(surface, text, x + outlineDist, y, width,
		                 outlineColor);
		font->drawString(surface, text, x, y + outlineDist, width,
		                 outlineColor);
		font->drawString(surface, text, x, y - outlineDist, width,
		                 outlineColor);
		font->drawString(surface, text, x - outlineDist, y - outlineDist, width,
		                 outlineColor);
		font->drawString(surface, text, x - outlineDist, y + outlineDist, width,
		                 outlineColor);
		font->drawString(surface, text, x + outlineDist, y - outlineDist, width,
...
void AGSGraphics::draw() {
	...
	bool guisTurnedOffAsDisabled =
	    (_vm->_guiDisabledStyle == GUIDIS_GUIOFF) && _vm->_guiDisabledState;
	...
	g_system->copyRectToScreen((byte *) _backBuffer.getPixels(),
	                           _width * _vm->_gameFile->_colorDepth, 0, 0,
	                           _width, _height);
	g_system->updateScreen();
}

void AGSGraphics::internalDraw(const Graphics::Surface *srcSurf,
...
static bool blitClip(Common::Point &pos, const Graphics::Surface *srcSurf,
                     Graphics::Surface *destSurf, bool mirrored, uint &startX,
...
void AGSGraphics::blit(const Graphics::Surface *srcSurf,
                       ...
                       uint transparency, bool mirrored) {
	if (transparency == 255)
		return;

	uint startX, startY, width, height;
	if (blitClip(pos, srcSurf, destSurf, mirrored, startX, startY, width,
	             height))
		return;
	...
	if (srcSurf->format.bytesPerPixel == 1) {
		...
		if (!transparency) {
			...
			for (uint y = 0; y < height; ++y) {
				uint16 *dest;
				if (mirrored)
					dest = (uint16 *) destSurf->getBasePtr(pos.x + width,
					                                       pos.y + y);
				else
		...
		for (uint y = 0; y < height; ++y) {
			...
			if (mirrored)
				dest =
				    (uint16 *) destSurf->getBasePtr(pos.x + width, pos.y + y);
			else
				dest = (uint16 *) destSurf->getBasePtr(pos.x, pos.y + y);
			const uint16 *src =
			    (const uint16 *) srcSurf->getBasePtr(startX, startY + y);
			for (uint x = 0; x < width; ++x) {
				...
				if (srcData != transColor) {
					...
					uint32 blendDest =
					    (destData | (destData << 16)) & 0x7E0F81F;
					uint32 blendSrc = (srcData | (srcData << 16)) & 0x7E0F81F;
					uint32 blended =
					    (blendSrc - blendDest) * transparency / 32 + blendDest;
					blended &= 0x7E0F81F;
		...
		if (srcSurf->format.aShift) {
			for (uint y = 0; y < height; ++y) {
				uint32 *dest;
				if (mirrored)
					dest = (uint32 *) destSurf->getBasePtr(pos.x + width,
					                                       pos.y + y);
				else
				...
				for (uint x = 0; x < width; ++x) {
					...
					if ((srcData & 0xffffff) != transColor) {
						...
						if (destSurf->format.aShift) {
							...
							uint alpha = MIN<uint>(0xff, transparency +
							                                 (destData >> 24));
							*dest = (alpha << 24) | (srcData & 0xffffff);
							...
							if (transparency)
								transparency++;
							uint32 blendedRB =
							    ((srcData & 0xFF00FF) - (destData & 0xFF00FF)) *
							        transparency / 256 +
							    (destData & 0xFF00FF);
							uint32 blended =
							    ((srcData & 0xFF00) - (destData & 0xFF00)) *
							        transparency / 256 +
							    (destData & 0xFF00);
							*dest = (blended & 0xFF00) |
							        (blendedRB & 0xFF00FF) | 0xFF000000;
						}
					}
					if (mirrored)
						dest--;
		...
		for (uint y = 0; y < height; ++y) {
			...
			if (mirrored)
				dest =
				    (uint32 *) destSurf->getBasePtr(pos.x + width, pos.y + y);
			else
				dest = (uint32 *) destSurf->getBasePtr(pos.x, pos.y + y);
			const uint32 *src =
			    (const uint32 *) srcSurf->getBasePtr(startX, startY + y);
...
bool Drawable::containsPoint(AGSEngine *vm, Common::Point point) {
	...
	if (!Common::Rect(pos.x, pos.y, pos.x + width, pos.y + height)
	         .contains(point))
		return false;
	...
	case 4:
		...
		break;
	default:
		error("Drawable::containsPoint: %dBpp not supported",
