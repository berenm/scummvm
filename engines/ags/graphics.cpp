...
public:
	WFNFont(Common::SeekableReadStream *stream) :
	    _maxCharWidth(0), _maxCharHeight(0) {
...
void AGSGraphics::drawOutlinedString(uint fontId, Graphics::Surface *surface,
	...
	const uint32 outlineColor =
	    resolveHardcodedColor(_vm->_state->_speechTextShadow);
...
void AGSGraphics::blit(const Graphics::Surface *srcSurf,
                       ...
                       uint transparency, bool mirrored, bool useAlpha) {
	if (transparency == 255)
		return;
...
bool Drawable::containsPoint(AGSEngine *vm, Common::Point point) {
	...
	Common::Point downPos(vm->divideDownCoordinate(pos.x),
	                      vm->divideDownCoordinate(pos.y));
	if (!Common::Rect(downPos.x, downPos.y, downPos.x + width,
	                  downPos.y + height)
	         .contains(point))
		return false;
