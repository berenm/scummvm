...
struct MoveList {
	...
	// (if not, characters need to make sure they end up on a walkable area when
	// done)
	bool _direct;
...
bool canSee(const Common::Point &from, const Common::Point &to,
            const Graphics::Surface *mask, Common::Point *lastGoodPos = NULL);
bool findPath(class AGSEngine *vm, const Common::Point &from,
              const Common::Point &to, const Graphics::Surface *mask,
              MoveList *moveList, int speedX, int speedY,
              bool onlyIfDestAllowed, bool ignoreWalls);
