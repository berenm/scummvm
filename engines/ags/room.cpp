...
static Graphics::Surface readLZSSImage(Common::SeekableReadStream *stream,
                                       ...
                                       byte *destPalette, uint32 imageBpp) {
	...
	case 4:
		...
		break;
	default:
		error("readLZSSImage: imageBpp of %d is invalid (must be 1, 2 or 4)",
		      imageBpp);
	}
...
void RoomObject::setGraphic(uint id) {
	if (_spriteId != id) {
		_spriteId = id;
		debugC(kDebugLevelGame, "object %d graphic changed to %d", _id,
		       _spriteId);
	}
...
void RoomObject::setObjectView(uint viewId) {
	if (viewId < 1 || viewId > _vm->_gameFile->_views.size())
		error("RoomObject::setObjectView: invalid view id %d (only have %d)",
		      viewId, _vm->_gameFile->_views.size());
	_view = viewId - 1;
...
void RoomObject::setObjectFrame(uint viewId, int loopId, int frameId) {
	if (viewId < 1 || viewId > _vm->_gameFile->_views.size())
		error("RoomObject::setObjectFrame: invalid view id %d (only have %d)",
		      viewId, _vm->_gameFile->_views.size());
	_view = viewId - 1;
	...
	if (_vm->getGameFileVersion() > kAGSVer272) {
		// Skip check on 2.x
		// "Fixes script error (invalid frame) in Ben There, Dan That! when
		// entering the graveyard."
		if (loopObj->_frames.empty())
			error("RoomObject::setObjectFrame: loop %d of view %d is empty",
			      _loop, _view);
	}
...
void RoomObject::animate(uint loopId, uint speed, uint repeat, uint direction) {
	...
	if (loopId >= _vm->_gameFile->_views[_view]._loops.size())
		error("RoomObject::animate: loop %d is too high (view %d only has %d "
		      "loops)",
	...
	if (_vm->getViewLoop(_view, loopId)->_frames.empty())
		error("RoomObject::animate: no frames in loop %d of view %d", _view,
		      loopId);
...
void RoomObject::update() {
	...
	if (_cycling >= ANIM_BACKWARDS) {
		...
		if (_frame == 0) {
			...
			if (_loop > 0 &&
			    _vm->getViewLoop(_view, _loop - 1)->shouldRunNextLoop()) {
		...
		if ((uint) _frame + 1 >= loop->_frames.size()) {
			...
			if (loop->shouldRunNextLoop()) {
				...
				if ((uint) _loop + 1 >=
					...
					error("RoomObject::update: last loop %d in view %d "
...
void RoomObject::move(int x, int y, int speed, bool ignoreWalkable) {
	if ((_vm->getGameFileVersion() <= kAGSVer261) && (speed == -1)) {
		// AGS <= 2.61 uses MoveObject with spp=-1 internally instead of
		// SetObjectPosition
		_pos.x = x;
		_pos.y = y;
		return;
	...
	if (findPath(_vm, Common::Point(objX, objY), Common::Point(x, y),
	             walkableMask, &_moveList, speed, speed, true,
	             ignoreWalkable)) {
		_moving = true;
...
bool RoomObject::isDrawMirrored() {
	return (_view != (uint16) -1) &&
	       (_vm->getViewFrame(_view, _loop, _frame)->_flags & VFLG_FLIPSPRITE);
}
...
void Room::initWalkBehinds() {
	// The original initialises the
	// walkBehindExists/walkBehindStartY/walkBehindEndY caches here too, but we
	// don't care about those (at least, not yet).
...
void Room::updateWalkBehinds() {
	const Graphics::Surface &background =
	    _backgroundScenes[_vm->_state->_bgFrame]._scene;
	...
	for (uint i = 1; i < _walkBehinds.size(); ++i) {
		...
		wb._surface.fillRect(Common::Rect(0, 0, width, height),
		                     _vm->_graphics->getTransparentColor());
		...
		for (uint y = wb._top; y <= wb._bottom; y++) {
			for (uint x = wb._left; x <= wb._right; x++) {
				...
				void *destPtr =
				    wb._surface.getBasePtr(x - wb._left, y - wb._top);
				switch (background.format.bytesPerPixel) {
				case 1: *(byte *) destPtr = *(const byte *) srcPtr; break;
				case 2: WRITE_UINT16(destPtr, READ_UINT16(srcPtr)); break;
				case 4: WRITE_UINT32(destPtr, READ_UINT32(srcPtr)); break;
				}
...
uint Room::getHotspotAt(int x, int y) {
	...
	if (hotspotId >= _hotspots.size())
		error("An invalid pixel was found on the room hotspot mask (color %d, "
		      "location: %d, %d)",
		      hotspotId, x, y);
...
uint Room::getObjectAt(int x, int y, int &objectYPos) {
	...
	for (uint i = 0; i < _objects.size(); ++i) {
		...
		if (obj->_view != (uint16)-1)
		    isFlipped = _vm->getViewFrame(obj->_view, obj->_loop,
		obj->_frame)->_flags & VFLG_FLIPSPRITE;*/
...
uint Room::getRegionAt(int x, int y) {
	...
	if (regionId >= _regions.size())
		error("An invalid pixel was found on the room region mask (color %d, "
		      "location: %d, %d)",
		      regionId, x, y);
	...
	while (true) {
		...
		case BLOCKTYPE_ANIMBKGRND: {
			...
			for (uint i = 1; i < _backgroundScenes.size(); ++i)
				_backgroundScenes[i]._scene = readLZSSImage(
				    dta, _vm->_graphics->getPixelFormat(),
				    _backgroundScenes[i]._palette, _bytesPerPixel);
		} break;
...
void Room::readMainBlock(Common::SeekableReadStream *dta) {
	...
	for (uint i = 0; i < _objects.size(); ++i) {
		...
		case 2:
			...
			_objects[i]->_visible = false;
			break;
		default: error("Room: invalid on status %d for object", objectOn);
		}
	}
	...
	if (!_loaded && _version >= kAGSRoomVer241) {
		...
		if (_version >= kAGSRoomVer300) {
			uint count =
			    1 + _hotspots.size() + _objects.size() + _regions.size();
			for (uint i = 0; i < count; ++i) {
	...
	if (_version >= 5)
		_backgroundScenes[0]._scene =
		    readLZSSImage(dta, _vm->_graphics->getPixelFormat(),
		                  _backgroundScenes[0]._palette, _bytesPerPixel);
	else
