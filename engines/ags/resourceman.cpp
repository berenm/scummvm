...
static const char *kGameExecutableSuffix = ".exe";
static const char *kSecretArchivePassword = "My\x1\xde\x4Jibzle";

namespace AGS {

ResourceManager::MasterArchive::MasterArchive(const Common::String n) :
    name(n) {
}
...
bool ResourceManager::init(const Common::String &masterArchive) {
	...
	if (!openMasterArchive(master)) {
		debug(1,
		      "ResourceManager::init(): Failed to open master archive \"%s\"",
		      master.name.c_str());
		return false;
	}
	...
	if ((master.version != 6) && (master.version != 10) &&
		...
		warning("ResourceManager::setArchive(): Unsupported version %d",
		        master.version);
		return false;
	}
	...
	if (master.file.readByte() != 0) {
		warning("ResourceManager::setArchive(): Archive not the first one in "
		        "the chain");
		return false;
	}
...
bool ResourceManager::openArchives(
    MasterArchive &master, const Common::Array<Common::String> &archives) {

	// Open the archive files
	_archives.reserve(archives.size());
	for (Common::Array<Common::String>::const_iterator archive =
	         archives.begin();
		...
		if (archive->hasSuffix(kMasterArchiveSuffix) ||
		    archive->hasSuffix(kGameExecutableSuffix)) {
	...
	// Fix up the offsets for files in our master archive, which might be
	// attached to the EXE
...
bool ResourceManager::readArchiveList_v06(MasterArchive &master) {
	...
	master.file.skip(
	    13); // "password dooberry", whatever that's supposed to mean
...
bool ResourceManager::readArchiveList_v21(MasterArchive &master) {
	...
	for (Common::Array<File>::iterator file = _files.begin();
	     file != _files.end(); ++file)
		file->archive =
		    (byte)((uint16) master.file.readByte() - getPseudoRand(seed));
...
ResourceManager::getFile(const Common::String &file) const {
