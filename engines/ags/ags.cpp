...
static uint mapKeycodeToAGS(KeyCode key) {
	...
	// Then, the rest.
	switch (key) {
	case KEYCODE_DELETE: return 383;
	case KEYCODE_UP: return 372;
	case KEYCODE_DOWN: return 380;
	case KEYCODE_RIGHT: return 377;
	case KEYCODE_LEFT: return 375;
	case KEYCODE_INSERT: return 382;
	case KEYCODE_HOME: return 371;
	case KEYCODE_END: return 379;
	case KEYCODE_PAGEUP: return 373;
	case KEYCODE_PAGEDOWN: return 381;
	case KEYCODE_F11: return 433;
	case KEYCODE_F12: return 434;
	case KEYCODE_LSHIFT: return 403;
	case KEYCODE_RSHIFT: return 404;
	case KEYCODE_LCTRL: return 405;
	case KEYCODE_RCTRL: return 406;
	case KEYCODE_RALT:
	case KEYCODE_LALT: return 407;
	default: return 0;
	}
}
...
AGSEngine::AGSEngine(OSystem *syst, const AGSGameDescription *gameDesc) :
    ...
    _scriptPlayerObject(NULL), _scriptMouseObject(NULL),
    _gameStateGlobalsObject(NULL), _saveGameIndexObject(NULL),
    _scriptSystemObject(NULL), _roomObjectState(NULL),
    _eventClaimed(EVENT_NONE), _currentRoom(NULL), _framesPerSecond(40),
...
bool AGSEngine::mainGameLoop() {
	...
	// call getLocationName, to force a GUI update if the result has changed
	// since last time (without this, @overhotspot@ often doesn't get updated)
	...
	if (_state->_getLocNameSaveCursor != UINT32_UNDEFINED &&
	    ...
	    _mouseOnGUI == UINT_UNDEFINED && _poppedInterface == UINT_UNDEFINED) {
		...
		debug(3, "Restore mouse to mode %d cursor %d",
		      _state->_restoreCursorModeTo, _state->_restoreCursorImageTo);
	}
...
void AGSEngine::tickGame(bool checkControls) {
	...
	if (!(_state->_groundLevelAreasDisabled & GLED_INTERACTION)) {
		...
		uint hotspotId =
		    _currentRoom->getHotspotAt(_playerChar->_x, _playerChar->_y);
...
void AGSEngine::updateEvents(bool checkControls) {
	...
	while (_eventMan->pollEvent(event)) {
		...
		case Common::EVENT_RBUTTONUP: mouseUp = kMouseRight; break;
		// FIXME: the others
		...
		case Common::EVENT_KEYDOWN: {
			...
			if (keycode == 8 || keycode == 13 ||
			    (keycode >= 32 && keycode < 256 && keycode != '[')) {
				for (uint i = 0; i < _gameFile->_guiGroups.size(); ++i) {
			...
			if (keycode >= 'a' && keycode <= 'z')
				keycode -= 32;
			queueGameEvent(kEventTextScript, kTextScriptOnKeyPress,
			               Common::mapKeycodeToAGS(event.kbd.keycode));
		} break;
	...
	Common::Point mousePos = _system->getEventManager()->getMousePos();
	bool guisTurnedOffAsDisabled =
	    (_guiDisabledStyle == GUIDIS_GUIOFF) && _guiDisabledState;
	...
	if (mouseUp) {
		...
		for (uint i = 0; i < group->_controls.size(); ++i) {
			...
			if (control->isOfType(sotGUIButton) ||
				...
				uint itemId = invControl->getItemAt(
				    mousePos - Common::Point(group->_x, group->_y) -
				    Common::Point(control->_x, control->_y));
				if (itemId != (uint) -1) {
					_state->_usedInvOn = itemId;
					if (getGameOption(OPT_HANDLEINVCLICKS)) {
						// let the script handle the click: 5 is left, 6 is
						// right
						queueGameEvent(kEventTextScript,
						               kTextScriptOnMouseClick,
						               mouseUp == kMouseRight ? 6 : 5);
	...
	if (buttonClicked) {
		...
		if (_state->_fastForward) {
			...
			// queueGameEvent(kEventInterfaceClick, activeGUI, (uint)-1, mouseUp
			// + 1); // FIXME: <--
...
public:
	...
	ScriptString *getStringObject(uint offset) {
		return _vm->getPlayerChar()->getStringObject(offset);
	}
...
public:
	...
	bool writeUint32(uint offset, uint value) {
		if (offset % 4 != 0)
			error("ScriptGameStateGlobalsObject::writeUint32: offset %d is "
			      "invalid",
			      offset);
		offset /= 4;
		if (offset >= _vm->_state->_globalVars.size())
			error("ScriptGameStateGlobalsObject::writeUint32: index %d is too "
			      "high (only %d globals)",
			      offset, _vm->_state->_globalVars.size());
...
uint AGSEngine::getWalkablePixelAt(const Common::Point &pos) {
	if (pos.x < 0 ||
	    (uint) convertToLowRes(pos.x) >= _currentRoom->_walkableMask.w ||
	    pos.y < 0 ||
	    (uint) convertToLowRes(pos.y) >= _currentRoom->_walkableMask.h)
		return (uint) -1;

	return *(const byte *) _currentRoom->_walkableMask.getBasePtr(
	    convertToLowRes(pos.x), convertToLowRes(pos.y));
}
...
// TODO: can we refactor this by getting rid of the areaId param? slightly
// tricky due to callers
uint AGSEngine::getAreaScalingFor(uint areaId, Common::Point pos) {
	...
	// FIXME: this should really be unsigned and we should sanity check data I
	// guess?
	int zoom;
...
AGSEngine::formatString(const Common::String &string,
	...
	for (uint i = 0; i < string.size(); ++i) {
		...
		if (paramId >= values.size()) {
			...
			warning("formatString: ran out of parameters for string '%s' (got "
			        "%d, needed #%d)",
	...
	if (paramId < values.size())
		warning("formatString: too many parameters for string '%s' (got %d, "
		        "used %d)",
		        string.c_str(), values.size(), paramId);
...
void AGSEngine::processGameEvent(const GameEvent &event) {
	...
	case kEventInterfaceClick:
		...
		break;
	case kEventNewRoom: scheduleNewRoom(event.data1); break;
...
bool AGSEngine::runInteractionEvent(NewInteraction *interaction, uint eventId,
	...
	bool ret = runInteractionCommandList(
	    interaction->_events[eventId], interaction->_events[eventId]._response,
	    commandsRunCount);
...
uint AGSEngine::getInteractionValue(const NewInteractionValue &value) {
	...
	if (value._val >= LOCAL_VARIABLE_OFFSET) {
		...
		if (val >= _currentRoom->_localVars.size())
			error(
			    "getInteractionValue: invalid local variable %d (only have %d)",
	...
	for (uint i = 0; i < commands.size(); ++i) {
		...
		case kActionRunScript:
			if (_eventBlockBaseName.contains("character") ||
			    _eventBlockBaseName.contains("inventory")) {
				// global script (character or inventory)
				Common::String name = makeTextScriptName(
				    _eventBlockBaseName, _eventBlockId, command._args[0]._val);
		...
		case kActionIfInventoryItemUsed:
			...
			if (runInteractionCommandList(event, command._children,
			                              commandsRunCount))
				return true;
			break;
		case kActionIfHasInventoryItem:
			if (_playerChar
			        ->_inventory[getInteractionValue(command._args[0])] == 0)
				break;
			if (runInteractionCommandList(event, command._children,
			                              commandsRunCount))
				return true;
		...
		case kActionIfVariablesEqual:
			if (getInteractionValue(command._args[0]) ==
			    getInteractionValue(command._args[1]))
				break;
			if (runInteractionCommandList(event, command._children,
			                              commandsRunCount))
				return true;
...
void AGSEngine::runUnhandledEvent(uint eventId) {
	...
	if (name.hasPrefix("hotspot")) {
		...
		// ignore: character stands on hotspot, mouse moves over hotspot, any
		// click
		if (eventId == kHotspotEventStandsOn ||
		    eventId == kHotspotEventClickOnHotspot ||
		    eventId == kHotspotEventMouseMovesOver)
			return;
...
bool AGSEngine::runClaimableEvent(const Common::String &name,
...
bool AGSEngine::runCharacterInteraction(uint charId, uint mode,
                                        bool checkOnly) {
	...
	switch (mode) {
	case MODE_LOOK: param = 0; break;
	case MODE_HAND: param = 1; break;
	case MODE_TALK: param = 2; break;
	case MODE_USE:
		param = 3;
		_state->_usedInv = _playerChar->_activeInv;
		break;
	case MODE_PICKUP: param = 5; break;
	case MODE_CUSTOM1: param = 6; break;
	case MODE_CUSTOM2: param = 7; break;
	}

	if (!checkOnly)
		debugC(kDebugLevelGame,
		       "run character interaction: hotspot %d, param %d", charId,
		       param);

	// can't use queueGameEvent, because "this ProcessClick is only executed
	// once in a eventlist"
	Common::String oldBaseName = _eventBlockBaseName;
	uint oldEventBlockId = _eventBlockId;
	...
	if (!_gameFile->_interactionsChar.empty()) {
		...
		if (param != (uint) -1)
			ret |=
			    runInteractionEvent(_gameFile->_interactionsChar[charId], param,
			                        kGeneralEventClick, param == 3, checkOnly);
		ret |= runInteractionEvent(_gameFile->_interactionsChar[charId],
		                           kGeneralEventClick, (uint) -1, false,
		                           checkOnly);
	} else {
		// 3.x
		if (param != (uint) -1)
			ret |= runInteractionScript(
			    &_gameFile->_charInteractionScripts[charId], param,
			    kGeneralEventClick, param == 3, checkOnly);
		ret |= runInteractionScript(&_gameFile->_charInteractionScripts[charId],
		                            kGeneralEventClick, (uint) -1, false,
		                            checkOnly);
	}
...
bool AGSEngine::runInventoryInteraction(uint itemId, uint mode,
                                        bool checkOnly) {
	...
	if (!checkOnly)
		debugC(kDebugLevelGame, "run inventory interaction: item %d, event %d",
		       itemId, eventId);

	bool ret;
	if (!_gameFile->_interactionsInv.empty()) {
		// 2.x
		ret = runInteractionEvent(_gameFile->_interactionsInv[itemId], eventId,
		                          (uint) -1, false, checkOnly);
	} else {
		// 3.x
		ret = runInteractionScript(&_gameFile->_invInteractionScripts[itemId],
		                           eventId, (uint) -1, false, checkOnly);
	}
...
bool AGSEngine::runHotspotInteraction(uint hotspotId, uint mode,
                                      bool checkOnly) {
	uint param = (uint) -1;

	switch (mode) {
	case MODE_TALK: param = 4; break;
	case MODE_WALK: param = 0; break;
	case MODE_LOOK: param = 1; break;
	case MODE_HAND: param = 2; break;
	case MODE_PICKUP: param = 7; break;
	case MODE_CUSTOM1: param = 8; break;
	case MODE_CUSTOM2: param = 9; break;
	case MODE_USE:
		param = 3;
		_state->_usedInv = _playerChar->_activeInv;
		break;
	...
	if (_state->_autoUseWalkToPoints &&
	...
	if (!checkOnly)
		debugC(kDebugLevelGame, "run hotspot interaction: hotspot %d, param %d",
		       hotspotId, param);
	...
	if (hotspot._interaction) {
		...
		if (param != (uint) -1)
			ret |= runInteractionEvent(hotspot._interaction, param,
			                           kHotspotEventClickOnHotspot, param == 3,
			                           checkOnly);
		if (!ret)
			ret |= runInteractionEvent(hotspot._interaction,
			                           kHotspotEventClickOnHotspot, (uint) -1,
			                           false, checkOnly);
	} else {
		// 3.x
		if (param != (uint) -1)
			ret |= runInteractionScript(&hotspot._interactionScripts, param,
			                            kHotspotEventClickOnHotspot, param == 3,
			                            checkOnly);
		ret |= runInteractionScript(&hotspot._interactionScripts,
		                            kHotspotEventClickOnHotspot, (uint) -1,
		                            false, checkOnly);
	}
...
bool AGSEngine::runObjectInteraction(uint objectId, uint mode, bool checkOnly) {
	...
	if (!checkOnly)
		debugC(kDebugLevelGame, "run object interaction: object %d, param %d",
		       objectId, param);
	...
	if (object->_interaction) {
		...
		if (param != (uint) -1)
			ret |=
			    runInteractionEvent(object->_interaction, param,
			                        kGeneralEventClick, param == 3, checkOnly);
		if (!ret)
...
void AGSEngine::runRegionInteraction(uint regionId, uint mode) {
	...
	assert(mode == kRegionEventStandsOn || mode == kRegionEventWalksOnto ||
	       mode == kRegionEventWalksOff);
...
bool AGSEngine::init() {
	...
	for (uint i = 0; i < _characters.size(); ++i) {
		if (_characters[i]->_view >= 0) {
			...
			if (getViewLoop(_characters[i]->_view, _characters[i]->_loop)
			        ->_frames.empty())
	...
	_scriptState->addSystemObjectImport(
	    "inventory", new ScriptObjectArray<InventoryItem>(
	                     &_gameFile->_invItemInfo, 8, "InventoryItem"));
	for (uint i = 0; i < _gameFile->_invItemInfo.size(); ++i) {
		InventoryItem &invItem = _gameFile->_invItemInfo[i];
...
enum { kOSDos = 1, kOSWindows = 2, kOSLinux = 3, kOSMac = 4 };
...
void AGSEngine::updateInvOrder() {
	for (uint i = 0; i < _characters.size(); ++i) {
		...
		chr->_invOrder.clear();
		for (uint itemId = 0; itemId < _gameFile->_invItemInfo.size();
		     ++itemId) {
	...
	_state->_invNumOrder =
	    _characters[_gameFile->_playerChar]->_invOrder.size();
...
uint AGSEngine::convertGUIDisabledStyle(uint style) {
	switch (style) {
	case 1: return GUIDIS_BLACKOUT;
	case 2: return GUIDIS_UNCHANGED;
	case 3: return GUIDIS_GUIOFF;
	default: return GUIDIS_GREYOUT;
	}
}
...
uint AGSEngine::getCharacterAt(const Common::Point &pos, int &charYPos) {
	...
	for (uint i = 0; i < _characters.size(); ++i) {
		...
		if (chr->_loop >= view._loops.size() ||
		    chr->_frame >= view._loops[chr->_loop]._frames.size())
			continue;
...
uint AGSEngine::getInventoryItemAt(const Common::Point &pos) {
	...
	Common::Point adjustedPos(multiplyUpCoordinate(pos.x),
	                          multiplyUpCoordinate(pos.y));
	...
	GUIInvControl *invControl = (GUIInvControl *) control;
	return invControl->getItemAt(adjustedPos -
	                             Common::Point(control->_x, control->_y));
}
...
Common::String AGSEngine::getLocationName(const Common::Point &pos) {
	...
	if (getGUIAt(pos) != (uint) -1) {
		...
		} else if (_state->_getLocNameLastTime > 1000 &&
		           _state->_getLocNameLastTime < 1000 + MAX_INV) {
...
Common::String AGSEngine::replaceTokens(const Common::String &text,
                                        bool macro) {
	Common::String out;
		...
		if (macro && macroName.equalsIgnoreCase("score"))
			...
			if (!_state->_disabledUserInterface) {
				...
				out += getLocationName(mousePos);
		           ...
		           (macroName.hasPrefix("in") || macroName.hasPrefix("gi"))) {
			// old-style token
			uint id = atoi(macroName.c_str() + 2);
			if (macroName[0] == 'i') {
				// inventory item count
				if (id < 1 || id >= _playerChar->_inventory.size())
					error("replaceTokens: bad inventory item %d in string '%s'",
					      id, text.c_str());
				out +=
				    Common::String::format("%d", _playerChar->_inventory[id]);
			} else {
				// global integer
				if (id >= _state->_globalScriptVars.size())
					error("replaceTokens: bad global integer %d in string '%s'",
					      id, text.c_str());
				out +=
				    Common::String::format("%d", _state->_globalScriptVars[id]);
			}
		} else
			out += '@' + macroName;
...
const CustomPropertySchemaProperty *
AGSEngine::getProperty(const Common::String &name) {
	int propertyId = -1;
...
bool AGSEngine::runScriptFunction(ccInstance *instance,
	...
	postScriptCleanup();
	// FIXME: sabotage any running scripts (set _eventClaimed) in the event of
	// restored game
