...
AGSEngine::AGSEngine(OSystem *syst, const AGSGameDescription *gameDesc) :
    ...
    _needsUpdate(true), _guiNeedsUpdate(true), _mouseFrame(0), _mouseDelay(0),
    ...
    _roomScriptFork(NULL), _currentRoom(NULL), _currentCursor(0xffffffff),
    _framesPerSecond(40), _lastFrameTime(0), _inNewRoom(kNewRoomStateNone),
    _newRoomWas(kNewRoomStateNone), _inEntersScreenCounter(0) {
...
void AGSEngine::loadNewRoom(uint32 id, Character *forChar) {
	...
	_scriptState->addSystemObjectImport(
	    "object", new ScriptObjectArray<RoomObject>(_currentRoom->_objects, 8),
	    true);
...
// occurred)
bool AGSEngine::runInteractionEvent(struct NewInteraction *interaction,
                                    uint eventId, uint checkFallback,
                                    bool isInventory) {
	...
	for (uint i = 0; i < commands.size(); ++i) {
		...
		case kActionRunScript:
			if (_eventBlockBaseName.contains("character") ||
				...
				// FIXME: bounds check?
				runTextScript(_roomScript,
				              makeTextScriptName(_eventBlockBaseName,
				                                 _eventBlockId,
...
int AGSEngine::convertToLowRes(int coord) {
	return (getGameOption(OPT_NATIVECOORDINATES)
	            ? (coord / _screenResolutionMultiplier)
	            : coord);
}
