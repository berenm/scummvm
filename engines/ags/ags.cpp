...
void AGSEngine::loadNewRoom(uint32 id, Character *forChar) {
	...
	if (_gameFile->_defaultResolution > 2 &&
	    !getGameOption(OPT_NATIVECOORDINATES))
...
bool AGSEngine::runInteractionScript(InteractionScript *scripts, uint eventId,
	...
	if (_eventBlockBaseName.contains("character") ||
		...
		if (_runningScripts.size())
			_runningScripts.back().queueScript(
			    scripts->_eventScriptNames[eventId]);
		else
...
void AGSEngine::displayAt(int x, int y, int width, const Common::String &text,
...
void AGSEngine::displayMain(int x, int y, int width, const Common::String &text,
...
void AGSEngine::displaySpeech(const Common::String &text, uint charId, int x,
                              int y, int width, bool isThought) {
...
void ExecutingScript::queueScript(const Common::String &name, uint p1,
                                  uint p2) {
	PendingScript i;
	i.name = name;
	i.p1 = p1;
