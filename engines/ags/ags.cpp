...
AGSEngine::AGSEngine(OSystem *syst, const AGSGameDescription *gameDesc) :
    ...
    _saveGameIndexObject(NULL), _scriptSystemObject(NULL), _currentRoom(NULL),
    ...
    _leavesScreenRoomId(-1), _blockingUntil(kUntilNothing),
...
void AGSEngine::scheduleNewRoom(int roomId) {
	...
	if (roomId < 0)
		error("NewRoom: room change requested to invalid room number %d",
		      roomId);
	...
	//  else
	//    quit("!NewRoom: two NewRoom/RunDialog/StopDialog requests within
	//    dialog");
	//  return;
	//}
