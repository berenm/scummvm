...
public:
	DialogOptionsDrawable(AGSEngine *vm, DialogTopic *topic,
...
DialogOptionsDrawable::DialogOptionsDrawable(
    AGSEngine *vm, DialogTopic *topic, Common::Array<uint> &displayedOptions) :
    _vm(vm),
	...
	if (_vm->_gameFile->_dialogBullet)
		_bulletWidth +=
		    _vm->getSprites()->getSpriteWidth(_vm->_gameFile->_dialogBullet) +
		    3;
	...
	if (_vm->getGameOption(OPT_DIALOGIFACE)) {
		...
		if (group->isTextWindow()) {
			...
			for (uint i = 0; i < _displayedOptions.size(); ++i) {
				...
				_font->wordWrapText(option._name,
				                    _textAreaWidth - _bulletWidth + 8, lines);

				for (uint j = 0; j < lines.size(); ++j) {
					uint lineWidth =
					    _font->getStringWidth(lines[j]) + 12 + _bulletWidth;
			...
			if (longestLine < _textAreaWidth) {
				...
				if (_textAreaWidth < (uint) _vm->multiplyUpCoordinate(
				                         _vm->_state->_minDialogOptionWidth))
					_textAreaWidth = _vm->multiplyUpCoordinate(
					    _vm->_state->_minDialogOptionWidth);
				// TODO: Original sanity-checks min <= max here, we should do
				// that elsewhere.
			}
...
uint DialogOptionsDrawable::getOptionsHeight() {
	...
	for (uint i = 0; i < _displayedOptions.size(); ++i) {
		...
		Common::Array<Common::String> lines;
		_font->wordWrapText(option._name, _textAreaWidth - _bulletWidth + 8,
		                    lines);
...
void DialogOptionsDrawable::drawDialogOptions() {
	...
	for (uint i = 0; i < _displayedOptions.size(); ++i) {
		...
		if ((_vm->_state->_readDialogOptionColor != (uint) -1) &&
		    (option._flags & DFLG_HASBEENCHOSEN))
			textColor = _vm->_state->_readDialogOptionColor;

		if (_selected == i) {
			// If the normal color is the same as the highlight color, use 13
			// instead.
		...
		if (bullet)
			_vm->_graphics->blit(bullet->_surface, &_surface,
			                     Common::Point(0, yPos), 0);

		if (_vm->getGameOption(OPT_DIALOGNUMBERED)) {
			uint xPos = 0;
			if (bullet)
				xPos = _vm->getSprites()->getSpriteWidth(
				           _vm->_gameFile->_dialogBullet) +
				       3;
			_vm->_graphics->drawOutlinedString(
			    _vm->_state->_normalFont, &_surface,
			    Common::String::format("%d.", i + 1), xPos, yPos,
			    _textAreaWidth, textColor);
		}
		...
		for (uint j = 0; j < lines.size(); ++j) {
			...
			_vm->_graphics->drawOutlinedString(
			    _vm->_state->_normalFont, &_surface, lines[j],
			    xPos + (j == 0 ? 0 : 9), yPos,
			    _textAreaWidth - _bulletWidth + 8, textColor);
			// FIXME: right height?
		...
		if (i + 1 != _displayedOptions.size())
			yPos +=
			    _vm->multiplyUpCoordinate(_vm->getGameOption(OPT_DIALOGGAP));
	}
...
int AGSEngine::showDialogOptions(uint dialogId, uint sayChosenOption) {
	...
	if (displayedOptions.empty())
		error("runConversation: no enabled options in dialog topic %d",
		      dialogId);

	int chosenOption;
	if (displayedOptions.size() == 1 && !parserEnabled &&
	    !_state->_showSingleDialogOption) {
		...
		while (!shouldQuit()) {
			...
			Common::Point mousePos = _system->getEventManager()->getMousePos() -
			                         Common::Point(0, drawable.getDrawPos().y);
			uint selectedWas = drawable._selected;
			drawable._selected = (uint) -1;
			if (mousePos.y >= 0 &&
			    mousePos.y <= (int) drawable._optionsHeight) {
...
void AGSEngine::doConversation(uint dialogId) {
	...
	// JJS: AGS 2.x always makes the mouse cursor visible when displaying a
	// dialog. "Fixed invisible mouse cursor in dialogs with some version 2.x
	// games, e.g. Murder in a Wheel."
	...
	while (result != RUN_DIALOG_STOP_DIALOG && !shouldQuit()) {
		...
		if (currDialogId != dialogId) {
			...
			// topic
			debugC(2, kDebugLevelGame, "changed from topic %d to topic %d",
		...
		if (result == RUN_DIALOG_GOTO_PREVIOUS) {
			if (previousTopics.empty()) {
				...
				debugC(2, kDebugLevelGame,
				       "back to previous topic (none left, done)");
...
int AGSEngine::runDialogScript(DialogTopic &topic, uint dialogId, uint16 offset,
                               uint optionId) {
	_saidSpeechLine = false;

	debugC(2, kDebugLevelGame,
	       "running dialog script for option %d of dialog %d", optionId,
	       dialogId);
		...
		while (scriptRunning && !shouldQuit()) {
			...
			case DCMD_OPTOFF:
				...
				if (param1 >= topic._options.size())
					error("runDialogScript: DCMD_OPTOFF: option %d is too high "
					      "(only have %d)",
			...
			case DCMD_OPTON:
				getDialogScriptParameters(topic, pos, &param1, NULL);
				if (param1 >= topic._options.size())
					error("runDialogScript: DCMD_OPTON: option %d is too high "
					      "(only have %d)",
					      param1, topic._options.size());
			...
			case DCMD_OPTOFFFOREVER:
				getDialogScriptParameters(topic, pos, &param1, NULL);
				if (param1 >= topic._options.size())
					error("runDialogScript: DCMD_OPTOFFFOREVER: option %d is "
	...
	debugC(2, kDebugLevelGame,
	       "finished dialog script for option %d of dialog %d, returning %d",
	       optionId, dialogId, result);
