...
void GUILabel::draw(Graphics::Surface *surface) {
	...
	for (uint i = 0; i < lines.size(); ++i) {
		...
		case GALIGN_LEFT:
			// nothing
			break;
		case GALIGN_CENTRE: x += _width / 2 - textWidth / 2; break;
		case GALIGN_RIGHT: x = _width - textWidth; break;
		default: error("GUILabel::draw: invalid alignment %d", _align);
		}
		_vm->_graphics->drawOutlinedString(_font, surface, lines[i], x, _y + y,
		                                   _width, color);
...
void GUITextBox::draw(Graphics::Surface *surface) {
	...
	uint textWidth = font->getStringWidth(_text); // FIXME
	_vm->_graphics->drawOutlinedString(_font, surface, _text, x, y, textWidth,
	                                   _textColor);
...
uint GUIListBox::getItemAt(Common::Point pos) {
	...
	// The original engine doesn't do this, but that causes problems if you
	// don't get a draw() before calling this.
	recalculate();
...
void GUIListBox::setTopItem(uint index) {
	...
	if (!_items.empty() || index)
		if (index >= _items.size())
			error("GUIListBox::setTopItem: %d is too high (only %d items)",
			      index, _items.size());
...
void GUIListBox::draw(Graphics::Surface *surface) {
	...
	for (uint i = 0; i < _numItemsFit; ++i) {
		...
		uint x;
		switch (_alignment) {
		case GALIGN_LEFT: x = _x + 1 + pixelSize; break;
		case GALIGN_RIGHT: x = rightHandEdge - textWidth; break;
		default: x = ((rightHandEdge - _x) / 2) + _x - (textWidth / 2); break;
		}
		_vm->_graphics->drawOutlinedString(_font, surface, text, x, y + 1,
		                                   textWidth, color);
	}
}
...
void GUIInvControl::scrollDown() {
	if (getCharToDisplay()->_invOrder.size() <=
	    (_topIndex + (_itemsPerLine * _numLines)))
		return;
...
void GUIInvControl::draw(Graphics::Surface *surface) {
	...
	for (uint i = 0; i < numItems; ++i) {
		...
		// draw inv graphic
		// FIXME
		Sprite *sprite = _vm->getSprites()->getSprite(
		    _vm->_gameFile->_invItemInfo[itemId]._pic);
		_vm->_graphics->blit(sprite->_surface, surface,
		                     Common::Point(curX, curY), 0);
...
void GUIInvControl::recalculateNumCells() {
	...
	if (_vm->getGameFileVersion() < kAGSVer270) {
		...
		_itemsPerLine = floor(
		    (float) _width / (float) _vm->multiplyUpCoordinate(_itemWidth) +
		    0.5f);
		_numLines = floor((float) _height /
		                      (float) _vm->multiplyUpCoordinate(_itemHeight) +
		                  0.5f);
		return;
	}
...
void GUIButton::updateAnimation() {
	...
	if (_animFrame >= view._loops[_animLoop]._frames.size()) {
		if (view._loops[_animLoop].shouldRunNextLoop()) {
			...
			while (_animLoop > 0 &&
			       view._loops[_animLoop - 1].shouldRunNextLoop())
				_animLoop--;
		} else {
			// end of animation
			_animating = false;
...
void GUIButton::draw(Graphics::Surface *surface) {
	...
	if (_vm->_guiDisabledStyle == GUIDIS_UNCHANGED ||
	    _vm->_guiDisabledStyle == GUIDIS_GUIOFF)
		drawDisabled = false;
	...
	if ((int) _usePic > 0 && (int) _pic > 0) {
		...
		_vm->_graphics->blit(sprite->_surface, surface, Common::Point(_x, _y),
		                     0);
	...
	// We don't use Graphics::TextAlign here, because we have to be
	// pixel-perfect.
	...
	_vm->_graphics->drawOutlinedString(_font, surface, text, useX, useY, _width,
	                                   color);
}
...
void GUIGroup::onMouseMove(const Common::Point &pos) {
	...
	if (!control || control->_id != (uint) _mouseOver) {
		...
		if (!control || control->isDisabled() || !control->isClickable()) {
			// either no control, or the control is disabled or not clickable -
			// ignore it
			_mouseOver = -1;
			return;
		}
...
bool GUIGroup::isMouseOver(const Common::Point &pos) {
	...
	return (pos.x >= _x && pos.y >= _y && pos.x <= _x + (int) _width &&
	        pos.y <= _y + (int) _height);
}
...
GUIControl *GUIGroup::getControlAt(const Common::Point &pos,
                                   bool mustBeClickable) {
	for (uint i = 0; i < _controls.size(); ++i) {
...
void GUIGroup::draw() {
	...
	// clear the surface, filling with either transparency or the background
	// color
	uint32 bgColor;
	if (_bgColor != 0)
	...
	if ((int) _bgPic > 0) {
		...
		_vm->_graphics->blit(sprite->_surface, &_surface, Common::Point(0, 0),
		                     0);
	}
	...
	for (uint i = 0; i < _controls.size(); ++i) {
		...
		if (control->isDisabled() &&
		    (_vm->_guiDisabledStyle == GUIDIS_BLACKOUT))
			continue;
		...
		// FIXME: highlighting
		// _surface.frameRect(Common::Rect(control->_x, control->_y, control->_x
		// + control->_width, control->_y + control->_height),
		// _vm->_graphics->resolveHardcodedColor(14));
	}
