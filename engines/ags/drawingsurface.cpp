...
DrawingSurface::DrawingSurface(AGSEngine *vm) :
    _vm(vm), _type(dstInvalid), _modified(false), _useHighResCoordinates(false),
    _currentColor(vm->_state->_rawColor) {

	if (_vm->getGameOption(OPT_NATIVECOORDINATES) &&
...
Graphics::Surface *DrawingSurface::startDrawing() {
	...
	case dstDynamicSurface: return _surface;
	default: error("DrawingSurface::startDrawing: internal error");
	}
}
