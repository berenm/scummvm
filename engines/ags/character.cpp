...
bool Character::writeUint32(uint offset, uint value) {
	switch (offset) {
	case 0: _defView = value; break;
	case 4: _talkView = value; break;
	case 8: _view = value; break;
	case 12: _room = value; break;
	case 16: _prevRoom = value; break;
	case 20: _x = value; break;
	case 24: _y = value; break;
	case 28: _wait = value; break;
	case 32: _flags = value; break;
	case 40: _idleView = value; break;
	case 52: _activeInv = value; break;
	case 56: _talkColor = value; break;
	case 60: _thinkView = value; break;
	case 76: _z = value; break;
	case 80: _walkWait = value; break;
	case 92: _indexId = value; break;
...
uint16 Character::readUint16(uint offset) {
	...
	switch (offset) {
	case 36: return _following;
	case 38: return _followInfo;
	case 44: return _idleTime;
	case 46: return _idleLeft;
	case 48: return _transparency;
	case 50: return _baseline;
	case 64: return _blinkView;
	case 66: return _blinkInterval;
	case 68: return _blinkTimer;
	case 70: return _blinkFrame;
	case 72: return _walkSpeedY;
	case 74: return _picYOffs;
	case 84: return _speechAnimSpeed;
	// (86 is reserved)
	case 88: return _blockingWidth;
	case 90: return _blockingHeight;
	case 96: return _picXOffs;
	case 98: return _walkWaitCounter;
	case 100: return _loop;
	case 102: return _frame;
	case 104: return _walking;
	case 106: return _animating;
	case 108: return _walkSpeed;
	case 110: return _animSpeed;
	case 714: // 112 + MAX_INV*2
		return _actX;
	case 716: return _actY;
	default: error("Character::readUint16: offset %d is invalid", offset);
	}
}
...
bool Character::writeUint16(uint offset, uint16 value) {
	...
	case 44: _idleTime = value; break;
	case 46: _idleLeft = value; break;
	case 48: _transparency = value; break;
	case 50: _baseline = value; break;
	case 64: _blinkView = value; break;
	case 66: _blinkInterval = value; break;
	case 68: _blinkTimer = value; break;
	case 70: _blinkFrame = value; break;
	case 72: _walkSpeedY = value; break;
	case 74: _picYOffs = value; break;
	case 84: _speechAnimSpeed = value; break;
	case 88: _blockingWidth = value; break;
	case 90: _blockingHeight = value; break;
	case 96: _picXOffs = value; break;
	case 98: _walkWaitCounter = value; break;
	case 100: _loop = value; break;
	case 102: _frame = value; break;
	case 104: _walking = value; break;
	case 106: _animating = value; break;
	case 108: _walkSpeed = value; break;
	case 110: _animSpeed = value; break;
	case 714: _actX = value; break;
	case 716: _actY = value; break;
	default: return false;
	}
...
byte Character::readByte(uint offset) {
	...
	switch (offset) {
	case 778: return _on;
	default: error("Character::readByte: offset %d is invalid", offset);
	}
}
...
public:
	...
	~CharacterNameString() { _parent->DecRef(); }

	const Common::String getString() { return _parent->_name; }

	void setString(const Common::String &text) { _parent->_name = text; }
...
ScriptString *Character::getStringObject(uint offset) {
	switch (offset) {
	case 718: return new CharacterNameString(this);
	default: return NULL;
	}
}
...
bool Character::update() {
	...
	if (_walking >= TURNING_AROUND) {
		...
		while (true) {
			...
			if ((turnLoopOrder[wantLoop] <
			     _vm->_gameFile->_views[_view]._loops.size()) &&
			    (!_vm->getViewLoop(_view, turnLoopOrder[wantLoop])
			          ->_frames.empty())) {
	...
	if (_room == _vm->getCurrentRoomId() && _walking > 0) {
		...
		if (_loop >= _vm->_gameFile->_views[_view]._loops.size())
			error("can't render character '%s' (id %d) because loop %d doesn't "
			      "exist in view %d",
			      _scriptName.c_str(), _indexId, _loop, _view + 1);

		uint framesInLoop =
		    _vm->_gameFile->_views[_view]._loops[_loop]._frames.size();
		if (_frame >= framesInLoop) {
			// force back to a valid frame
			if (framesInLoop == 0)
				error("can't render character '%s' (id %d) because loop %d has "
				      "no frames in view %d",
				      _scriptName.c_str(), _indexId, _loop, _view + 1);
		...
		if (!_walking) {
			...
			if (!(_flags & CHF_MOVENOTWALK)) {
				...
				_frame++;
				if (_frame >= framesInLoop) {
					// end of loop, so loop back round skipping the standing
					// frame (if possible)
					if (framesInLoop == 1)
						_frame = 0;
					else
				...
				_animWait = _vm->_gameFile->_views[_view]
				                ._loops[_loop]
				                ._frames[_frame]
				                ._speed +
				            _animSpeed;
	...
	if (_room == _vm->getCurrentRoomId() && (_animating || (_idleLeft < 0)) &&
		...
		if (_wait > 0)
			_wait--;
		else if (_vm->getGameOption(OPT_LIPSYNCTEXT) &&
		         false /* FIXME: check _charSpeaking */) {
			// FIXME
			return false;
		} else {
			...
			if (_animating & CHANIM_BACKWARDS) {
				if (_frame == 0) {
					...
					if (_loop > 0 && _vm->getViewLoop(_view, _loop - 1)
					                     ->shouldRunNextLoop()) {
						// If it's a Go-to-next-loop on the previous one, then
						// go back
						_loop--;
						_frame =
						    _vm->getViewLoop(_view, _loop)->_frames.size() - 1;
					} else if (_animating & CHANIM_REPEAT) {
						// repeating animation
						_frame =
						    _vm->getViewLoop(_view, _loop)->_frames.size() - 1;
						while (_vm->getViewLoop(_view, _loop)
						           ->shouldRunNextLoop()) {
							_loop++;
							_frame =
							    _vm->getViewLoop(_view, _loop)->_frames.size() -
							    1;
						}
					} else {
			...
			// FIXME: stop talking if we were _charSpeaking and the speech is
			// done
			...
			if (_frame >= loop->_frames.size()) {
				...
				if (loop->shouldRunNextLoop()) {
					...
					if ((uint) _loop + 1 >=
					    _vm->_gameFile->_views[_view]._loops.size())
						error("Character::update: last loop %d in view %d "
						      "requested to move to next loop",
						      _loop, _view);
					_loop++;
					_frame = 0;
				} else if (!(_animating & CHANIM_REPEAT)) {
					...
					if (!_vm->_state->_noMultiLoopRepeat) {
						...
						while (_loop > 0 && _vm->getViewLoop(_view, _loop - 1)
						                        ->shouldRunNextLoop())
							_loop--;
					}
				}
	...
	if (_idleView >= 1 && _idleLeft >= 0 && _room == _vm->getCurrentRoomId()) {
		// char is in current room, and has an idle anim which is not currently
		// playing

		if (!doingNothing || (_flags & CHF_FIXVIEW)) {
			// they are moving or animating, or the view is locked, so reset
			// idle timeout
			_idleLeft = _idleTime;
			...
			if (_idleLeft == -1) {
				debugC(kDebugLevelGame,
				       "character '%s' (id %d) now idle: view %d",
				...
				// don't try using diagonal loops if useDiagonal() doesn't
				// return 0 (either there aren't any, or they're standing frame
				// only)
				if (maxLoops > 4 && useDiagonal())
					maxLoops = 4;

				if (_idleTime > 0 && useLoop >= maxLoops) {
					// If it's not a "swimming"-type idleanim, choose a random
					// loop if there arent enough loops to do the current one.

					do {
						useLoop = _vm->getRandomSource()->getRandomNumber(
						    maxLoops - 1);
						// don't select a loop which is a continuation of a
						// previous one
					} while (useLoop > 0 &&
					         view._loops[useLoop - 1].shouldRunNextLoop());
				...
				animate(useLoop, _animSpeed + 5, (_idleTime == 0) ? 1 : 0,
				        true);
...
// or 2 if there are only standing frames for the diagonal loops (to allow
// providing smoother turning)
...
void Character::walk(int x, int y, bool ignoreWalkable, bool autoWalkAnims) {
	if (_room != _vm->getCurrentRoomId())
		error("Character::walk: character '%s' (id %d) in room %d, not in "
		      "current room (%d)",
	...
	if (to == from) {
		stopMoving();
		debugC(kDebugLevelGame,
		       "character '%s' (id %d) already at dest, not moving",
	...
	if (moveSpeedX == 0 && moveSpeedY == 0)
		warning("character '%s' (id %d) moving with walk speed 0",
		        _scriptName.c_str(), _indexId);

	Graphics::Surface *walkableMask = _vm->getWalkableMaskFor(_indexId);

	if (findPath(_vm, from, to, walkableMask, &_moveList, moveSpeedX,
	             moveSpeedY, true, ignoreWalkable)) {
		_walking = 1;
...
void Character::walkStraight(int x, int y) {
	if (_room != _vm->getCurrentRoomId())
		error("Character::walkStraight: character '%s' (id %d) in room %d, not "
		      "in current room (%d)",
		      _scriptName.c_str(), _indexId, _room, _vm->getCurrentRoomId());
...
void Character::addWaypoint(int x, int y) {
	if (_room != _vm->getCurrentRoomId())
		error("Character::addWaypoint: character '%s' (id %d) in room %d, not "
		      "in current room (%d)",
		      _scriptName.c_str(), _indexId, _room, _vm->getCurrentRoomId());
...
void Character::stopMoving() {
	...
	if (_walking > 0 && _walking < TURNING_AROUND) {
		// if it's not a MoveCharDirect, make sure they end up on a walkable
		// area
...
void adjustDiagonalMoveFrame(uint &useLoop, int mainDir, int otherDir,
                             uint loop1, uint loop2) {
...
void Character::fixPlayerSprite() {
	...
	if (_vm->getGameFileVersion() <= kAGSVer272) {
		...
		// "This fixes edge cases where the function was used to select specific
		// loops, e.g. in Murder in a Wheel."

		bool canRight =
		    _vm->_gameFile->_views[_view]._loops.size() >= 3 &&
		    !_vm->_gameFile->_views[_view]._loops[2]._frames.empty();
		bool canLeft = _vm->_gameFile->_views[_view]._loops.size() >= 2 &&
		               !_vm->_gameFile->_views[_view]._loops[1]._frames.empty();
	...
	if (_loop >= _vm->_gameFile->_views[_view]._loops.size() ||
	    _vm->_gameFile->_views[_view]._loops[_loop]._frames.empty() ||
	    !hasUpDownLoops()) {
...
bool Character::faceLocation(int x, int y) {
	...
	debugC(kDebugLevelGame,
	       "character '%s' (id %d) turning to face location: %d, %d",
	       _scriptName.c_str(), _indexId, x, y);
	...
	// Allow use of any available diagonal frames, even if they're
	// standing-only.
	uint diagonalState = useDiagonal();
	if (diagonalState != 1)
	...
	if (_vm->getGameOption(OPT_TURNTOFACELOC) && useLoop != _loop &&
	    _loop <= highestLoopForTurning) {
		if (!_vm->inEntersScreen()) {
			...
			if (_on == 1) {
				...
				startTurning(useLoop, diagonalState);
				// beware: the caller is reponsible for blocking and setting
				// _frame to 0, unlike in the original code
				return true;
			}
		}
	}
...
void Character::startTurning(uint useLoop, uint diagonalState) {
	...
	debugC(1, kDebugLevelGame,
	       "character '%s' (id %d) starting turn from %d (loop %d) to %d (loop "
	       "%d), direction %d",
	       _scriptName.c_str(), _indexId, fromIndex, _loop, toIndex, useLoop,
	       turnDirection);
	...
	for (int i = (int) fromIndex; i != (int) toIndex; i += turnDirection) {
		...
		if (_vm->_gameFile->_views[_view]
		        ._loops[turnLoopOrder[i]]
		        ._frames.empty())
			continue;
...
void Character::animate(uint loopId, uint speed, uint repeat,
                        bool noIdleOverride, uint direction) {
	if (_view < 0)
		error("Character::animate: character '%s (id %d) has no view set",
		      _scriptName.c_str(), _indexId);
	if ((uint) _view >= _vm->_gameFile->_views.size())
		error("Character::animate: character '%s' (id %d) has invalid view %d "
		      "(only have %d)",
		      _scriptName.c_str(), _indexId, _view,
		      _vm->_gameFile->_views.size());

	debugC(kDebugLevelGame,
	       "character '%s' (id %d) starting animation: view %d, loop %d, speed "
	       "%d, repeat %d",
	...
	if (loopId >= _vm->_gameFile->_views[_view]._loops.size())
		error("Character::animate: character '%s' (id %d) using invalid loop "
		      "%d for view %d (only have %d)",
		      _scriptName.c_str(), _indexId, loopId, _view,
		      _vm->_gameFile->_views[_view]._loops.size());
	...
	_wait = speed +
	        _vm->_gameFile->_views[_view]._loops[_loop]._frames[_frame]._speed;
	checkViewFrame();
}
...
void Character::findReasonableLoop() {
	...
	if (view._loops.empty())
		error("Character::findReasonableLoop: character '%s' (id %d) using "
		      "empty view %d (no loops)",
		      _scriptName.c_str(), _indexId, _view);
	...
	error("Character::findReasonableLoop: character '%s' (id %d) using empty "
	      "view %d (all loops are empty)",
	      _scriptName.c_str(), _indexId, _view);
}
...
void Character::changeView(uint viewId) {
	...
	if (viewId >= _vm->_gameFile->_views.size())
		error("Character::changeView: character '%s' (id %d) can't use view %d "
		      "(we only have %d)",
		      _scriptName.c_str(), _indexId, viewId,
		      _vm->_gameFile->_views.size());

	// fixed view, not running idle anim?
	if ((_flags & CHF_FIXVIEW) && (_idleLeft >= 0))
		warning("Character::changeView: character '%s' (id %d) had a fixed "
		        "view, you should release it first",
		        _scriptName.c_str(), _indexId);
...
void Character::lockView(uint viewId) {
	if (viewId < 1 || viewId > _vm->_gameFile->_views.size())
		error("Character::lockView: invalid view number %d (max is %d)", viewId,
		      _vm->_gameFile->_views.size());
	viewId--;
...
void Character::lockViewOffset(uint viewId, int xOffs, int yOffs) {
	...
	if (_vm->_graphics->_screenResolutionMultiplier == 1 &&
	    _vm->_gameFile->_defaultResolution >= 3) {
		// running a 640x400 game at 320x200, adjust
		xOffs /= 2;
		yOffs /= 2;
	} else if (_vm->_graphics->_screenResolutionMultiplier > 1 &&
	           _vm->_gameFile->_defaultResolution <= 2) {
...
void Character::unlockView() {
	if (_flags & CHF_FIXVIEW)
		debugC(kDebugLevelGame,
		       "released view of character '%s' back to default",
		       _scriptName.c_str());
...
void Character::setIdleView(int view, uint time) {
	if (view == 1)
		...
		error("Character::setIdleView: invalid view number %d (max is %d)",
		      view, _vm->_gameFile->_views.size());
	...
	if (_idleView >= 0)
		debugC(kDebugLevelGame,
		       "set idle view of character '%s' to %d (time %d)",
	...
	if (_flags & CHF_FIXVIEW)
		debugC(kDebugLevelGame,
		       "view of character '%s' is locked, idle view will not kick in "
		       "until released",
		       _scriptName.c_str());
...
void Character::setSpeechView(int view) {
	...
	if (view < 1 || (uint) view > _vm->_gameFile->_views.size())
		error("Character::setSpeechView: invalid view number %d (max is %d)",
		      view, _vm->_gameFile->_views.size());
...
void Character::setThinkView(int view) {
	...
	if (view < 2 || (uint) view > _vm->_gameFile->_views.size())
		error("Character::setThinkView: invalid view number %d (max is %d)",
		      view, _vm->_gameFile->_views.size());
...
void Character::addInventory(uint itemId, uint addIndex) {
	...
	if (_inventory[itemId] >= 32000)
		error("Character::addInventory: can't carry more than 32000 of one "
		      "inventory item");
	_inventory[itemId]++;
	...
	if (_invOrder.size() >= MAX_INVORDER)
		error("Character::addInventory: too many inventory items added, can "
		      "only have 500 at one time");
...
void Character::loseInventory(uint itemId) {
	...
	debugC(kDebugLevelGame,
	       "removing inv %d (had %d) from character '%s' (id %d)", itemId,
	       _inventory[itemId], _scriptName.c_str(), _indexId);
...
void Character::setActiveInventory(uint itemId) {
	...
	if (_inventory[itemId] < 1)
		error("setActiveInventory: character doesn't have any items with id %d",
		      itemId);
...
bool Character::moveToNearestWalkableAreaWithin(int range, int step) {
	...
	// TODO: This could be a lot more efficient than it is, if it ever shows up
	// in a profile enough to care.
	for (int newX = startX; newX < width; newX += step) {
		for (int newY = startY; newY < height; newY += step) {
			...
			if (newX <= boundary.left || newX >= boundary.right ||
			    newY <= boundary.top || newY >= boundary.bottom)
				continue;

			// otherwise, calculate distance from target
			uint thisIs = (uint) sqrt(
			    (float) ((newX - x) * (newX - x) + (newY - y) * (newY - y)));
			if (thisIs < nearest) {
				nearest = thisIs;
				nearestX = newX;
				nearestY = newY;
...
void Character::moveToNearestWalkableArea() {
	...
	byte pixValue = 0;
	if (x >= 0 && y >= 0 && x < _vm->getCurrentRoom()->_width &&
	    y < _vm->getCurrentRoom()->_height)
		pixValue =
		    *(byte *) _vm->getCurrentRoom()->_walkableMask.getBasePtr(x, y);
...
Common::Point Character::getDrawPos() {
	...
	return Common::Point(_vm->multiplyUpCoordinate(_x) - getDrawWidth() / 2 +
	                         _picXOffs,
	                     _vm->multiplyUpCoordinate(_y) -
	                         _vm->getSprites()->getSpriteHeight(spriteId) -
...
int Character::getDrawOrder() const {
	return getBaseline() +
	       ((_flags & CHF_NOWALKBEHINDS) ? _vm->getCurrentRoom()->_height : 0);
}
...
void Character::getDrawTint(int &lightLevel, int &luminance, byte &red,
