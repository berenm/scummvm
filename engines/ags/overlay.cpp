...
// TODO: the 'color' parameter is abused in these functions to mean two things,
// should switch it back to uint and add another parameter for the extra -ve
// functionality
uint AGSEngine::createTextOverlayCore(int x, int y, uint width, uint fontId,
	...
	return displayMain(x, y, width, text, 2, fontId, -color, false, allowShrink,
	                   false);
}

uint AGSEngine::createTextOverlay(int x, int y, uint width, uint fontId,
                                  int color, const Common::String &text) {
	uint allowShrink = 0;
...
uint AGSEngine::addScreenOverlay(int x, int y, uint type,
	...
	ScreenOverlay *overlay = new ScreenOverlay(this, Common::Point(x, y), type,
	                                           surface, alphaChannel);
...
void AGSEngine::updateSpeech() {
	...
	if (_state->_messageTime >= 0) {
		...
		if (_audio->_channels[SCHAN_SPEECH]->isValid()) {
			if (_audio->_channels[SCHAN_SPEECH]->isPlaying() &&
			    !_state->_fastForward) {
		...
		if (_state->_messageTime < 1) {
			if (_state->_fastForward) {
				...
				_state->_ignoreUserInputUntilTime =
				    _system->getMillis() +
				    _state->_ignoreUserInputAfterTextTimeoutMs;
			}
		}
	}
...
void AGSEngine::displayAtY(int y, const Common::String &text) {
	...
	if (getGameOption(OPT_ALWAYSSPCH)) {
		displaySpeechAt(-1, (y > 0) ? divideDownCoordinate(y) : y, -1,
		                _gameFile->_playerChar, text);
	} else {
		// Normal "Display" in text box

		// FIXME: some dirty stuff here

		displayAt(-1, y, _graphics->_width / 2 + _graphics->_width / 4,
		          getTranslation(text), 1);
	}
}

void AGSEngine::displayAt(int x, int y, int width, Common::String text,
                          int blocking, int asSpeech, bool isThought,
	...
	displayMain(x, y, width, text, blocking, usingFont, asSpeech, isThought,
	            allowShrink, overlayPositionFixed);
...
uint AGSEngine::displayMain(int x, int y, int width, const Common::String &text,
                            int blocking, int usingFont, int asSpeech,
                            bool isThought, int allowShrink,
	...
	// TODO: this code, to remove '&5 ' type prefixes and handle '[', should be
	// elsewhere (it is in break_up_text_into_lines in original code)
	...
	// JJS: AGS 2.x: If the screen is faded out, fade in again when displaying a
	// message box. "The narrator messages after a fadeout in a Tale of Two
	// Kingdoms are now drawn."
	...
	surface.create((width > 0) ? width : 2,
	               lines.size() * fontHeight + extraHeight,
	               _graphics->getPixelFormat());
	surface.fillRect(Common::Rect(0, 0, surface.w, surface.h),
	                 _graphics->getTransparentColor());
	...
	if (!text.empty() && text[0] != ' ' && width) {
		if (asSpeech) {
			...
			for (uint i = 0; i < lines.size(); ++i) {
				...
				_graphics->drawOutlinedString(usingFont, &surface, lines[i],
			...
			// FIXME: adjust_y_coordinate_for_text to try and work around some
			// ttf thing
...
void AGSEngine::displaySpeech(Common::String text, uint charId, int x, int y,
                              int width, bool isThought) {
	if (charId >= _characters.size())
		error("displaySpeech: character %d is invalid", charId);
	Character *speakingChar = _characters[charId];
	if (speakingChar->_view < 0 ||
	    (uint) speakingChar->_view >= _gameFile->_views.size())
		error("displaySpeech: character %d has invalid view %d", charId,
		      speakingChar->_view);
	...
	if (text[0] == '&') {
		...
		int speechId = atoi(text.c_str() + 1);
		if (speechId <= 0)
			error("displaySpeech: invalid voice id %d for auto-voice",
			      speechId);
	...
	if (speakingChar->_room != _displayedRoom) {
		...
		if (useX < 0)
			useX =
			    multiplyUpCoordinate(speakingChar->_x) - _graphics->_viewportX;
		if (useX < 2)
			useX = 2;
		...
		if (speakingChar->_loop >=
		    _gameFile->_views[speakingChar->_view]._loops.size())
			speakingChar->_loop = 0;

		// FIXME: sanity-check view/loop/frame

		if (useY < 0) {
			// FIXME: frame 0: uint picId = getViewFrame(speakingChar->_view,
			// speakingChar->_loop, 0)->_pic;
			useY = multiplyUpCoordinate(speakingChar->getEffectiveY()) -
			       _graphics->_viewportY;
		...
		if ((useView >= 0) && (getGameOption(OPT_SPEECHTYPE) > 0)) {
			...
			if (width < 0) {
				...
				int relX = multiplyUpCoordinate(speakingChar->_x) -
				           _graphics->_viewportX;
				if ((relX < _graphics->_width / 4) ||
				    (relX > _graphics->_width - (_graphics->_width / 4)))
	...
	displayAt(useX, useY, useWidth, text, 0, textColor, isThought, allowShrink,
...
void AGSEngine::displaySpeechAt(int x, int y, int width, uint charId,
...
uint AGSEngine::displaySpeechBackground(uint charId,
                                        const Common::String &text) {
	...
	uint overlayType = createTextOverlay(
	    OVR_AUTOPLACE, charId, _graphics->_width / 2, _state->_speechFont,
...
void AGSEngine::displayMessage(uint messageId, int y) {
	...
	if (messageId >= 500) {
		...
		// TODO: Original source code uses long-dead display_message_aschar
		// here. Work out what's going on there, and if it was ever useful.
...
ScreenOverlay::ScreenOverlay(AGSEngine *vm, const Common::Point &pos, uint type,
                             ...
                             bool alphaChannel) :
    _vm(vm),
    _pos(pos), _type(type), _timeout(0), _bgSpeechForChar((uint) -1),
    _positionRelativeToScreen(true), _hasAlphaChannel(alphaChannel),
    _isValid(true) {
...
Common::Point ScreenOverlay::getDrawPos() {
	...
	if (_pos.x == OVR_AUTOPLACE) {
		...
		if (chr->_room != _vm->getCurrentRoomId()) {
			return Common::Point(
			    (_vm->_graphics->_width / 2) - (_surface.w / 2),
		...
		pos.x = _vm->multiplyUpCoordinate(chr->_x) - _surface.w / 2 -
		        _vm->_graphics->_viewportX;
		if (pos.x < 0)
			pos.x = 0;
...
void ScreenOverlay::getDrawTint(int &lightLevel, int &luminance, byte &red,
